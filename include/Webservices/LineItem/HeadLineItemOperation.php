...
require_once "include/Webservices/LineItem/HeadInventoryOperation.php";
...
class HeadLineItemOperation  extends HeadActorOperation {
	...
	function getProductPrice($productId){
		...
		$sql = "select unit_price from jo_products where productid=?";
	...
	public function updateParent($createdElement,$parent){
		...
		$result = $this->pearDB->pquery('SELECT * FROM jo_inventorychargesrel WHERE recordid = ?', array($parentId));
	...
	public function getCollectiveTaxList(){
		$db = PearDatabase::getInstance();
		$sql = 'select * from jo_inventorytaxinfo where deleted=0';
		$params = array();
		$result = null;
		$transactionSuccessful = vtws_runQueryAsTransaction($sql,$params,$result);
		if(!$transactionSuccessful){
	...
	private function getProductTaxList($productId){
		$db = PearDatabase::getInstance();
		$sql = 'select * from jo_producttaxrel inner join jo_inventorytaxinfo on
			jo_producttaxrel.taxid=jo_inventorytaxinfo.taxid and deleted=0
			where productid=?';
		$params = array($productId);
		$result = null;
		$transactionSuccessful = vtws_runQueryAsTransaction($sql,$params,$result);
