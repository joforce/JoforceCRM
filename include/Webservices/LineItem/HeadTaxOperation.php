...
require_once "include/Webservices/HeadActorOperation.php";
require_once 'include/Webservices/LineItem/HeadTaxMeta.php';
require_once("include/events/include.inc");
require_once 'modules/com_jo_workflow/VTEntityCache.inc';
require_once 'data/CRMEntity.php';
require_once 'include/events/SqlResultIterator.inc';
require_once 'include/Webservices/LineItem/HeadLineItemMeta.php';
...
/**
 * Description of HeadTaxOperation
 */
class HeadTaxOperation  extends HeadActorOperation {
	...
	public function __construct($webserviceObject, $user, $adb, $log) {
		...
		$this->meta = new HeadTaxMeta($this->entityTableName,$webserviceObject,$adb,$user);
	...
	public function create($elementType, $taxElement) {
		...
		$sql = "alter table jo_inventoryproductrel add column $taxName decimal(7,3)";
	...
	private function dropRow($id) {
		$sql = 'delete from jo_inventorytaxinfo where taxid = ?';
		$params = array($id);
		$result = $this->pearDB->pquery($sql, $params);
	}

	private function getCurrentTaxName() {
		$sql = 'select taxname from jo_inventorytaxinfo order by taxid desc limit 1';
		$params = array();
		$result = $this->pearDB->pquery($sql, $params);
	...
	private function getTaxName($element) {
		if ($element['taxlabel']) {
			$sql = 'SELECT taxname FROM jo_inventorytaxinfo WHERE taxlabel = ?';
	...
	private function sanitizeElementForInsert($element) {
		...
		if ($taxFormula) {
			...
			$sql = 'SELECT taxid, method FROM jo_inventorytaxinfo WHERE taxname IN ('.generateQuestionMarks($taxFormulaElements).')';
