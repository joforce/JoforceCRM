...
Vtiger_Widget_Js('Vtiger_Pie_Widget_Js', {}, {
    ...
    generateData: function() {
        ...
        for (var index in data) {
            ...
            chartDataName.push(row.last_name);
            chartData.push(parseFloat(row.amount));
        }
        return {
            'chartData': chartData,
            'name': chartDataName
    ...
    loadChart: function() {
        ...
        var config = {
            ...
            data: {
                datasets: [{
                    ...
                    backgroundColor: [
                          '#FF7599', '#A9FF96', '#95CEFF', '#999EFF', '#FDEC6D', '#F7A35C', '#FFBC75'
                    ],
                    label: 'Dataset 1'
                }],
        ...
        window.myPie = myPieChart = new Chart(ctx, config);
...
Vtiger_Widget_Js('Vtiger_Barchat_Widget_Js', {}, {
	...
	var myBarChart;
        var ctx = document.getElementById(canvas_id).getContext("2d");
        window.myBar = myBarChart = new Chart(ctx, {
            type: 'bar',
            data: barChartData,
...
Vtiger_Widget_Js('Vtiger_MultiBarchat_Widget_Js', {
    ...
    generateDataSets: function(chartRelatedData) {
        ...
        for (var i = 0; i <= chartRelatedData['labels'].length; i++) {
            var dataset = {
                label: chartRelatedData['labels'][i],
    ...
    loadChart: function() {
        ...
        var barChartData = {
            labels: chartRelatedData['ticks'],
            datasets: this.generateDataSets(chartRelatedData)
        };
        var links = chartRelatedData.links;
        var items = document.querySelectorAll("canvas");
        var canvas_id = this.getContainer().find('canvas').attr('id');
        var canvas = document.getElementById(canvas_id);
	var myMultiBarChart;
        var ctx = document.getElementById(canvas_id).getContext("2d");
        window.myBar = myMultiBarChart = new Chart(ctx, {
            type: 'bar',
            data: barChartData,
            options: {
                responsive: true,
        ...
        canvas.onclick = function (evt) {
            var activePoints = myMultiBarChart.getElementsAtEvent(evt);
	    var activeDataset = myMultiBarChart.getDatasetAtEvent(evt);
	    var datasetIndex = activeDataset[0]._datasetIndex;
            var firstPoint = activePoints[0];
            window.location.href= links[datasetIndex][firstPoint._index];
        }
