					...
					{if $FIELD_MODEL->getFieldDataType() eq "picklist"}
						...
						<select class="col-sm-9 select2" name="{$NAME_ATTR}">
							{foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$PICKLIST_VALUES}
								<option value="{Head_Util_Helper::toSafeHTML($PICKLIST_NAME)}" {if $DEFAULT_VALUE eq $PICKLIST_NAME} selected {/if}>{vtranslate($PICKLIST_VALUE, $SELECTED_MODULE_NAME)}</option>
							{/foreach}
						</select>
					{elseif $FIELD_MODEL->getFieldDataType() eq "multipicklist"}
						...
						<select multiple class="col-sm-9 select2" name="{$NAME_ATTR}[]" >
							{foreach item=PICKLIST_VALUE from=$PICKLIST_VALUES}
								<option value="{Head_Util_Helper::toSafeHTML($PICKLIST_VALUE)}" {if in_array(Head_Util_Helper::toSafeHTML($PICKLIST_VALUE), $FIELD_VALUE_LIST)} selected {/if}>{vtranslate($PICKLIST_VALUE, $SELECTED_MODULE_NAME)}</option>
							{/foreach}
						</select>
