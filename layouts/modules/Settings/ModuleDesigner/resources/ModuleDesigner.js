...
var modGeneratorUIType = [{
	...
	{
		num: 53,
		label: 'Dropdown combo radiobutton',
		dbtype: 'VARCHAR(128)',
		datatype: 'V',
		mandatory: false,
		two_columns: false
	...
	{
		num: 55,
		label: 'Salutation and Firstname',
		dbtype: 'VARCHAR(128)',
		datatype: 'V',
		mandatory: false,
		two_columns: false
...
$(document).ready(
	function () {
		...
		jmd_container.find('.md-tab').click(
			function (event) {
				if ($(this).attr("id") != "md-tab-general" && jmd_container.find("select[name='module_directory_template']").val() == '') {
					...
					$("#md-page-" + tab_id).addClass('active');
					$("#md-page-" + tab_id).show();
				}
			}
		);
			...
			.droppable({
				...
				drop: function (event, ui) {
					if (custom_links_ul_droppable) {
						md_openPopup(site_url + "index.php?module=" + MD_MODULE_NAME + "&view=EditCustomLink&type=" + $(ui.draggable).text());
			...
			.droppable({
				...
				drop: function (event, ui) {
					if (related_lists_ul_droppable) {
						md_openPopup(site_url + "index.php?module=" + MD_MODULE_NAME + "&view=EditRelatedList&relmodule=" + $(ui.draggable).text());
			...
			.droppable({
				...
				drop: function (event, ui) {
					if (events_ul_droppable) {
						md_openPopup(site_url + "index.php?module=" + MD_MODULE_NAME + "&view=EditEvent&event=" + $(ui.draggable).text());
...
function md_createMandatoryFields() {
	...
	o_field.tableName = 'jo_crmentity';
...
function md_updateFieldsTableName(ti) {
	...
	var new_module_table_name = 'jo_' + $(ti).val().toLowerCase();
...
function md_selectDirectory(cb, moduleName, moduleDirName) {
	...
	if (cb != undefined) {
		...
		calledUrl = site_url + "index.php?module=" + MD_MODULE_NAME + "&action=GetLanguages&dirname=modules/" + MD_MODULE_NAME + "/templates/" + template + "/LANGUAGES/&parent=Settings";

		autoSelectModuleDirectory = false;
	} else if (moduleDirName != undefined) {
		calledUrl = site_url + "index.php?module=" + MD_MODULE_NAME + "&action=GetLanguages&dirname=" + moduleDirName + "&mod=" + moduleName + "&findlangdir=1&parent=Settings";
	} else if (moduleName != undefined) {
		calledUrl = site_url + "index.php?module=" + MD_MODULE_NAME + "&action=GetLanguages&dirname=languages/&mod=" + moduleName + "&parent=Settings";
	}
		...
		success: function (response) {
			if (!response.success) {
				...
				for (var i = 0; i < response.result.languages.length; i++) {
					...
					html += '<tr class="md-module-name-translation col-sm-6">' +
						'<td class="col-sm-4">' + app.vtranslate('LBL_MODULE_NAME_TRANSLATION', MD_QUALIFIED_MODULE_NAME) + ' <em>' + language + '</em></td>' +
						'<td colspan="2" class="col-sm-8 p0"><input type="text" name="module_label_' + language + '" class="md-medium-text-input inputElement" /></td>' +
						'</tr>';

					html += '<tr class="md-module-name-translation col-sm-6">' +
						'<td class="col-sm-4">' + app.vtranslate('LBL_MODULE_NAME_SINGLE_TRANSLATION', MD_QUALIFIED_MODULE_NAME) + ' <em>' + language + '</em></td>' +
						'<td colspan="2" class="col-sm-8 p0"><input type="text" name="module_label_single_' + language + '" class="md-medium-text-input inputElement" /></td>' +
						'</tr>';
				}
...
function md_addBlock(o_block, isImporting) {
	...
	} else //Add new block
	{
		...
		switch (a_blocks.length) {
			case 0:
				o_block.label = 'LBL_BLOCK_GENERAL_INFORMATION';
				break;
			case 1:
				o_block.label = 'LBL_BLOCK_SYSTEM_INFORMATION';
				break;
			default:
				o_block.label = 'LBL_BLOCK_' + a_blocks.length;
				break;
		}
	...
	row += '<div class="title" style="cursor:pointer;">';
	row += '<input type="text" name="' + o_block.id + '-label" value="' + o_block.label + '" class="md_block_name inputElement" readonly="readonly" style="cursor:pointer;" /> ';
	...
	row += '<ul class="md-fields-ul content" block_id="' + o_block.id + '"></ul>';
	row += '</li>';

	$("#md-blocks-ul").append(row);

	var x = $("#md-blocks-ul li").last().position();
	window.scroll({
		top: x.top,
		left: x.left
	});
	...
	jfields_ul.droppable({
		...
		drop: function (event, ui) {
			...
			if (!stopPopup) {
				...
				md_openPopup(site_url + "index.php?module=" + MD_MODULE_NAME + "&view=EditField&mod=" + editedModuleName + "&exist=" + existantModule + "&uitype=" + uitype_num);
			}
		}
	});
...
function md_addField(o_field, isImporting) {
	...
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_field.mandatory ? 'mandatory' : 'mandatory-bw') + '.png" class="' + (o_field.mandatory ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_field.isEntityIdentifier ? 'entityidentifier' : 'entityidentifier-bw') + '.png" class="' + (o_field.isEntityIdentifier ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_field.inFilterAll ? 'filterall' : 'filterall-bw') + '.png" class="' + (o_field.inFilterAll ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_field.inPopup ? 'popup' : 'popup-bw') + '.png" class="' + (o_field.inPopup ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_field.inRelatedList ? 'related' : 'related-bw') + '.png" class="' + (o_field.inRelatedList ? 'md-icon-active' : 'md-icon-inactive') + '" /><br /> ';
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_field.quickCreate ? 'create' : 'create-bw') + '.png" class="' + (o_field.quickCreate ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_field.massEditable ? 'massedit' : 'massedit-bw') + '.png" class="' + (o_field.massEditable ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_field.defaultValue != '' ? 'defaultvalue' : 'defaultvalue-bw') + '.png" class="' + (o_field.defaultValue != '' ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_field.helpInfoLabel != '' ? 'helpinfo' : 'helpinfo-bw') + '.png" class="' + (o_field.helpInfoLabel != '' ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_field.readOnly ? 'readonly' : 'readonly-bw') + '.png" class="' + (o_field.readOnly != '' ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '</div>';
	row += '</li>';
	...
	jmd_container.find(".md-field").mousedown(function () {
		fieldEdited = this;
		stopPopup = true;
	});
	...
	$("#" + o_field.id).dblclick(
		function () {
			if ($(this).attr("md-editable") == 'true') {
				...
				md_openPopup(site_url + "index.php?module=" + MD_MODULE_NAME + "&view=EditField&mod=" + editedModuleName + "&exist=" + existantModule + "&field=" + escape(field_json));
			} else {
...
function md_addCustomLink(o_customLink, isImporting) {
	...
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_customLink.icon != '' ? 'icon' : 'icon-bw') + '.png" class="' + (o_customLink.icon != '' ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_customLink.handlerPath != '' ? 'handler-path' : 'handler-path-bw') + '.png" class="' + (o_customLink.handlerPath != '' ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_customLink.handlerClass != '' ? 'handler-class' : 'handler-class-bw') + '.png" class="' + (o_customLink.handlerClass != '' ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_customLink.handler != '' ? 'handler' : 'handler-bw') + '.png" class="' + (o_customLink.handler != '' ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '</div>';
	...
	$("#" + o_customLink.id).dblclick(
		function () {
			...
			md_openPopup(site_url + "index.php?module=" + MD_MODULE_NAME + "&view=EditCustomLink&customlink=" + escape(JSON.stringify(o_customLink)));
		}
	);
}
...
function md_addRelatedList(o_relatedList, isImporting) {
	...
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_relatedList.presence == 1 ? 'presence' : 'presence-bw') + '.png" class="' + (o_relatedList.presence == 1 ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_relatedList.actionAdd ? 'add' : 'add-bw') + '.png" class="' + (o_relatedList.actionAdd ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_relatedList.actionSelect ? 'select' : 'select-bw') + '.png" class="' + (o_relatedList.actionSelect != '' ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '</div>';
...
function md_addEvent(o_event, isImporting) {
	...
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_event.isActive ? 'active' : 'active-bw') + '.png" class="' + (o_event.isActive ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_event.cond != '' ? 'cond' : 'cond-bw') + '.png" class="' + (o_event.cond != '' ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '<img src="' + site_url + 'layouts/modules/Settings/' + MD_MODULE_NAME + '/assets/images/' + (o_event.dependentOn != '' ? 'dependent' : 'dependent-bw') + '.png" class="' + (o_event.dependentOn != '' ? 'md-icon-active' : 'md-icon-inactive') + '" /> ';
	row += '</div>';
	...
	$("#" + o_event.id).dblclick(
		function () {
			...
			md_openPopup(site_url + "index.php?module=" + MD_MODULE_NAME + "&view=EditEvent&eventobj=" + escape(JSON.stringify(o_event)));
		}
	);
}
...
function md_makePackage(installModule) {
	...
	$.ajax({
		...
		type: "POST",
		data: {
			structure: JSON.stringify(a_postData)
		},
			...
			if (data.success) {
				...
				if (zip_url) {
					$.ajax({
						...
						success: function () {
							if (installModule) {
								md_installModule(md_moduleName, md_version, zip_url, md_moduleParentTab);
							} else {
...
function showLoadModulePopup() {
	var site_url = jQuery('#joforce_site_url').val();
	md_openPopup(site_url + "index.php?module=" + MD_MODULE_NAME + "&view=GetModules");
}

function showUploadModulePopup() {
	var site_url = jQuery('#joforce_site_url').val();
	md_openPopup(site_url + "index.php?module=" + MD_MODULE_NAME + "&view=UploadModule");
}
...
function md_loadModule(moduleNameOrPath, uploadedModule) {
	...
	if (uploadedModule) {
		calledUrl = site_url + "index.php?module=" + MD_MODULE_NAME + "&action=GetManifestStructure&path=" + moduleNameOrPath + "&parent=Settings";
	} else {
		calledUrl = site_url + "index.php?module=" + MD_MODULE_NAME + "&action=GetManifestStructure&mod=" + moduleNameOrPath + "&parent=Settings";
	}
...
function setModuleData(autoSelectModuleDirectory, moduleDirName) {
	...
	if (o_module.type != undefined) {
		if (o_module.type != 'module') {
			//TODO Show correct content - JoForce
			alert('Extension module not supported');
			return;
		}
	}
	...
	if (autoSelectModuleDirectory) {
		...
		jmd_container.find("input[name='old_module_table_name']").val('jo_' + o_module.name.toLowerCase());
	} else {
