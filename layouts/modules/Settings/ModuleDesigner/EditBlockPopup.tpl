...
<html>
	...
	<div style="font-family: Arial,Verdana,'Times New Roman',sans-serif;">
		...
		<tr>
		    <td colspan="2"><h3>{vtranslate("LBL_BLOCK_DESCRIPTION", $QUALIFIED_MODULE)}</h3></td>
		</tr>
		<tr>
		    <td>{vtranslate("LBL_BLOCK_LABEL", $QUALIFIED_MODULE)}</td>
		    <td>
			<input type="text" size="50" name="label" value="{if !empty($a_block.label)}{$a_block.label}{/if}" onkeyup="md_setLabel(this, 'label', 'LBL_')" readonly='readonly' style="cursor: not-allowed;" />
		    </td>
		</tr>
		...
		{foreach item=language from=$a_languages}
			...
			<td colspan="2">
			    <input type="text" name="label-{$language}" size="50" value="{if !empty($a_block.$label)}{$a_block.$label}{/if}" />
			</td>
		    </tr>
		{/foreach}
		...
		<tr>
		    <td>
			<input type="checkbox" name="show_title" {if !isset($a_block.showTitle) || $a_block.showTitle == 1}checked="checked"{/if} /> 
			{vtranslate("LBL_BLOCK_SHOW_TITLE", $QUALIFIED_MODULE)}
		    </td>
		    <td>
			<input type="button" value="{vtranslate('LBL_SAVE', $QUALIFIED_MODULE)}" onclick="md_popupSave()" />
		    </td>
		</tr>
		<tr>
		    <td colspan="2">
			<input type="checkbox" name="visible" {if !isset($a_block.visible) || $a_block.visible == 1}checked="checked"{/if} />
			{vtranslate("LBL_BLOCK_VISIBLE", $QUALIFIED_MODULE)}
		    </td>
		</tr>
		<tr>
		    <td colspan="2">
			<input type="checkbox" name="create_view" {if !empty($a_block.createView)}checked="checked"{/if} />
			{vtranslate("LBL_BLOCK_CREATE_VIEW", $QUALIFIED_MODULE)}
		    </td>
		</tr>
		<tr>
		    <td colspan="2">
			<input type="checkbox" name="edit_view" {if !empty($a_block.editView)}checked="checked"{/if} />
			{vtranslate("LBL_BLOCK_EDIT_VIEW", $QUALIFIED_MODULE)}
		    </td>
		</tr>
		<tr>
		    <td colspan="2">
			<input type="checkbox" name="detail_view" {if !empty($a_block.detailView)}checked="checked"{/if} />
			{vtranslate("LBL_BLOCK_DETAIL_VIEW", $QUALIFIED_MODULE)}
		    </td>
		</tr>
		<tr>
		    <td colspan="2">
			<input type="checkbox" name="display_status" {if !isset($a_block.displayStatus) || $a_block.displayStatus == 1}checked="checked"{/if} /> 
			{vtranslate("LBL_BLOCK_DISPLAY_STATUS", $QUALIFIED_MODULE)}
		    </td>
		</tr>
		<tr>
		    <td colspan="2">
			<input type="checkbox" name="is_custom" {if !empty($a_block.isCustom)}checked="checked"{/if} />
			{vtranslate("LBL_BLOCK_IS_CUSTOM", $QUALIFIED_MODULE)}
		    </td>
		</tr>
		<tr>
		    <td colspan="2">
			<input type="checkbox" name="is_list" {if !empty($a_block.isList)}checked="checked"{/if} />
			{vtranslate("LBL_BLOCK_IS_LIST", $QUALIFIED_MODULE)}
		    </td>
		</tr>
	    </table>
	</div>
