...
var Settings_Picklist_Js = {
	...
	registerAddItemEvent: function () {
		...
		jQuery('#addItem').on('click', function (e) {
			...
			popupShown = true;
			app.helper.showModal(clonedCreateView.html(), {
				cb: callBackFunction
			});
		});
	},
			...
			app.request.post({
				...
				if (data) {
					...
					app.helper.showSuccessNotification({
						message: app.vtranslate('JS_LIST_UPDATED_SUCCESSFULLY')
					});
				}
			});
		});
	...
	registerScrollForNonEditablePicklistValues: function (container) {
		...
		app.helper.showVerticalScroll(jQuery(container).find('.nonEditablePicklistValues'), {
			setHeight: '70px'
		});
	},
		...
		var params = {
			submitHandler: function (form) {
				...
				app.request.post({
					...
					app.helper.hideProgress();
					app.helper.showSuccessNotification({
						message: app.vtranslate('JS_ITEM_ADDED_SUCCESSFULLY')
					});
			...
			submitHandler: function (form) {
				...
				app.request.post({
					...
					if (!error) {
						...
						app.helper.showSuccessNotification({
							message: app.vtranslate('JS_STATUS_UPDATE_SUCCESS_MSG')
						});
	...
	showDeleteItemForm: function (params) {
		...
		var callBackFunction = function (data) {
			...
			vtUtils.showSelect2ElementView(jQuery('[name="delete_value[]"]'), {
	...
	saveSequence: function () {
		...
		app.request.post({
			...
			if (data) {
				app.helper.showSuccessNotification({
					message: app.vtranslate('JS_SEQUENCE_UPDATED_SUCCESSFULLY')
				});
			}
		});
	},
	...
	registerAssingValueToRoleTabClickEvent: function () {
		...
		$(document).ready(function () {
			$("li.taxesTab").click(function () {
				$(".taxesTab").toggleClass("active");
				$("li.chargesTab").removeClass("active");
				//$(".taxRegionsTab").removeClass("active");

			});
			$(".chargesTab").click(function () {
				$("li.chargesTab").toggleClass("active");
				$("li.taxesTab").removeClass("active");;
				//$(".taxRegionsTab").removeClass("active");
			});
		});
	},
