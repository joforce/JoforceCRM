...
<html lang="en">
	...
	<style type="text/css">
		...
		.group-heading .basicChkbox{
			...
			width:24px;
			height:24px;
			background: url("layouts/skins/images/check_radio_sprite.png") no-repeat 0 0;
			opacity: 0.65;
		}
		.group-desc{
			padding-bottom:9px;
		...
		input[type="radio"] + div::before
		{
			...
			height: 24px;
			margin-right: 6px;
			background: url("layouts/skins/images/check_radio_sprite.png") no-repeat 0 -24px;
	...
	<script>
		...
		$(document).on('click','#view-enabled-modules', function() {
		    var selected = [];
		    $('#SystemSetup input:checked').each(function() {
		        selected.push($(this).data('pkgname'));
		    });
		    $.ajax({
			type: 'GET',
...
console.log(data);
	...
	<div class="container-fluid page-container">
		...
		<div class="span12 inner-container">
		    ...
		    <form id="SystemSetup" class="form" method="POST" action="index.php?module=Users&action=SystemSetupSave">
			    ...
			    {foreach from=$PACKAGES_LIST key=PACKAGE_NAME item=PACKAGE_INFO}
				    ...
				    <div class="row-fluid">
					    ...
					    <img src="layouts/skins/images/{$PACKAGE_INFO.imageName}" alt="Head Logo"/>
				    	</div>
					    ...
					    <div class="group-heading">
						    ...
						    <div class="pull-right">
						    	<input type="checkbox" name="packages[{$PACKAGE_NAME}]" data-pkgname="{$PACKAGE_NAME}" /><div class="chkbox"></div>
						    </div>
					    	{else}
					    ...
					    <div class="group-desc">
					    	<br> <p>{$PACKAGE_INFO.description}</p>
					    </div>
				    	</div>
				    </div>
				    {if $COUNTER eq 0}		
					    ...
					    <div class="offset2 span10">
					    	<input type="hidden" name="packages[{$PACKAGE_NAME}]" data-pkgname="{$PACKAGE_NAME}" value="on"/>
				    ...
				    <div class="reveal_modules">
						...
						{foreach from=$PACKAGE_INFO.modules item=MODULE_NAME}
						    ...
						    {if $ITEMCOUNTER eq 3}
						        <br> {assign var=ITEMCOUNTER value=0}
						    {/if}
						{/foreach}
					    </div>
					</div>
		    ...
		    <div id="system_modules_div_form" style="display:none;"></div>
		    <div class="row-fluid">
		    	<div class="span9">
			...
			<div class="span3">
			    <div class="button-container">
			    	<a class="btn btn-large btn-primary" id="view-enabled-modules"> Next </a>
				<button type="submit" class="btn btn-large btn-primary next go-next" value="Next" style="display:none;"> Next </button>
			    </div>
			</div>
		    </div>
		</form>
