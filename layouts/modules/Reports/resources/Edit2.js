...
Reports_Edit_Js("Reports_Edit2_Js",{},{
	...
	isFormValidate : function(){
		...
		var result = Head_MultiSelect_Validator_Js.invokeValidation(selectElement);
		if(result != true){
	...
	arrangeSelectChoicesInOrder : function() {
		...
		//var coulmnListSelectElement = Head_CustomView_Js.getColumnSelectElement();
	...
	registerLineItemCalculationLimit : function() {
		...
		if(jQuery.inArray(primaryModule, inventoryModules) !== -1 || secondaryIsInventory) {
			jQuery('.CalculationFields').on('change', 'input[type="checkbox"]', function(e) {
				...
				var reg = new RegExp(/cb:jo_inventoryproductrel*/);
				var attr = element.is(':checked');
				var moduleCalculationFields = jQuery('.CalculationFields input[type="checkbox"]').not('[value^="cb:jo_inventoryproductrel"]');
				var lineItemCalculationFields = jQuery('.CalculationFields').find('[value^="cb:jo_inventoryproductrel"]');
	...
	registerLineItemCalculationLimitOnLoad : function() {
		var moduleCalculationFields = jQuery('.CalculationFields input[type="checkbox"]').not('[value^="cb:jo_inventoryproductrel"]');
		var lineItemFields = jQuery('.CalculationFields').find('[value^="cb:jo_inventoryproductrel"]');
