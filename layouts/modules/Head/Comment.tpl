...
{strip}
	...
	<div class="singleComment">
		...
		<div class="row">
		    <div class="col-lg-12">
			<div class="media">
			    <div class="{if $RECENT_COMMENT eq 1}media-left title col-lg-2 col-md-2 col-sm-2 p0 mt10{else}col-lg-1 col-md-1 col-sm-1 d-flex{/if}" id="{$COMMENT->getId()}" >
				{assign var=CREATOR_NAME value=$COMMENT->getCommentedByName()}
				<div class="commentImage commentInfoHeader" data-commentid="{$COMMENT->getId()}" data-parentcommentid="{$COMMENT->get('parent_comments')}" data-relatedto = "{$COMMENT->get('related_to')}" style="margin:unset;">
				    {assign var=IMAGE_PATH value=$COMMENT->getImagePath()}
				    {if !empty($IMAGE_PATH)}
					<img src="{$SITEURL}{$IMAGE_PATH}" width="100%" height="100%" align="left" style="border-radius:50%;">
				    {else}
					<div class="name"><span><strong> {$CREATOR_NAME|substr:0:2} </strong></span></div>
				    {/if}
				</div>
			    </div>
			    <div class="{if $RECENT_COMMENT eq 1}media-body col-lg-10 col-md-10 col-sm-9{else}col-lg-11 col-md-11 col-sm-11{/if}">
				<div class="comment" style="line-height:1;">
					<div class="d-flex justify-content-between">
						<h5 class="creatorName" style="color:blue">{$CREATOR_NAME}</h5>&nbsp;&nbsp;
					...
					{if $ROLLUP_STATUS and $COMMENT->get('module') ne $MODULE_NAME}
						...
						<a href="index.php?module={$COMMENT->get('module')}&view=Detail&record={$COMMENT->get('related_to')}">{$ENTITY_NAME[$COMMENT->get('related_to')]}</a>
					    </span>&nbsp;&nbsp;
					{/if}
					...
					<div class="editedStatus" name="editStatus">
						...
						<p class="text-muted" style="text-align:left;"><small>[ {vtranslate('LBL_EDIT_REASON',$MODULE_NAME)} ] : <span name="editReason" class="textOverflowEllipsis">{nl2br($REASON_TO_EDIT)}</span></small></p>
					    </div>
					    {if $COMMENT->getCommentedTime() neq $COMMENT->getModifiedTime()}
						<p class="text-muted cursorDefault" style="text-align:left;"><small><em>{vtranslate('LBL_MODIFIED',$MODULE_NAME)}</em></small>&nbsp;<small title="{Head_Util_Helper::formatDateTimeIntoDayString($COMMENT->getModifiedTime())}" class="commentModifiedTime">{Head_Util_Helper::formatDateDiffInStrings($COMMENT->getModifiedTime())}</small></p>
					    {/if}
					</div>
					<div >
