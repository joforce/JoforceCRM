		...
		{foreach key=$index item=HISTORY from=$HISTORIES}
			...
			{if $MODELNAME == 'ModTracker_Record_Model'}
				...
				{if $PROCEED}
											...
											{if $FIELD && $FIELD->getFieldInstance() && $FIELD->getFieldInstance()->isViewableInDetailView()}
												<div>
													...
													{if $FIELD->get('prevalue') neq '' && $FIELD->get('postvalue') neq '' && !($FIELD->getFieldInstance()->getFieldDataType() eq 'reference' && ($FIELD->get('postvalue') eq '0' || $FIELD->get('prevalue') eq '0'))}
														&nbsp;{vtranslate('LBL_FROM')} <b>{Head_Util_Helper::toHead6SafeHTML($FIELD->getDisplayValue(decode_html($FIELD->get('prevalue'))))}</b>
													{else if $FIELD->get('postvalue') eq '' || ($FIELD->getFieldInstance()->getFieldDataType() eq 'reference' && $FIELD->get('postvalue') eq '0')}
														&nbsp; <b> {vtranslate('LBL_DELETED')} </b> ( <del>{Head_Util_Helper::toHead6SafeHTML($FIELD->getDisplayValue(decode_html($FIELD->get('prevalue'))))}</del> )
													{else}
													...
													{if $FIELD->get('postvalue') neq '' && !($FIELD->getFieldInstance()->getFieldDataType() eq 'reference' && $FIELD->get('postvalue') eq '0')}
														{vtranslate('LBL_TO')} <b>{Head_Util_Helper::toHead6SafeHTML($FIELD->getDisplayValue(decode_html($FIELD->get('postvalue'))))}</b>
													{/if}    
												</div>
											{/if}
										{else}
					...
					<p class="pull-right muted" style="padding-right:10px;"><small title="{Head_Util_Helper::formatDateTimeIntoDayString("$TIME")}">{Head_Util_Helper::formatDateDiffInStrings("$TIME")}</small></p>
				</div>
			{/if}
			{else if $MODELNAME == 'ModComments_Record_Model'}
				<div class="row">
					...
					<p class="pull-right muted" style="padding-right:10px;"><small title="{Head_Util_Helper::formatDateTimeIntoDayString("$COMMENT_TIME")}">{Head_Util_Helper::formatDateDiffInStrings("$COMMENT_TIME")}</small></p>
				</div>
			{/if}
		{/foreach}
