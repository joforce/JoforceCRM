...
jQuery.Class('Head_Validations_Helper', {
	getInstance : function() {
		return new Head_Validations_Helper();
	}
},{ 
...
jQuery.validator.addMethod("greaterThanDependentField", function(value, element, params) {
	var helper = Head_Validations_Helper.getInstance();
...
(function ($) {
	...
	$.fn.vtValidate = function (params) {
			...
			errorPlacement: function (error, element) {
				if ($(error).text()) {
					...
					element.qtip({
						...
						show: {
							event: 'Head.Validation.Show.Messsage'
						},
						hide: {
							event: 'Head.Validation.Hide.Messsage'
						},
					...
					element.trigger('Head.Validation.Show.Messsage');
				}
				/*
		...
		this.on('Head.PostReference.Selection Head.PostReference.QuickCreateSave', function (e) {
