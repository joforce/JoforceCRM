...
Head.Class("Head_List_Js", {
	listInstance: false,
	filterClick: false,
	getInstance: function () {
		if (Head_List_Js.listInstance == false) {
			...
			if (parentModule == 'Settings') {
				...
				var fallbackClassName = parentModule + "_Head_List_Js";
				if (typeof window[fallbackClassName] == 'undefined') {
					fallbackClassName = "Head_List_Js";
				}
			} else {
				moduleClassName = module + "_List_Js";
				fallbackClassName = "Head_List_Js";
			...
			Head_List_Js.listInstance = instance;
			return instance;
		}
	...
	getRecordSelectTrackerInstance: function () {
		if (this.recordSelectTrackerInstance === false) {
			this.recordSelectTrackerInstance = Head_RecordSelectTracker_Js.getInstance();
...
//			http://stackoverflow.com/questions/26482645/number-isintegerx-which-is-created-can-not-work-in-ie
	...
	registerInlineEdit: function (currentTrElement) {
		...
		for (var i = 0; i < tdElements.length; i++) {
			...
			var fieldObject = new Head_Field_Js.getInstance(fieldData, moduleName);
...
/*			else
	...
	registerListViewBasicActions: function () {
		...
		app.event.on('post.listViewMassEdit.loaded', function (e, container) {
			...
			var editInstance = Head_Edit_Js.getInstance();
		...
		app.event.on('post.listViewMassAction.loaded', function (e, container) {
			...
			jQuery('#massSave').vtValidate({
				...
				submitHandler: function (form) {
					...
					if (Head_Index_Js.files) {
						formData.append("filename", Head_Index_Js.files);
						delete Head_Index_Js.files;
					}
					var params = {
						url: "index.php",
						type: "POST",
...
//			e.stopPropagation();
	...
	initializePaginationEvents: function () {
		var thisInstance = this;
		var paginationObj = this.getComponentInstance('Head_Pagination_Js');
		var listViewContainer = thisInstance.getListViewContainer();
