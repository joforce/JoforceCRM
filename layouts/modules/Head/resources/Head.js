...
Head.Class('Head_Index_Js', {
	...
	getInstance : function() {
        ...
        loadWidgets : function(widgetContainer, open) {
             ...
             var postParams  = app.convertUrlToDataParams(url);
                ...
                app.request.get({data:postParams, dataType:"html"}).then(
                        function(err,data){
	...
	registerActivityReminder : function() {
		...
		if(activityReminderInterval != '') {
			...
			if(currentTime >= nextActivityReminderCheck) {
				...
				setTimeout(function() {Head_Index_Js.requestReminder()}, nextInterval*1000);
			}
		}
	},
	...
	requestReminder : function() {
		...
		setTimeout(function() {Head_Index_Js.requestReminder()}, activityReminder*1000);
...
//		jQuery('.main-container').on('click', '.essentials-toggle', function() {
			...
			if(jQuery('.sidebar-essentials').hasClass('hide')) {
				...
				jQuery('#listViewContent').css('left', '42px');
			} else {
...
//                                                js.registerEvents();
	...
	registerMenuToggle : function(){
		var thisInstance = this;
		jQuery('.mega-Menus li a').click(function(event) {
	...
	registerAutoCompleteFields : function(container) {
		...
		container.find('input.autoComplete').autocomplete({
			...
			'select' : function(event, ui ){
				...
				sourceField.trigger(Head_Edit_Js.postReferenceSelectionEvent,{'data':selectedItemData});
			}
		});
	},
	...
	_showInventoryQuickPreviewForId: function (recordId, moduleName, templateId, isReference, mode) {
		...
		app.request.get({data: params}).then(function (err, response) {
			app.helper.hideProgress();
                        ...
                        jQuery('#quickviewcontent').css({'background': 'white', 'float': 'right','left': '68%','position': 'absolute','top': '50px', 'height':'100%', 'overflow-x':'hidden','overflow-y':'auto'});
