...
Head.Class("Head_Detail_Js", {
	...
	getInstanceByModuleName: function (moduleName) {
		...
		if (parentModule == 'Settings') {
			...
			var fallbackClassName = parentModule + "_Head_Detail_Js";
			if (typeof window[fallbackClassName] == 'undefined') {
				fallbackClassName = "Head_Detail_Js";
			}
		} else {
			moduleClassName = moduleName + "_Detail_Js";
			fallbackClassName = "Head_Detail_Js";
		...
		app.request.post({
			...
			if (data) {
				...
				app.request.get({
					...
					Head_Detail_Js.getInstance().registerEventForActivityWidget();
				});
			}
		});
	},
	...
	transferOwnershipSave: function (form) {
		form.on("click", "button[name='saveButton']", function (e) {
			e.preventDefault();
			var rules = {};
			rules["related_modules"] = {
				'required': true
			};
			rules["transferOwnerId"] = {
				'required': true
			};
			var params = {
				rules: rules,
	...
	previewFile: function (e, recordId, attachmentId) {
		Head_Index_Js.previewFile(e, recordId, attachmentId);
	}
	...
	widgetPostLoad: 'Head.Widget.PostLoad',
	_moduleName: false,
	...
	registerOverlayEditEvents: function (module, container) {
		var editInstance = Head_Edit_Js.getInstanceByModuleName(module);
		...
		app.event.on('post.overLayEditView.loaded', function (e, container) {
			jQuery('#EditView').vtValidate({
				submitHandler: function (form) {
					...
					var e = jQuery.Event(Head_Edit_Js.recordPresaveEvent);
					app.event.trigger(e);
	...
	addComponents: function () {
		var emailPreviewClassName = 'Head_EmailPreview_Js';
	...
	getRelatedListSearchParams: function () {
		var detailViewContainer = jQuery('.relatedContainer'); //this.getDetailViewContainer();
	...
	registerRollupCommentsSwitchEvent: function (rollupstatus) {
		var self = this;
				...
				function (err, data) {
					if (err === null) {
						if (typeof data !== 'object') {
							...
							app.helper.showAlertBox({
								'message': data.prototype.message
							});
						}
					} else {
						app.helper.showAlertBox({
	...
	registerRelatedListSearch: function () {
		var thisInstance = this;
		var detailViewContainer = jQuery('.details'); //thisInstance.getDetailViewContainer();
	...
	getRelatedController: function (relatedModuleName) {
		...
		var relatedListClass = 'Head_RelatedList_Js';
		if (typeof window[relatedListClass] != 'undefined') {
			return Head_RelatedList_Js.getInstance(recordId, moduleName, selectedTabElement, relatedModuleName);
		}
		return null;
	},
	...
	ajaxEditHandling: function (currentTdElement) {
		...
		if (jQuery('.editElement', editElement).length === 0) {
			...
			var fieldObject = Head_Field_Js.getInstance(fieldInfo);
			var fieldModel = fieldObject.getUiTypeModel();

			var ele = jQuery('<div class="input-group editElement" style="height:50px"></div>');
			var actionButtons = '<span class="input-group-addon inlineAjaxSave" id="saveicon" style="background:green; color: #fff;top: 4px;padding:2px;right:6px;width:25px; border-radius:4px!important;border:1px solid green!important"><i class="fa fa-check" style="margin-left:3px!important"></i></span>';
			actionButtons += '<span class="input-group-addon inlineAjaxCancel" id="cancel" style="color: #fff;background:red;top: 4px;width:25px!important;padding:2px;right:33px;margin-right:1px;z-index:99;border-radius:4px!important;border:1px solid red!important"><i class="fa fa-close" style="margin-left:4px!important"></i></span>';
		...
		contentHolder.on('click', '.inlineAjaxSave', function (e) {
			...
			if (previousValue == ajaxEditNewValue) {
				...
				var params = {
					...
					submitHandler: function (form) {
						var preAjaxSaveEvent = jQuery.Event(Head_Detail_Js.PreAjaxSaveEvent);
						...
						thisInstance.saveFieldValues(fieldNameValueMap).then(function (response) {
							...
							if (fieldBasicData.data('type') == 'picklist' && app.getModuleName() != 'Users') {
								...
								if (color) {
									...
									var picklistHtml = '<span class="picklist-color ' + postSaveRecordDetails[fieldName].display_value + '" style="background-color: ' + color + '; color: ' + textColor + ';">' +
										postSaveRecordDetails[fieldName].display_value +
										'</span>';
								} else {
									var picklistHtml = '<span class="picklist-color ' + postSaveRecordDetails[fieldName].display_value + '">' +
										postSaveRecordDetails[fieldName].display_value +
										'</span>';
								}
								...
								for (var i = 0; i < rawPicklistValues.length; i++) {
									...
									if (color) {
										...
										picklistHtml = picklistHtml +
											'<span class="picklist-color ' + postSaveRecordDetails[fieldName].display_value + '" style="background-color: ' + color + '; color: ' + textColor + ';">' +
											picklistValues[i] +
											'</span>';
									} else {
										picklistHtml = picklistHtml +
											'<span class="picklist-color ' + postSaveRecordDetails[fieldName].display_value + '">' +
											picklistValues[i] +
											'</span>';
									}
							...
							jQuery('.editAction').removeClass('hide');
							actionElement.show();
							var postAjaxSaveEvent = jQuery.Event(Head_Detail_Js.PostAjaxSaveEvent);
	...
	registerAjaxEditCancelEvent: function (contentHolder) {
		...
		contentHolder.on('click', '.inlineAjaxCancel', function (e) {
			...
			detailViewValue.css('display', 'inline-block');
			editElement.addClass('hide');
			editElement.find('.inputElement').trigger('Head.Validation.Hide.Messsage')
			editElement.removeClass('ajaxEdited');
			jQuery('.editAction').removeClass('hide');
			actionElement.show();
		});
	...
	registerBlockStatusCheckOnLoad: function () {
		...
		blocks.each(function (index, block) {
			...
			var bodyContents = currentBlock.find('.blockData');
	...
	saveComment: function (e) {
		...
		if (commentMode == "edit") {
			...
			postData['filename'] = Head_Index_Js.files,
				incrementCount = true;
		}
		...
		app.request.post(postData).then(
			function (err, data) {
				...
				if (incrementCount) {
					...
					var relatedController = new Head_RelatedList_Js(self.getRecordId(), app.getModuleName(), tabElement, self.getRelatedModuleName());
	...
	getChildComments: function (commentId) {
		var aDeferred = jQuery.Deferred();
		if ($('.recentCommentsBody').length) {
			var recentComment = 1;
		} else {
			var recentComment = 0;
		}
		var url = 'module=' + app.getModuleName() + '&view=Detail&record=' + this.getRecordId() + '&mode=showChildComments&commentid=' + commentId + '&recentComment=' + recentComment;
	...
	toggleRollupComments: function (e) {
		...
		var rollup_status = jQuery('#rollupcomments').bootstrapSwitch('state');
		if (rollup_status == true)
			rollup_status = 1;
		else
			rollup_status = 0;
		...
		if (viewtype == 'relatedlist') {
			url = 'index.php?module=Head&view=ModCommentsDetailAjax&parent=' + moduleName + '&parentId=' + recordId +
				'&rollupid=' + rollupId + '&rollup_status=' + rollup_status + '&mode=saveRollupSettings';
			params = {
				'type': 'GET',
				'url': url
			};
			app.request.get(params).then(function (err, data) {
				currentTarget.attr('rollup-status', !rollup_status);
			...
			app.request.get(params).then(function (err, data) {
				...
				// contents.html(data);
				vtUtils.enableTooltips();
				self.registerRollupCommentsSwitchEvent(rollup_status);
				//jQuery('#rollupcomments').bootstrapSwitch('state', !rollup_status, true);
			});
		} else {
			url = 'index.php?module=' + moduleName + '&relatedModule=ModComments&view=Detail&record=' +
				...
				'&rollup_status=' + rollup_status + '&parent=' + moduleName + '&rollup-toggle=1&limit=5';
	...
	postTagDeleteActions: function (deletedTagClone) {
		var summaryTagContainer = jQuery('.detailTagList');
		var tagInstance = this.getComponentInstance('Head_Tag_Js');
		var tagInfo = {
				...
				if (typeof recordUrl != "undefined") {
					...
					app.request.get({
						...
						app.helper.loadPageContentOverlay(response, overlayParams).then(function (container) {
							var detailjs = Head_Detail_Js.getInstanceByModuleName(params.module);
	...
	registerClickEvent: function () {
		this.getContentHolder().on('click', '.inventoryLineItemDetails', function (e) {
			jQuery('.inventoryLineItemDetails').popover({
				html: true
			}).show();
		});
	},
	...
	registerEvents: function () {
		...
		$("#show_all_comments").live('click', function (e) {
			$("li[data-module='ModComments'] a")[0].click();
		});
		...
		app.event.on("post.mail.sent", function (event, data) {
			...
			if (success.length > 0) {
				...
				if (relatedLoad == 1) {
					...
					window.app.controller().loadRelatedListRecords({
						page: pageNumber
					});
				} else {}
			}
	...
	registerPostAjaxSaveEvent: function () {
		var _this = this;
		app.event.on(Head_Detail_Js.PostAjaxSaveEvent, function (e, fieldBasicData, postSaveRecordDetails, contentHolder) {
			if (typeof contentHolder == 'undefined') {
				return;
			}
...
$(document).ready(function () {
	...
	const timeline_list = document.querySelectorAll('.timeline');

	timeline_list.forEach(timeline_lits_click => {
		size_li = $('#' + timeline_lits_click.id + " .container").size();
		if (size_li >= 4) {
			$('#' + timeline_lits_click.id + " #loadMoreActivity").show();
			$('#' + timeline_lits_click.id + " #showLessActivity").hide();
		} else if (size_li <= 3) {
			$('#' + timeline_lits_click.id + " #loadMoreActivity").hide();
			$('#' + timeline_lits_click.id + " #showLessActivity").hide();
		}
		x = 3;
		$('#' + timeline_lits_click.id + ' .container:lt(' + size_li + ')').hide();
		$('#' + timeline_lits_click.id + ' .container:lt(' + x + ')').show();
		timeline_lits_click.addEventListener("click", (e) => {
			if (e.target.id === "loadMoreActivity") {
				// console.log('#' + timeline_lits_click.id + '.container');

				size_li = $('#' + timeline_lits_click.id + " .container").size();
				// $('.timeline .container:lt(' + x + ')').show();
				if (x == 3) {
				...
				$('#' + timeline_lits_click.id + ' .container:lt(' + x + ')').show();
				$('#showLessActivity').show();
				if (x == size_li) {
					$('#loadMoreActivity').hide();
				}
			} else if (e.target.id === "showLessActivity") {
				size_li = $('#' + timeline_lits_click.id + " .container").size();
				if (x == 3) {
					$('#showLessActivity').hide();
				}
				...
				x = (x - 5 < 0) ? 3 : x - 5;
				$('#' + timeline_lits_click.id + ' .container').not(':lt(' + x + ')').hide();
	...
	// })
	size_li_updates = $('.updates_timeline .timeline-wrapper').size();
	xy = 3;
	$('.updates_timeline .timeline-wrapper:lt(' + size_li_updates + ')').hide();
	$('.updates_timeline .timeline-wrapper:lt(' + xy + ')').show();
	if (size_li_updates >= 4) {
		$('#loadMore').show();
		$('#showLess').hide();
	} else if (size_li_updates <= 3) {
		$('#loadMore').hide();
		$('#showLess').hide();
	}
	$('#loadMore').click(function () {

		xy = (xy + 5 <= size_li_updates) ? xy + 5 : size_li_updates;
		$('.updates_timeline .timeline-wrapper:lt(' + xy + ')').show();
		$('#showLess').show();
		if (xy == size_li_updates) {
			$('#loadMore').hide();
		}
	});
	$('#showLess').click(function () {
		xy = (xy - 5 < 0) ? 3 : xy - 5;
		$('.updates_timeline .timeline-wrapper').not(':lt(' + xy + ')').hide();
		$('#loadMore').show();
		$('#showLess').show();
		if (xy == 3) {
			$('#showLess').hide();
	...
	var height_activity = $(".recentActivitiesContainer").height();
	if (height_activity <= 400) {
		$('.recentActivitiesContainer').addClass('max-height-comments');
	} else if (height_activity >= 650) {
		$('.recentActivitiesContainer').removeClass('max-height-comments');
	}
});

function checkWidth_comment() {
	if ($(window).width() < 500) {
		$('.toggle-comments').removeClass('col-lg-12');

	} else {
		$('.toggle-comments').addClass('col-lg-12');
	}

}
$(window).resize(checkWidth_comment);
// window.onload=checkWidth_comment;
window.onload = function () {
	$(window).resize(checkWidth_comment);
	if ($(window).width() <= 500) {
		$('.toggle-comments').removeClass('col-lg-12');
		// console.log('.toggle-comments',$('.toggle-comments'));
