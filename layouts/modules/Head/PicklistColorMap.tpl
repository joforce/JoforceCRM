...
<style type="text/css">
    {foreach item=FIELD_MODEL key=FIELD_NAME from=$LISTVIEW_HEADERS}
        {if $FIELD_MODEL->getFieldDataType() eq 'picklist' or $FIELD_MODEL->getFieldDataType() eq 'multipicklist'}
            ...
            {foreach item=PICKLIST_COLOR key=PICKLIST_VALUE from=$PICKLIST_COLOR_MAP}
                ...
                {assign var=CONVERTED_PICKLIST_VALUE value=Head_Util_Helper::convertSpaceToHyphen($PICKLIST_VALUE)}
                    .picklist-{$FIELD_MODEL->getId()}-{Head_Util_Helper::escapeCssSpecialCharacters($CONVERTED_PICKLIST_VALUE)} {
    ...
    {if ($MODULE_NAME eq 'Calendar' && $MODULE_MODEL) OR ($RELATED_MODULE_NAME  eq 'Calendar' && $RELATED_MODULE) OR ($SOURCE_MODULE eq 'Calendar')}
		{assign var=STATUS_FIELD value=Head_Field_Model::getInstance('eventstatus', Head_Module_Model::getInstance('Events'))}
        ...
        {if $STATUS_FIELD}
            ...
            {foreach item=PICKLIST_COLOR key=PICKLIST_VALUE from=$PICKLIST_COLOR_MAP}
                ...
                {assign var=CONVERTED_PICKLIST_VALUE value=Head_Util_Helper::convertSpaceToHyphen($PICKLIST_VALUE)}
                    .picklist-{$STATUS_FIELD->getId()}-{Head_Util_Helper::escapeCssSpecialCharacters($CONVERTED_PICKLIST_VALUE)} {
