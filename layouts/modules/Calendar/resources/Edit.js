...
$(document).ready(function () {
	Head_Edit_Js("Calendar_Edit_Js", {
		...
		registerRelatedContactSpecificEvents: function (form) {
			...
			form.find('[name="contact_id"]').on(Head_Edit_Js.preReferencePopUpOpenEvent, function (e) {
				...
				if (popupReferenceModule == 'Leads' && parentIdElement.val().length > 0) {
					...
					app.helper.showErrorNotification({
						message: app.vtranslate('LBL_CANT_SELECT_CONTACT_FROM_LEADS')
					});
				}
			})
			...
			form.find('[name="contact_id"]').on(Head_Edit_Js.refrenceMultiSelectionEvent, function (e, result) {
		...
		registerRecordPreSaveEvent: function (form) {
			...
			app.event.one(Head_Edit_Js.recordPresaveEvent, function (e) {
		...
		referenceCreateHandler: function (container) {
			...
			var postQuickCreateSave = function (data) {
				...
				thisInstance.addNewContactToRelatedList({
					'data': [params]
				}, container);
			}
			...
			var quickCreateNode = jQuery('#quickCreateModules').find('[data-name="' + referenceModuleName + '"]');
			if (quickCreateNode.length <= 0) {
				return app.helper.showErrorNotification({
