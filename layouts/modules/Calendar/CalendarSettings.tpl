	...
	<div class="modal-dialog modal-lg calendarSettingsContainer">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form class="form-horizontal" id="CalendarSettings" name="CalendarSettings" method="post" action="{$SITEURL}index.php">
						...
						<div style="margin-left: 20px;">
							{foreach item="FIELD_MODEL" from=$RECORD_STRUCTURE['LBL_CALENDAR_SETTINGS']}
								...
								{if $FIELD_NAME neq 'callduration' && $FIELD_NAME neq 'othereventduration' && $FIELD_NAME neq 'defaulteventstatus' && $FIELD_NAME neq 'defaultactivitytype' && $FIELD_NAME neq 'hidecompletedevents'}
									...
									<div class="col-lg-5">
										<label class="fieldLabel col-lg-12">{vtranslate($FIELD_MODEL->get('label'),$TRANSLATION_MODULE)}</label>
									</div>
									<div class="col-lg-6">
										<div class="fieldValue col-lg-12">
											...
											{elseif $FIELD_NAME eq 'start_hour'}
												...
												<select class="select2" style="width: 100%;" name="{$FIELD_NAME}">
											...
											{else}
												<select class="select2" name="{$FIELD_NAME}" {if $FIELD_NAME eq 'time_zone'} style="width: 100%;" {else} style="width:100%;" {/if}>
													{if $FIELD_MODEL->isEmptyPicklistOptionAllowed()}<option value="">{vtranslate('LBL_SELECT_OPTION','Head')}</option>{/if}
													{foreach key=ID item=LABEL from=$FIELD_MODEL->getPicklistValues()}
														<option value="{$ID}" {if $FIELD_VALUE eq $ID} selected="" {/if}>{vtranslate($LABEL,$MODULE)}</option>
													{/foreach}
												</select>
							...
							{assign var=EVENTS_MODULE_MODEL value=Head_Module_Model::getInstance('Events')}
							...
							<div class="form-group">
								<div class="col-lg-5">
								<label class="fieldLabel col-lg-12">{vtranslate('LBL_DEFAULT_STATUS_TYPE',$MODULE)}</label>
								</div>
								<div class="col-lg-6">
								<div class="fieldValue col-lg-12">
									<span class="alignMiddle">{vtranslate('LBL_STATUS',$MODULE)}</span>&nbsp;&nbsp;
									<select class="select2" style="width: 100%;" name="{$DEFAULT_EVENT_STATUS_MODEL->get('name')}">
									...
									<select class="select2" style="width: 100%;" name="{$DEFAULT_ACTIVITY_TYPE_MODEL->get('name')}">
										<option value="{vtranslate('LBL_SELECT_OPTION','Head')}">{vtranslate('LBL_SELECT_OPTION','Head')}</option>
							...
							<div class="form-group">
							<div class="col-lg-5">
								<label class="fieldLabel col-lg-12">{vtranslate('LBL_DEFAULT_EVENT_DURATION',$MODULE)}</label>
							</div>
							<div class="col-lg-6">
								<div class="fieldValue col-lg-12">
							...
							<div class="form-group">
							<div class="col-lg-5">
								<label class="fieldLabel col-lg-12">{vtranslate($HIDE_COMPLETED_EVENTS_MODEL->get('label'),$MODULE)}</label>
							</div>
							<div class="col-lg-6">
								<div class="fieldValue col-lg-12">
							...
							<div class="form-group">
							<div class="col-lg-5">
								<label class="fieldLabel col-lg-12">{vtranslate('LBL_CALENDAR_SHARING',$MODULE)}</label>
							</div>
							<div class="col-lg-6">
