...
window.app = (function () {
	...
	function Request() {
		this._requestPjax = function (params) {
            ...
            if(params.tab_view == undefined)
                params.url = jQuery(location).attr('href');
	...
	return {
		...
		getModuleSpecificViewClass: function (view, module, parentModule) {
			...
			if (parentModule == 'Settings') {
				...
				if (typeof window[handlerClass] == 'undefined') {
					handlerClass = parentModule+'_Head_'+view+'_Js';
				}
			}
			if (typeof window[handlerClass] != 'undefined') {
				controllerClass = handlerClass;
			} else {
				handlerClass = 'Head_'+view+'_Js';
				if (typeof window[handlerClass] != 'undefined') {
					controllerClass = handlerClass;
				}
			}
		...
		vimage_path: function (img) {
			return jQuery('#joforce_site_url').val() + jQuery('body').data('skinpath')+'/images/'+img;
		},
		/**
