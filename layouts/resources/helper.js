...
jQuery.Class("Head_Helper_Js", {
    ...
    getInternalMailer: function (selectedId, fieldname, fieldmodule) {
        ...
        if (cacheResponse === '') {
            var helperInstance = new Head_Helper_Js;
    ...
    loadPageContentOverlay: function (data, params) {
        ...
        if (alreadyShown) {
            ...
            jQuery('#overlayPageContent').addClass('in');
        }
        return aDeferred.promise();
    },
    ...
    _initNotificationDefaults: function () {
        if (jQuery('.module-action-bar').length) {
            ...
            var defaultSettings = {
                ...
                'z_index': 10003,
                'template': '<div data-notify="container" class="col-xs-11 col-sm-3 col-12 vt-notification vt-notification-{0}" role="alert">' +
        ...
        jQuery('.modal .close,.modal .cancelLink').click(function (e) {
            if (initialFormData != form.serialize() && form.data('submit') != "true") {
                app.helper.showConfirmationBox({
                    'message': app.vtranslate("JS_CHANGES_WILL_BE_LOST") + ' ' + app.vtranslate('JS_WISH_TO_PROCEED')
                }).then(function () {
                    window.onbeforeunload = null;
