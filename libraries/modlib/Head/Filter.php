...
include_once('libraries/modlib/Head/Version.php');
...
class Head_Filter {
	...
	function __getUniqueId() {
		global $adb;
		return $adb->getUniqueID('jo_customview');
	}

	/**
	 * Initialize this filter instance
	 * @param Head_Module Instance of the module to which this filter is associated.
	 * @access private
	 */
	function initialize($valuemap, $moduleInstance=false) {
		$this->id = $valuemap[cvid];
		$this->name= $valuemap[viewname];
		$this->module=$moduleInstance? $moduleInstance: Head_Module::getInstance($valuemap[tabid]);
	}

	/**
	 * Create this instance
	 * @param Head_Module Instance of the module to which this filter should be associated with
	 * @access private
	 */
	function __create($moduleInstance) {
		...
		$adb->pquery("INSERT INTO jo_customview(cvid,viewname,setdefault,setmetrics,entitytype) VALUES(?,?,?,?,?)", 
		...
		if(!$this->status) {
			...
			$adb->pquery("UPDATE jo_customview SET status=? WHERE cvid=?", Array($this->status, $this->id));
	...
	function __delete() {
		global $adb;
		$adb->pquery("DELETE FROM jo_cvadvfilter WHERE cvid=?", Array($this->id));
		$adb->pquery("DELETE FROM jo_cvcolumnlist WHERE cvid=?", Array($this->id));
		$adb->pquery("DELETE FROM jo_customview WHERE cvid=?", Array($this->id));
	}

	/**
	 * Save this instance
	 * @param Head_Module Instance of the module to use
	 */
	...
	function addField($fieldInstance, $index=0) {
		...
		$adb->pquery("UPDATE jo_cvcolumnlist SET columnindex=columnindex+1 WHERE cvid=? AND columnindex>=? ORDER BY columnindex DESC", 
			Array($this->id, $index));
		$adb->pquery("INSERT INTO jo_cvcolumnlist(cvid,columnindex,columnname) VALUES(?,?,?)", Array($this->id, $index, $cvcolvalue));
	...
	function addRule($fieldInstance, $comparator, $comparevalue, $index=0, $group=1, $condition='and') {
		...
		$adb->pquery("UPDATE jo_cvadvfilter set columnindex=columnindex+1 WHERE cvid=? AND columnindex>=? ORDER BY columnindex DESC",
			Array($this->id, $index));		
		$adb->pquery("INSERT INTO jo_cvadvfilter(cvid, columnindex, columnname, comparator, value, groupid, column_condition) VALUES(?,?,?,?,?,?,?)",
			Array($this->id, $index, $cvcolvalue, $comparator, $comparevalue, $group, $condition));

		Head_Utils::Log("Adding Condition " . self::translateComparator($comparator,true) ." on $fieldInstance->name of $this->name filter ... DONE");
	 ...
	 * @access private
	 * @internal Used from Head_PackageExport also
	 */
	 ...
	 * @param Head_Module Instance of the module to use when filtername is used
	 */
	static function getInstance($value, $moduleInstance=false) {
		...
		if(Head_Utils::isNumber($value)) {
			$query = "SELECT * FROM jo_customview WHERE cvid=?";
			$queryParams = Array($value);
		} else {
			$query = "SELECT * FROM jo_customview WHERE viewname=? AND entitytype=?";
	...
	static function getAllForModule($moduleInstance) {
		...
		$query = "SELECT * FROM jo_customview WHERE entitytype=?";
	...
	static function deleteForModule($moduleInstance) {
		...
		$cvidres = $adb->pquery("SELECT cvid FROM jo_customview WHERE entitytype=?", Array($moduleInstance->name));
		if($adb->num_rows($cvidres)) {
			...
			if(!empty($cvids)) {
				$adb->pquery("DELETE FROM jo_cvadvfilter WHERE cvid  IN (" . implode(',', $cvids) . ")", array());
				$adb->pquery("DELETE FROM jo_cvcolumnlist WHERE cvid IN (" . implode(',', $cvids) . ")", array());
				$adb->pquery("DELETE FROM jo_customview WHERE cvid   IN (" . implode(',', $cvids) . ")", array());
			}
		}
	}
}
