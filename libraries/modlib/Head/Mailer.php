 ...
 * @package modlib
 */
class Head_Mailer extends PHPMailer {
	...
	function __getUniqueId() {
		global $adb;
		return $adb->getUniqueID('jo_mailer_queue');
	}
	...
	function initialize() {
		...
		global $adb;
		$result = $adb->pquery("SELECT * FROM jo_systems WHERE server_type=?", Array('email'));
...
//			$this->Sender= getReturnPath($this->Host);
	...
	function initFromTemplate($emailtemplate) {
		global $adb;
		$result = $adb->pquery("SELECT * from jo_emailtemplates WHERE templatename=? AND foldername=?",
	...
	*/
	function addSignature($userId) {
		global $adb;
		$sign = nl2br($adb->query_result($adb->pquery("select signature from jo_users where id=?", array($userId)),0,"signature"));
		$this->Signature = $sign;
	}
	...
	function __initializeQueue() {
		if(!$this->_queueinitialized) {
			if(!Head_Utils::CheckTable('jo_mailer_queue')) {
				Head_Utils::CreateTable('jo_mailer_queue',
			...
			if(!Head_Utils::CheckTable('jo_mailer_queueinfo')) {
				Head_Utils::CreateTable('jo_mailer_queueinfo',
					'(id INTEGER, name VARCHAR(100), email VARCHAR(100), type VARCHAR(7))',
					true);
			}
			if(!Head_Utils::CheckTable('jo_mailer_queueattachments')) {
				Head_Utils::CreateTable('jo_mailer_queueattachments',
	...
	function __AddToQueue($linktoid) {
		if($this->__initializeQueue()) {
			...
			$adb->pquery('INSERT INTO jo_mailer_queue(id,fromname,fromemail,content_type,subject,body,mailer,relcrmid) VALUES(?,?,?,?,?,?,?,?)',
			...
			foreach($this->to as $toinfo) {
				...
				$adb->pquery('INSERT INTO jo_mailer_queueinfo(id, name, email, type) VALUES(?,?,?,?)',
			...
			foreach($this->attachment as $attachmentinfo) {
				...
				$adb->pquery('INSERT INTO jo_mailer_queueattachments(id, path, name, encoding, type) VALUES(?,?,?,?,?)',
	 ...
	 */
	static function dispatchQueue(Head_Mailer_Listener $listener=null) {
		global $adb;
		if(!Head_Utils::CheckTable('jo_mailer_queue')) return;

		$mailer = new self();
		$queue = $adb->pquery('SELECT * FROM jo_mailer_queue', array());
		if($adb->num_rows($queue)) {
			for($index = 0; $index < $adb->num_rows($queue); ++$index) {
				...
				$emails = $adb->pquery('SELECT * FROM jo_mailer_queueinfo WHERE id=?', Array($queueid));
				...
				$attachments = $adb->pquery('SELECT * FROM jo_mailer_queueattachments WHERE id=?', Array($queueid));
				...
				if($sent) {
					Head_Event::trigger('vtiger.mailer.mailsent', $relcrmid);
					if($listener) {
						$listener->mailsent($queueid);
					}
					$adb->pquery('DELETE FROM jo_mailer_queue WHERE id=?', Array($queueid));
					$adb->pquery('DELETE FROM jo_mailer_queueinfo WHERE id=?', Array($queueid));
					$adb->pquery('DELETE FROM jo_mailer_queueattachments WHERE id=?', Array($queueid));
				} else {
					if($listener) {
						$listener->mailerror($queueid);
					}
					$adb->pquery('UPDATE jo_mailer_queue SET failed=?, failreason=? WHERE id=?', Array(1, $mailer->ErrorInfo, $queueid));
				}
			}
		}
	}
 ...
 * @package modlib
 */
abstract class Head_Mailer_Listener {
