 ...
 ************************************************************************************/
include_once('libraries/modlib/Head/PackageImport.php');

/**
 * Provides API to update module into vtiger CRM
 * @package modlib
 */
class Head_PackageUpdate extends Head_PackageImport {
        ...
        if($module != null) {
            ...
            $unzip->unzipAllEx( ".",
                    ...
                    Array(// Templates folder to be renamed while copying
						'templates' => "layouts/modules/$module",
						...
						'settings/templates' => "layouts/modules/Settings/$module",
     ...
     * @param Head_Module Instance of the module to update
                ...
                foreach ($installSequenceArray as $sequence) {
                    foreach ($buildModuleArray as $moduleInfo) {
                        if($moduleInfo['install_sequence'] == $sequence) {
                            $moduleInstance = Head_Module::getInstance($moduleInfo['name']);
    ...
    function update_Module($moduleInstance) {
        ...
        Head_Module::fireEvent($moduleInstance->name,
                Head_Module::EVENT_MODULE_PREUPDATE);
        ...
        Head_Module::fireEvent($moduleInstance->name,
                Head_Module::EVENT_MODULE_POSTUPDATE);
    }
    ...
    function update_Blocks($modulenode, $moduleInstance) {
        ...
        foreach($modulenode->blocks->block as $blocknode) {
            ...
            $blockInstance = Head_Block::getInstance((string)$blocknode->label, $moduleInstance);
            if(!$blockInstance) {
		...
		$listFieldBeforeUpdate = Head_Field::getAllForModule($moduleInstance);
		...
		$listBlockBeforeUpdate = Head_Block::getAllForModule($moduleInstance);
    ...
    function update_Fields($blocknode, $blockInstance, $moduleInstance) {
        ...
        foreach($blocknode->fields->field as $fieldnode) {
            ...
            $fieldInstance = Head_Field::getInstance((string)$fieldnode->fieldname, $moduleInstance);
            if(!$fieldInstance) {
    ...
    function update_CustomViews($modulenode, $moduleInstance) {
        ...
        foreach($modulenode->customviews->customview as $customviewnode) {
            $filterInstance = Head_Filter::getInstance($customviewnode->viewname, $moduleInstance);
            if(!$filterInstance) {
    ...
    function update_Events($modulenode, $moduleInstance) {
        ...
        $adb->query("DELETE FROM jo_eventhandlers WHERE handler_class  IN (SELECT handler_class FROM jo_eventhandler_module  WHERE module_name = '".$moduleInstance->name."')");
        $adb->query("DELETE FROM jo_eventhandler_module  WHERE module_name = '".$moduleInstance->name."'");
    ...
    function update_Event($modulenode, $moduleInstance, $eventnode) {
        Head_Event::register($moduleInstance, $eventnode->eventname, $eventnode->classname, $eventnode->filename);
    ...
    function update_Relatedlist($modulenode, $moduleInstance, $relatedlistnode) {
        $relModuleInstance = Head_Module::getInstance((string)$relatedlistnode->relatedmodule);
	...
	function update_CronTasks($modulenode) {
		...
		$cronTasks = Head_Cron::listAllInstancesByModule($modulenode->name);
		foreach ($modulenode->crons->cron as $importCronTask) {
			foreach($cronTasks as $cronTask) {
				if($cronTask->getName() == $importCronTask->name && $importCronTask->handler == $cronTask->getHandlerFile()) {
					Head_Cron::deregister($importCronTask->name);
				}
			}
			if(empty($importCronTask->status)){
                            $cronTask->status = Head_Cron::$STATUS_DISABLED;
                        } else {
                            $cronTask->status = Head_Cron::$STATUS_ENABLED;
                        } 
			if((empty($importCronTask->sequence))){
				$importCronTask->sequence=Head_Cron::nextSequence();
			}
			Head_Cron::register("$importCronTask->name","$importCronTask->handler", "$importCronTask->frequency", "$modulenode->name","$importCronTask->status","$importCronTask->sequence","$importCronTask->description");
		}
	}
}
?>
