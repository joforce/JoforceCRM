...
class Head_Block {
	...
	function __getUniqueId() {
		...
		$maxblockid = $adb->getUniqueID('jo_blocks');
		return $maxblockid;
	}
	...
	function __getNextSequence() {
		global $adb;
		$result = $adb->pquery("SELECT MAX(sequence) as max_sequence from jo_blocks where tabid = ?", Array($this->module->id));
		$maxseq = 0;
	 ...
	 * @param Head_Module Instance of module to which this block is associated
	 * @access private
	 */
	...
	function __create($moduleInstance) {
		...
		$adb->pquery("INSERT INTO jo_blocks(blockid,tabid,blocklabel,sequence,show_title,visible,create_view,edit_view,detail_view,iscustom)
	...
	function __delete() {
		...
		$adb->pquery("DELETE FROM jo_blocks WHERE blockid=?", Array($this->id));
		self::log("DONE");
	}

	/**
	 * Save this block instance
	 * @param Head_Module Instance of the module to which this block is associated
	 */
	...
	function delete($recursive=true) {
		if($recursive) {
			$fields = Head_Field::getAllForBlock($this);
			if(!empty($fields))
			{
	 ...
	 * @param Head_Field Instance of field to add to this block.
	 ...
	 * @param Head_Module Instance of the module if block label is passed
	 */
	static function getInstance($value, $moduleInstance=false) {
		...
		if($moduleInstance && $cache->getBlockInstance($value, $moduleInstance->id)){
			...
			if(Head_Utils::isNumber($value)) {
				$query = "SELECT * FROM jo_blocks WHERE blockid=?";
				$queryParams = Array($value);
			} else {
				$query = "SELECT * FROM jo_blocks WHERE blocklabel=? AND tabid=?";
	...
	static function getAllForModule($moduleInstance) {
		...
		$query = "SELECT * FROM jo_blocks WHERE tabid=? ORDER BY sequence";
	 ...
	 * @param Head_Module Instnace of module to use
	...
	static function deleteForModule($moduleInstance, $recursive=true) {
		global $adb;
		if($recursive) Head_Field::deleteForModule($moduleInstance);
		$adb->pquery("DELETE FROM jo_blocks WHERE tabid=?", Array($moduleInstance->id));
