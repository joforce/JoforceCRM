...
class Head_PackageExport {
	var $_export_tmpdir = 'cache/modlib';
	...
	function __initExport($module, $moduleInstance) {
		if($moduleInstance->isentitytype) {
			...
			Head_Utils::checkFileAccessForInclusion("modules/$module/$module.php");
		}
	...
	function export($moduleInstance, $todir='', $zipfilename='', $directDownload=false) {
		...
		if(is_dir("layouts/modules/$module"))
			$zip->copyDirectoryFromDisk ("layouts/modules/$module", "templates");

		//Copy Settings module templates files, if any
		if(is_dir("layouts/modules/Settings/$module"))
			$zip->copyDirectoryFromDisk ("layouts/modules/Settings/$module", "settings/templates");

		//Copy language files
		$this->__copyLanguageFiles($zip, $module);
		
		//Copy image file
		if(file_exists("layouts/skins/images/$module.png"))
		{
			$zip->copyFileFromDisk("layouts/skins/images", "", "$module.png");
		}
	...
	function export_Dependencies($moduleInstance) {
		...
		$sqlresult = $adb->pquery("SELECT * FROM jo_tab_info WHERE tabid = ?", array($moduleid));
		...
		for($i=0; $i<$noOfPreferences; ++$i) {
			...
			if($prefName == 'jo_min_version') {
				$vtigerMinVersion = $prefValue;
			}
			if($prefName == 'jo_max_version') {
	...
	function export_Module($moduleInstance) {
		...
		$sqlresult = $adb->pquery("SELECT * FROM jo_parenttabrel WHERE tabid = ?", array($moduleid));
		$parenttabid = $adb->query_result($sqlresult, 0, 'parenttabid');
		$menu = Head_Menu::getInstance($parenttabid);
		$parent_name = $menu->label;

		$sqlresult = $adb->pquery("SELECT * FROM jo_tab WHERE tabid = ?", array($moduleid));
	...
	function export_Tables($moduleInstance) {
		...
		if($moduleInstance->isentitytype) {
			...
			// Setup required module variables which is need for modlib API's
			modlib_setup_modulevars($modulename, $focus);
			...
			foreach($tables as $table) {
				...
				$this->outputNode('<![CDATA['.Head_Utils::CreateTableSql($table).']]>', 'sql');
	...
	function export_Blocks($moduleInstance) {
		global $adb;
		$sqlresult = $adb->pquery("SELECT * FROM jo_blocks WHERE tabid = ?", Array($moduleInstance->id));
	...
	function export_Fields($moduleInstance, $blockid) {
		...
		$fieldresult = $adb->pquery("SELECT * FROM jo_field WHERE tabid=? AND block=?", Array($moduleInstance->id, $blockid));
		$fieldcount = $adb->num_rows($fieldresult);

		if(empty($fieldcount)) return;

		$entityresult = $adb->pquery("SELECT * FROM jo_entityname WHERE tabid=?", Array($moduleInstance->id));
		...
		for($index = 0; $index < $fieldcount; ++$index) {
			...
			if($uitype == '15' || $uitype == '16' || $uitype == '111' || $uitype == '33' || $uitype == '55') {
				...
				if($uitype == '16') {
					$picklistvalues = modlib_getPicklistValues($fieldname);
				} else {
					$picklistvalues = modlib_getPicklistValues_AccessibleToAll($fieldname);
				}
			...
			if($uitype == '10') {
				$relatedmodres = $adb->pquery("SELECT * FROM jo_fieldmodulerel WHERE fieldid=?", Array($fieldid));
	...
	function export_CustomViews($moduleInstance) {
		...
		$customviewres = $adb->pquery("SELECT * FROM jo_customview WHERE entitytype = ?", Array($moduleInstance->name));
		...
		for($cvindex = 0; $cvindex < $customviewcount; ++$cvindex) {
			...
			$cvcolumnres = $adb->pquery("SELECT * FROM jo_cvcolumnlist WHERE cvid=?", array($cvid));
			...
			for($index = 0; $index < $cvcolumncount; ++$index) {
				...
				$cvcolumnruleres = $adb->pquery("SELECT * FROM jo_cvadvfilter WHERE cvid=? AND columnname=?",
				...
				if($cvcolumnrulecount) {
					...
					for($rindex = 0; $rindex < $cvcolumnrulecount; ++$rindex) {
						...
						$cvcolumnrulecomp  = Head_Filter::translateComparator($cvcolumnrulecomp, true);
	...
	function export_SharingAccess($moduleInstance) {
		...
		$deforgshare = $adb->pquery("SELECT * FROM jo_def_org_share WHERE tabid=?", Array($moduleInstance->id));
	...
	function export_Events($moduleInstance) {
		$events = Head_Event::getAll($moduleInstance);
		if(!$events) return;
	...
	function export_Actions($moduleInstance) {
		...
		global $adb;
		$result = $adb->pquery('SELECT distinct(actionname) FROM jo_profile2utility, jo_actionmapping
			WHERE jo_profile2utility.activityid=jo_actionmapping.actionid and tabid=?', Array($moduleInstance->id));
	...
	function export_RelatedLists($moduleInstance) {
		...
		global $adb;
		$result = $adb->pquery("SELECT * FROM jo_relatedlists WHERE tabid = ?", Array($moduleInstance->id));
		if($adb->num_rows($result)) {
			...
			for($index = 0; $index < $adb->num_rows($result); ++$index) {
				...
				$relModuleInstance = Head_Module::getInstance($row['related_tabid']);
	...
	function export_CronTasks($moduleInstance){
        $cronTasks = Head_Cron::listAllInstancesByModule($moduleInstance->name);
        $this->openNode('crons');
