 ...
 ************************************************************************************/
include_once('libraries/modlib/Head/PackageExport.php');
include_once('libraries/modlib/Head/Unzip.php');
 ...
 * @package modlib
 */
class Head_PackageImport extends Head_PackageExport {
	...
	function initImport($zipfile, $overwrite=true) {
                ...
                if($module != null) {
					...
					"layouts/modules/Settings/$module" => "layouts/modules/Settings/$module",
                                        ...
                                        //module images
                                        'images' => "layouts/skins/images/$module",
                                        'settings' => "modules/Settings",
                                )
                        );

                        if($unzip->checkFileExistsInRootFolder("$module.png")) {
                                $unzip->unzip("$module.png", "layouts/skins/images/$module.png");
                        }
	 ...
	 * @access private
	 */
	function getDependentHeadVersion() {
		return $this->_modulexml->dependencies->jo_version;
	}
	 ...
	 * @access private
	 */
	function getDependentMaxHeadVersion() {
		return $this->_modulexml->dependencies->jo_max_version;
	}
	...
	function import_Module() {
		...
		$vtigerMinVersion = $this->_modulexml->dependencies->jo_version;
		$vtigerMaxVersion = $this->_modulexml->dependencies->jo_max_version;

		$moduleInstance = new Head_Module();
		...
		Head_Module::fireEvent($moduleInstance->name,
			Head_Module::EVENT_MODULE_POSTINSTALL);
	}
	...
	function import_Tables($modulenode) {
		...
		foreach($modulenode->tables->table as $tablenode) {
			...
			if(Head_Utils::IsCreateSql($tablesql)) {
				if(!Head_Utils::checkTable($tablename)) {
					self::log("SQL: $tablesql ... ", false);
					Head_Utils::ExecuteQuery($tablesql);
					self::log("DONE");
				}
			} else {
	...
	function import_CustomView($modulenode, $moduleInstance, $customviewnode) {
		...
		$filterInstance = new Head_Filter();
	...
	function import_Event($modulenode, $moduleInstance, $eventnode) {
		...
		Head_Event::register($moduleInstance,
	...
	function import_Relatedlist($modulenode, $moduleInstance, $relatedlistnode) {
		$relModuleInstance = Head_Module::getInstance($relatedlistnode->relatedmodule);
	...
	function import_CronTasks($modulenode){
		...
		foreach ($modulenode->crons->cron as $cronTask){
			...
			if((empty($cronTask->sequence))){
				$cronTask->sequence=Head_Cron::nextSequence();
			}
			Head_Cron::register("$cronTask->name","$cronTask->handler", "$cronTask->frequency", "$modulenode->name","$cronTask->status","$cronTask->sequence","$cronTask->description");
		}
	}
}
?>
