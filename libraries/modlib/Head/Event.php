...
class Head_Event
{
	/** Event name like: jo.entity.aftersave, jo.entity.beforesave */
	var $eventname;
	...
	static function hasSupport()
	{
		if (self::$is_supported === '') {
			self::$is_supported = Head_Utils::checkTable('jo_eventhandlers');
		}
	 ...
	 * @param String Name of the Event like jo.entity.aftersave, jo.entity.beforesave
	...
	static function register($moduleInstance, $eventname, $classname, $filename, $condition = '', $dependent = '[]')
	{
		...
		if (Head_Utils::checkFileAccess($filename, false)) {
			global $adb;
			$eventsManager = new EventsManager($adb);
	...
	static function trigger($eventname, $crmid)
	{
		...
		global $adb;
		$checkres = $adb->pquery("SELECT setype, crmid, deleted FROM jo_crmentity WHERE crmid=?", array($crmid));
		if ($adb->num_rows($checkres)) {
			...
			if ($result['deleted'] == '0') {
				...
				$em->triggerEvent($eventname, EntityData::fromCRMEntity($moduleInstance));
			}
		}
	}
	...
	static function getAll($moduleInstance)
	{
		...
		if (self::hasSupport()) {
			...
			$records = $adb->pquery("SELECT * FROM jo_eventhandlers WHERE handler_class IN 
				(SELECT handler_class FROM jo_eventhandler_module WHERE module_name=?)", array($moduleInstance->name));
			if ($records) {
				while ($record = $adb->fetch_array($records)) {
					$event = new Head_Event();
