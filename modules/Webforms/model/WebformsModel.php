...
class Webforms_Model {
	...
	function setData($data) {
		...
		if (isset($data["fields"])) {
			$this->setFields(modlib_purify($data["fields"]), modlib_purify($data["required"]), modlib_purify($data["value"]));
		}
		if (isset($data['id'])) {
	...
	function setFields(array $fieldNames, $required, $value) {
		...
		foreach ($fieldNames as $ind => $fieldname) {
			...
			if (($field['type']['name'] == 'date')) {
				...
				$defaultvalue = modlib_purify($value[$fieldname]);
			}
	...
	function getId() {
		return modlib_purify($this->data["id"]);
	}

	function getName() {
		return html_entity_decode(modlib_purify($this->data["name"]));
	}

	function getTargetModule() {
		return modlib_purify($this->data["targetmodule"]);
	}

	function getPublicId() {
		return modlib_purify($this->data["publicid"]);
	}

	function getEnabled() {
		return modlib_purify($this->data["enabled"]);
	}
    
	function getDescription() {
		return modlib_purify($this->data["description"]);
	}

	function getReturnUrl() {
		return modlib_purify($this->data["returnurl"]);
	}

	function getOwnerId() {
		return modlib_purify($this->data["ownerid"]);
	}
    
    function getRoundrobin() {
        return modlib_purify($this->data["roundrobin"]);
    }
    
    function getRoundrobinOwnerId() {
        global $adb;
        $roundrobin_userid = modlib_purify($this->data["roundrobin_userid"]);
        $roundrobin_logic = modlib_purify($this->data["roundrobin_logic"]);
        ...
        $adb->pquery("UPDATE jo_webforms SET roundrobin_logic = ? WHERE id = ?", array($nextRoundrobinLogic,$this->getId()));
        return modlib_purify($roundrobinOwnerId);
    }
	...
	function retrieveFields() {
		global $adb;
		$fieldsResult = $adb->pquery("SELECT * FROM jo_webforms_field WHERE webformid=?", array($this->getId()));
	...
	function save() {
		...
		if ($isNew) {
			...
			$insertSQL = "INSERT INTO jo_webforms(name, targetmodule, publicid, enabled, description,ownerid,returnurl) VALUES(?,?,?,?,?,?,?)";
			...
			// Udpate
			$updateSQL = "UPDATE jo_webforms SET description=? ,returnurl=?,ownerid=?,enabled=? WHERE id=?";
		...
		$adb->pquery("DELETE FROM jo_webforms_field WHERE webformid=?", array($this->getId()));
		$fieldInsertSQL = "INSERT INTO jo_webforms_field(webformid, fieldname, neutralizedfield, defaultvalue,required) VALUES(?,?,?,?,?)";
	...
	function delete() {
		...
		$adb->pquery("DELETE from jo_webforms_field where webformid=?", array($this->getId()));
		$adb->pquery("DELETE from jo_webforms where id=?", array($this->getId()));
		return true;
	}
	...
	static function retrieveWithPublicId($publicid) {
		...
		$result = $adb->pquery("SELECT * FROM jo_webforms WHERE publicid=? AND enabled=?", array($publicid, 1));
	...
	static function retrieveWithId($data) {
		...
		$model = false;
		// Retrieve model and populate information
		$result = $adb->pquery("SELECT * FROM jo_webforms WHERE id=?", array($id));
		if ($adb->num_rows($result)) {
			$model = new Webforms_Model($adb->fetch_array($result));
			$model->retrieveFields();
		}
	...
	static function isWebformField($webformid, $fieldname) {
		...
		$checkSQL = "SELECT 1 from jo_webforms_field where webformid=? AND fieldname=?";
	...
	static function isRequired($webformid, $fieldname) {
		global $adb;
		$sql = "SELECT required FROM jo_webforms_field where webformid=? AND fieldname=?";
	...
	static function retrieveDefaultValue($webformid, $fieldname) {
		...
		$sql = "SELECT defaultvalue FROM jo_webforms_field WHERE webformid=? and fieldname=?";
		$result = $adb->pquery($sql, array($webformid, $fieldname));
		$defaultvalue = false;
		if ($adb->num_rows($result)) {
	...
	static function existWebformWithName($name) {
		global $adb;
		$checkSQL = "SELECT 1 FROM jo_webforms WHERE name=?";
	...
	static function isActive($field, $mod) {
		global $adb;
		$tabid = getTabid($mod);
		$query = 'SELECT 1 FROM jo_field WHERE fieldname = ?  AND tabid = ? AND presence IN (0,2)';
