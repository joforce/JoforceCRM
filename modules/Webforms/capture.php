...
include_once 'includes/Zend/Json.php';
...
include_once 'includes/QueryGenerator/QueryGenerator.php';
...
class Webform_Capture {
	...
	function captureNow($request) {
		...
		$returnURL = false;
		try {
			if (!modlib_isModuleActive('Webforms'))
				throw new Exception('webforms is not active');

			$webform = Webforms_Model::retrieveWithPublicId(modlib_purify($request['publicid']));
			if (empty($webform))
			...
			foreach ($webformFields as $webformField) {
				if ($webformField->getDefaultValue() != null) {
					...
					if (is_array(modlib_purify($request[$webformNeutralizedField]))) {
						$fieldData = implode(" |##| ", modlib_purify($request[$webformNeutralizedField]));
					} else {
						$fieldData = modlib_purify($request[$webformNeutralizedField]);
			...
			$moduleModel = Head_Module_Model::getInstance($webform->getTargetModule());
	...
	protected function sendResponse($url, $success = false, $failure = false) {
		if (empty($url)) {
			...
			if (!empty($_REQUEST['callback'])) {
				$callback = modlib_purify($_REQUEST['callback']);
...
$request = modlib_purify($_REQUEST);
$isURLEncodeEnabled = $request['urlencodeenable'];
