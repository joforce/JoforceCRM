...
class Users_SaveAjax_Action extends Head_SaveAjax_Action {
	...
	public function process(Head_Request $request) {
		...
		foreach ($fieldModelList as $fieldName => $fieldModel) {
			$fieldValue = $displayValue = Head_Util_Helper::toSafeHTML($recordModel->get($fieldName));
			if ($fieldModel->getFieldDataType() !== 'currency') {
				$displayValue = $fieldModel->getDisplayValue($fieldValue, $recordModel->getId());
			}
			if($fieldName == 'language') {
				$displayValue =  Head_Language_Handler::getLanguageLabel($fieldValue);
			}
	...
	public function getRecordModelFromRequest(Head_Request $request) {
		...
		if($fieldName == "is_owner") {
			...
			if(!empty($recordId)) {
				$existingRecordModel =  Head_Record_Model::getInstanceById($recordId, $moduleName);
	...
	public function userExists(Head_Request $request){
	...
	public function savePassword(Head_Request $request) {
		$module = $request->getModule();
		...
		/*
		 * To restore a user
		 * @param Head_Request Object
		 */
		public function restoreUser(Head_Request $request) {
			$moduleName = $request->getModule();
			...
			$recordModel = Users_Record_Model::getInstanceById($record, $moduleName);
				...
				$db->pquery("UPDATE jo_users SET deleted=? WHERE id=?", array(0,$record));
	 ...
	 */
	public function transferOwner(Head_Request $request) {
	 ...
	 */
	public function changeUsername(Head_Request $request) {
		$response = new Head_Response();
		$userId = $request->get('userid');
	...
	public function changeAccessKey(Head_Request $request) {
		$recordId = $request->get('record');
		$moduleName = $request->getModule();
		...
		try {
			...
			Head_AccessControl::clearUserPrivileges($recordId);
