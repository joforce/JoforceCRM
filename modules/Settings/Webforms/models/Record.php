...
class Settings_Webforms_Record_Model extends Settings_Head_Record_Model {
	...
	public function getSelectedFieldsList($mode='') {
		if (!$this->selectedFields) {
			...
			if ($targetModule) {
				...
				$result = $db->pquery("SELECT * FROM jo_webforms_field WHERE webformid = ? ORDER BY sequence", array($this->getId()));
	...
	public function save() {
		...
		if ($mode) {
			$updateQuery = "UPDATE jo_webforms SET description = ?, returnurl = ?, ownerid = ?, enabled = ?, captcha = ? , roundrobin = ?, roundrobin_userid = ?, roundrobin_logic = ? ,targetmodule = ? WHERE id = ?";
			...
			$insertQuery = "INSERT INTO jo_webforms(name, targetmodule, publicid, enabled, description, ownerid, returnurl, captcha, roundrobin, roundrobin_userid, roundrobin_logic) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		...
		$db->pquery("DELETE FROM jo_webforms_field WHERE webformid = ?", array($this->getId()));
		...
		$fieldInsertQuery = "INSERT INTO jo_webforms_field(webformid, fieldname, neutralizedfield, defaultvalue, required, sequence, hidden) VALUES(?, ?, ?, ?, ?, ?, ?)";
		foreach ($selectedFieldsData as $fieldName => $fieldDetails) {
			...
			$fieldModel = Head_Field_Model::getInstance($fieldName, $sourceModuleModel);
			...
			if ($dataType === 'date') {
				$fieldDefaultValue = Head_Date_UIType::getDBInsertedValue($fieldDefaultValue);
			}

			if ($dataType === 'time') {
				$fieldDefaultValue = Head_Time_UIType::getTimeValueWithSeconds($fieldDefaultValue);
			}

			if ($dataType === 'reference') {
				$referenceModule = $fieldDetails['referenceModule'];
				$referenceObject = HeadWebserviceObject::fromName($db,$referenceModule);
	...
	public function checkDuplicate() {
		...
		$query = "SELECT 1 FROM jo_webforms WHERE name = ?";
	...
	static public function getInstanceById($recordId, $qualifiedModuleName) {
		...
		$result = $db->pquery("SELECT * FROM jo_webforms WHERE id = ?", array($recordId));
		if ($db->num_rows($result)) {
