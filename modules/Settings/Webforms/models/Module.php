...
class Settings_Webforms_Module_Model extends Settings_Head_Module_Model {

	var $baseTable = 'jo_webforms';
	var $baseIndex = 'id';
	...
	public function deleteRecord($recordModel) {
		...
		$db->pquery("DELETE from jo_webforms_field WHERE webformid = ?", array($recordId));
		$db->pquery("DELETE from jo_webforms WHERE id = ?", array($recordId));
		return true;
	}
