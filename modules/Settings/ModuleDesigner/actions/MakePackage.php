...
class Settings_ModuleDesigner_MakePackage_Action extends Settings_Head_Index_Action
{
	...
	protected function getDirectoryName(Head_Request $request)
	{
		$dirname = false;
	...
	protected function getModule(Head_Request $request)
	{
		$o_module = false;
	...
	{		
		ob_start();
		include "modules/$moduleName/templates/module.xml.php";
	...
	protected function makeModuleDirectory($dirname, $moduleName, $module_dir, $o_module)
	{
		...
		{			
			$this->copyr("modules/$moduleName/templates/Module 6.x", $module_dir);
		...
		{	
			...
			if(empty($o_module->modifiedModulePath) && !is_dir($o_module->modifiedModulePath.'settings/templates') && is_dir("layouts/modules/Settings/{$o_module->modifiedModule}"))
			{
				$this->copyr("layouts/modules/Settings/{$o_module->modifiedModule}", DIR_TEMP.$dirname."/settings/templates");
			}
			...
			elseif(empty($o_module->modifiedModulePath) && file_exists('layouts/skins/images/'.$o_module->modifiedModule.'.png'))
			{
				$this->copyr('layouts/skins/images/'.$o_module->modifiedModule.'.png', 	DIR_TEMP.$dirname."/".$o_module->name.".png");
			}
		...
		foreach($o_module->a_fields as $o_field)
		{
		    if(!empty($o_field->relatedModule) && is_array($o_field->relatedModule))
		    {
		    	foreach($o_field->relatedModule as $key => $relatedModule)
				{
					...
					{	
						$relatedListStr = "\r\n".
											"\t\t\$moduleInstance = Head_Module::getInstance('$relatedModule');\r\n".
											"\t\t\$relatedModuleInstance = Head_Module::getInstance('$o_module->name');\r\n".
	...
	protected function createZipPackage($dirname, $o_module)
	{
		...
		{
			$zip->copyDirectoryFromDisk(DIR_TEMP."{$dirname}/templates", "layouts/modules/".$module);
		}
