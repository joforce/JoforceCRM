...
class Settings_AddressLookup_SaveFields_Action extends Head_Action_Controller {
		...
		if($modulename == ""){
			header("location:index.php?module=AddressLookup&parent=Settings&view=List&sourceModule=$modulename&check=1");
			die;
		}
		...
		$moduleQuery = $adb->pquery("INSERT INTO jo_vtaddressmapping(modulename) SELECT * FROM (SELECT ?)AS tmp WHERE NOT EXISTS (SELECT modulename FROM jo_vtaddressmapping WHERE modulename = ?)",array($modulename,$modulename));
		...
		if($saveFieldQuery)	{
			...
			header("location:$site_URL/AddressLookup/Settings/List/$modulename/success");
			else
			header("location:$site_URL/AddressLookup/Settings/List");
		}
		else
			header("location:$site_URL/AddressLookup/Settings/List/$modulename/error");
