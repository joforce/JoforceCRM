...
class Settings_DuplicateCheck_Module_Model extends Head_Module_Model {
	...
	public function getFields() {
		if(empty($this->fields)){
            ...
            if(count($blockId) > 0) {
                $fieldList = Settings_DuplicateCheck_Field_Model::getInstanceFromBlockIdList($blockId,$moduleModel);

            }
         
            //To handle special case for invite users
            if($this->getName() == 'Events') {
                $blockModel = new Settings_DuplicateCheck_Block_Model();
                $blockModel->set('id','EVENT_INVITE_USER_BLOCK_ID');
                $blockModel->set('label','LBL_INVITE_USER_BLOCK');
                $blockModel->set('module', $this);

                $fieldModel = new Settings_DuplicateCheck_Field_Model();
	...
	public function getBlocks() {
		if(empty($this->blocks)) {
			$blocksList = array();
			$moduleBlocks = Settings_DuplicateCheck_Block_Model::getAllForModule($this);
    ...
    public static function getInstanceByFieldToMatch($sourceModule){
      $sourceModule = Settings_DuplicateCheck_Field_Model::getInstanceByFieldToMatch($sourceModule);
      return $sourceModule;
    }
