...
class Settings_Workflows_Edit_View extends Settings_Head_Index_View
{
	...
	function v7Edit(Head_Request $request)
	{
		...
		if ($recordId) {
			...
			$viewer->assign('MODE', 'edit');
			$viewer->assign('TASK_LIST', $workflowModel->getTasks());
			$task_moduleModel = $workflowModel->getModule();
			$viewer->assign('TASK_TYPES', Settings_Workflows_TaskType_Model::getAllForModule($task_moduleModel));
		} else {
			$workflowModel = Settings_Workflows_Record_Model::getCleanInstance($moduleName);
			$selectedModule = $request->get('source_module');
			if (!empty($selectedModule)) {
		...
		$andconditioncount = 1;
		if ($workflowModel->isFilterSavedInNew()) {
			$advance_criteria = $workflowModel->transformToAdvancedFilterCondition();
			if (count($advance_criteria[2]['columns']) == 0) {
				$andconditioncount = 1;
			} else {
				$andconditioncount = count($advance_criteria[2]['columns']);
			}
		}
		$viewer->assign('ADVANCE_CRITERIA_COUNT', $andconditioncount);
