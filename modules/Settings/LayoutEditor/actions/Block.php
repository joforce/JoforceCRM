...
class Settings_LayoutEditor_Block_Action extends Settings_Head_Index_Action {
    ...
    public function save(Head_Request $request) {
        ...
        $modueInstance = Head_Module_Model::getInstance($sourceModule);
        ...
        if(!empty($blockId)) {
            ...
            if(!empty($beforeBlockId)) {
                $beforeBlockInstance = Head_Block_Model::getInstance($beforeBlockId);
				...
				Head_Block_Model::pushDown($beforeBlockSequence, $modueInstance->getId());
            }
			$isDuplicate = Head_Block_Model::checkDuplicate(trim($request->get('label')), $modueInstance->getId());
        }
		...
		if (!$isDuplicate) {
			try{
				...
				if(empty($blockId)) {
					...
					$responseInfo['sequenceList'] = Head_Block_Model::getAllBlockSequenceList($modueInstance->getId());
				}
    ...
    public function updateSequenceNumber(Head_Request $request) {
        $response = new Head_Response();
        try{
            $sequenceList = $request->get('sequence');
            Head_Block_Model::updateSequenceNumber($sequenceList,$request->get('selectedModule'));
    ...
    public function delete(Head_Request $request) {
        $response = new Head_Response();
        $blockId = $request->get('blockid');
        $checkIfFieldsExists = Head_Block_Model::checkFieldsExists($blockId);
        if($checkIfFieldsExists) {
            $response->setError('502','Fields exists for the block');
            $response->emit();
            return;
        }
        $blockInstance = Head_Block_Model::getInstance($blockId);
