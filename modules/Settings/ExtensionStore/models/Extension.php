...
class Settings_ExtensionStore_Extension_Model extends Head_Base_Model {
	...
	public function __construct() {
		parent::__construct();
		$this->EXTENSIONSTORE_LOOKUP_URL = 'https://www.joforce.com/products.html';
		global $site_URL;
		if (empty($site_URL)) {
	...
	public function isHeadCompatible() {
		...
		if ((Head_Version::check($vtigerVersion, '>=') && $vtigerMaxVersion && Head_Version::check($vtigerMaxVersion, '<')) || Head_Version::check($vtigerVersion, '=')) {
			return true;
		}
		return false;
	}
	...
	public static function getLanguageInstance($lang) {
		$sql = 'SELECT id,name,prefix FROM jo_language WHERE name = ? OR prefix = ?';
		...
		if ($connector) {
			...
			if ($listings['success']) {
				...
				foreach ($listings as $key => $listing) {
					$extensionModelsList[$key] = $this->getInstanceFromArray($listing);
				}
			} else {
	...
	public function findListings($searchTerm = null, $searchType) {
		...
		if ($connector) {
			...
			if ($listings['success']) {
				...
				foreach ($listings as $key => $listing) {
					$name = $listing['Name'];
					if(preg_match("/{$searchTerm}/i", $name)) {
						$extensionModelsList[$key] = $this->getInstanceFromArray($listing);
					}
				}
			} else {
	 ...
	 */
	public function logoutMarketPlace(Head_Request $request) {
		$sql = 'DELETE FROM jo_extnstore_users';
		$db = PearDatabase::getInstance();
		$db->pquery($sql, array());
	}
	...
	public static function getModuleFromExtnName($extnName) {
		$moduleModel = Head_Module_Model::getInstance($extnName);
		if ($moduleModel) {
			$moduleModel->set('extnType', 'module');
		}
		if (!$moduleModel) {
			if (self::getLanguageInstance($extnName)) {
				$moduleModel = new Head_Module_Model();
