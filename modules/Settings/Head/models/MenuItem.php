...
/*
 * Head Settings MenuItem Model Class
 */
class Settings_Head_MenuItem_Model extends Head_Base_Model {

	protected static $itemsTable = 'jo_settings_field';
	...
	public function setMenu($menuId) {
		$this->menu = Settings_Head_Menu_Model::getInstanceById($menuId);
		return $this;
	}

	/**
	 * Function to set the Menu to which the Item belongs, given Menu Model instance
	 * @param <Settings_Head_Menu_Model> $menu - Settings Menu Model instance
	 * @return Settings_Head_MenuItem_Model
	 */
	...
	public function getUrl() {
		global $site_URL;
		$url = $site_URL.decode_html($this->get('linkto'));
		$menu = $this->getMenu();
		$url .= '/'.$this->getMenu()->getId().'/'.$this->getId();
		return $url;
	}
	...
	public function getPinUnpinActionUrl() {
		return 'index.php?module=Head&parent=Settings&action=Basic&mode=updateFieldPinnedStatus&fieldid='.$this->getId();
	}
	 ...
	 * @return Settings_Head_MenuItem_Model instance
	 */
	 ...
	 * @param <String> $name
	 * @param <Settings_Head_Menu_Model> $menuModel
	 * @return Settings_Head_MenuItem_Model instance
	 */
	public static function getInstance($name, $menuModel=false) {
		...
		if($db->num_rows($result) > 0) {
			...
			$menuItem = Settings_Head_MenuItem_Model::getInstanceFromArray($rowData);
			if($menuModel) {
