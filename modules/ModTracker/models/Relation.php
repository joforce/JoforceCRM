...
class ModTracker_Relation_Model extends Head_Record_Model {
	...
	function getLinkedRecord() {
        ...
        $query = 'SELECT * FROM jo_crmentity WHERE crmid = ?';
		...
		if($noOfRows) {
			if(!array_key_exists($targetModule, $moduleModels)) {
				$moduleModel = Head_Module_Model::getInstance($targetModule);
			}
			$row = $db->query_result_rowdata($result, 0);
			$modelClassName = Head_Loader::getComponentClassName('Model', 'Record', $targetModule);
	...
	public function getRecordDetailViewUrl() {
		try {
			$recordModel = Head_Record_Model::getInstanceById($this->get('targetid'), $this->get('targetmodule'));
