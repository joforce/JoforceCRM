...
class VTPDFMaker_Module_Model extends Head_Module_Model {
	...
	public function saveRecord($recordModel) {
		...
		if(empty($templateid)){
			$templateid = $db->getUniqueID('jo_vtpdfmaker');
			$sql = "INSERT INTO jo_vtpdfmaker(name, module, description, body, header, footer, status, settings, vtpdfmakerid) VALUES (?,?,?,?,?,?,?,?,?)";
		}else{
			$sql = "UPDATE jo_vtpdfmaker SET name=?, module=?, description=?, body=?, header=?, footer=?, status=?, settings=? WHERE vtpdfmakerid = ?";
		}
	...
	public function deleteRecord($recordModel) {
		...
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM jo_vtpdfmaker WHERE vtpdfmakerid = ? ', array($recordId));
	}
	...
	public function deleteAllRecords() {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM jo_vtpdfmaker', array());
	}
	...
	public function getRecordIds($skipRecords){
		...
		$query = 'SELECT vtpdfmakerid FROM jo_vtpdfmaker WHERE vtpdfmakerid NOT IN ('.generateQuestionMarks($skipRecords).')';
