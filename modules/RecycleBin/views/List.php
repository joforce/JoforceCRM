...
class RecycleBin_List_View extends Head_Index_View {
	...
	public function initializeListViewContents(Head_Request $request, Head_Viewer $viewer) {
		...
		$orderParams = Head_ListView_Model::getSortParamsSession($moduleName . '_' . $sourceModule);
		if ($request->get('mode') == 'removeSorting') {
			Head_ListView_Model::deleteParamsSession($moduleName . '_' . $sourceModule, array('orderby', 'sortorder'));
			$orderBy = '';
			$sortOrder = '';
		}
		...
		if (empty($orderBy) && empty($searchValue) && empty($pageNumber) && empty($searchParams)) {
			...
			Head_ListView_Model::setSortParamsSession($moduleName . '_' . $sourceModule, $params);
		}
		...
		$transformedSearchParams = Head_Util_Helper::transferListSearchParamsToFilterCondition($searchParams, $listViewModel->getModule());
	...
	function getPageCount(Head_Request $request){
		...
		$listViewModel = RecycleBin_ListView_Model::getInstance($moduleName, $sourceModule);
		$searchParams = $request->get('search_params');
		$listViewModel->set('search_params',Head_Util_Helper::transferListSearchParamsToFilterCondition($searchParams, $listViewModel->getModule()));
