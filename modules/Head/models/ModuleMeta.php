...
class Head_ModuleMeta_Model extends Head_Base_Model {
	...
	/**
	 * creates an instance of Head_ModuleMeta_Model
	 * @param <String> $name - module name
	 * @param <Object> $user - Users Object
	 * @return Head_ModuleMeta_Model
	 */
	public static function getInstance($name, $user) {
		$moduleMetaClassName = Head_Loader::getComponentClassName('Model', 'ModuleMeta', $name);
		...
		if(method_exists($focus, 'getImportableFields')) {
			...
			foreach($moduleFields as $fieldName => $fieldInstance) {
				if(($this->isEditableField($fieldInstance)
							&& ($fieldInstance->getTableName() != 'jo_crmentity' || $fieldInstance->getColumnName() != 'modifiedby')
