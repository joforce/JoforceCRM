 ...
 */
class Head_ListView_Model extends Head_Base_Model {
	...
	public function getListViewMassActions($linkParams) {
		...
		if($moduleModel->isCommentEnabled() && $modCommentsModel->isPermitted('CreateView')) {
			$massActionLinks[] = array(
				...
				'linkurl' => $site_URL.'index.php?module='.$moduleModel->get('name').'&view=MassActionAjax&mode=showAddCommentForm',
				'linkicon' => ''
			);
		}
	...
	public function getListViewEntries($pagingModel) {
		...
		if(!empty($orderBy) && $orderByFieldModel) {
			if($orderBy == 'roleid' && $moduleName == 'Users'){
				$listQuery .= ' ORDER BY jo_role.rolename '.' '. $sortOrder; 
			} else {
