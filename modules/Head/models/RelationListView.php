...
class Head_RelationListView_Model extends Head_Base_Model {
	...
	public function getSelectRelationLinks() {
		...
		foreach($selectLinkList as $selectLink) {
			$selectLinkModel[] = Head_Link_Model::getInstanceFromValues($selectLink);
		}
		return $selectLinkModel;
	}
	...
	public function getEntries($pagingModel) {
		...
		if ($this->get('whereCondition') && is_array($this->get('whereCondition'))) {
			...
			foreach ($whereCondition as $fieldName => $fieldValue) {
				if (is_array($fieldValue)) {
					...
					$type = $fieldValue[3];
					if ($type == 'time') {
						$searchValue = Head_Time_UIType::getTimeValueWithSeconds($searchValue);
					}
		...
		if($orderBy) {
			...
			if($orderByFieldModuleModel && $orderByFieldModuleModel->isReferenceField()) {
				...
				$qualifiedOrderBy = 'jo_crmentity'.$orderByFieldModuleModel->get('column');
				$selectAndFromClause .= ' LEFT JOIN jo_crmentity AS '.$qualifiedOrderBy.' ON '.
				...
				$query .= ' ORDER BY '.$qualifiedOrderBy.'.label '.$sortOrder;
				 ...
				 $query .= ' ORDER BY COALESCE(CONCAT(jo_users.first_name,jo_users.last_name),jo_groups.groupname) '.$sortOrder;
			} else{
				...
				if($qualifiedOrderBy == 'jo_activity.date_start' && ($relationModuleName == 'Calendar' || $relationModuleName == 'Emails')) {
					$qualifiedOrderBy = "str_to_date(concat(jo_activity.date_start,jo_activity.time_start),'%Y-%m-%d %H:%i:%s')";
				}
			...
			$query .= ' ORDER BY jo_crmentity.modifiedtime DESC';
		}
	...
	public function getRelationQuery() {
		...
		$query = $queryComponents[0].' ,jo_crmentity.crmid FROM '.$queryComponents[1];
	...
	public static function getInstance($parentRecordModel, $relationModuleName, $label=false) {
		...
		$className = Head_Loader::getComponentClassName('Model', 'RelationListView', $parentModuleName);
		$instance = new $className();

		$parentModuleModel = $parentRecordModel->getModule();
		$relatedModuleModel = Head_Module_Model::getInstance($relationModuleName);
		$instance->setRelatedModuleModel($relatedModuleModel);

		$relationModel = Head_Relation_Model::getInstance($parentModuleModel, $relatedModuleModel, $label);
		...
		if(!$relationModel){
			...
			foreach ($referenceFieldOfParentModule as $fieldName=>$fieldModel) {
				...
				if(in_array($relatedModuleName, $refredModulesOfReferenceField)){
					$relationModelClassName = Head_Loader::getComponentClassName('Model', 'Relation', $parentModuleModel->getName());
	...
	public function getRelatedEntriesCount() {
		...
		if ($position) {
			...
			if($relatedModuleName == 'Calendar') {
				$relationQuery = 'SELECT DISTINCT jo_crmentity.crmid, jo_activity.activitytype ';
			} else {
				$relationQuery = 'SELECT COUNT(DISTINCT jo_crmentity.crmid) AS count';
			}
	...
	public function getCurrencySymbol($recordId, $fieldModel) {
		...
		if(($fieldName == 'unit_price') && ($moduleName == 'Products' || $moduleName == 'Services')) {
			$query = "SELECT currency_symbol FROM jo_currency_info WHERE id = (";
			if($moduleName == 'Products') 
				$query .= "SELECT currency_id FROM jo_products WHERE productid = ?)";
			else if($moduleName == 'Services')
				$query .= "SELECT currency_id FROM jo_service WHERE serviceid = ?)";

			$result = $db->pquery($query, array($recordId));
			return $db->query_result($result, 0, 'currency_symbol');
		} else if(($tableName == 'jo_invoice' || $tableName == 'jo_quotes' || $tableName == 'jo_purchaseorder' || $tableName == 'jo_salesorder') &&
			...
			$query = "SELECT currency_symbol FROM jo_currency_info WHERE id = ( SELECT currency_id FROM ".$tableName." WHERE ".$focus->table_index." = ? )";
			$result = $db->pquery($query, array($recordId));
			return $db->query_result($result, 0, 'currency_symbol');
		} else {
