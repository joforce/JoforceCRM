...
/**
 * Head Entity Record Model Class
 */
class Head_Record_Model extends Head_Base_Model {
	...
	public function getName() {
		...
		return Head_Util_Helper::toSafeHTML(decode_html($displayName));
	}
	 ...
	 * @return Head_Record_Model instance
	 */
...
//        return $site_URL . $this->getModuleName().'/view/' . $module->getEditViewName() . '/' . $this->getId();
	...
	public function getDisplayValue($fieldName,$recordId = false) {
		...
		if($fieldName == "time_start" && $this->getModule()->getName() == "Emails"){
			...
			$value = Head_Time_UIType::getDisplayValue($dateTime->getDisplayTime());
	...
	/**
	 * Function returns the Head_Field_Model
	...
	/**
	 * Static Function to get the instance of a clean Head Record Model for the given module name
	 * @param <String> $moduleName
	 * @return Head_Record_Model or Module Specific Record Model instance
	 */
	...
	/**
	 * Static Function to get the instance of the Head Record Model given the recordid and the module name
	...
	public function getImageDetails() {
		...
		if ($recordId) {
			$sql = "SELECT jo_attachments.*, jo_crmentity.setype FROM jo_attachments
						...
						WHERE jo_crmentity.setype = ? and jo_seattachmentsrel.crmid = ?";
	...
	public function deleteImage($imageId) {
		...
		$checkResult = $db->pquery('SELECT crmid FROM jo_seattachmentsrel WHERE attachmentsid = ?', array($imageId));
		$crmId = intval($db->query_result($checkResult, 0, 'crmid'));
		if (intval($this->getId()) === $crmId) {
			$db->pquery('DELETE FROM jo_seattachmentsrel WHERE crmid = ? AND attachmentsid = ?', array($crmId,$imageId));
	...
	public function getDescriptionValue() {
		...
		if(empty($description)) {
			$db = PearDatabase::getInstance();
			$result = $db->pquery("SELECT description FROM jo_crmentity WHERE crmid = ?", array($this->getId()));
	...
	public static function getInstancesFromIds($recordIds, $moduleName) {
		...
		if($moduleName == 'Users') {
			$query .= ' AND jo_users.id IN (' . generateQuestionMarks($recordIds) . ')';
		} else{
			$query .= ' AND jo_crmentity.crmid IN (' . generateQuestionMarks($recordIds) . ')';
		}
	...
	public function getFileDetails($attachmentId = false) {
		$db = PearDatabase::getInstance();
		$fileDetails = array();
		$query = "SELECT * FROM jo_attachments
				INNER JOIN jo_seattachmentsrel ON jo_seattachmentsrel.attachmentsid = jo_attachments.attachmentsid
				WHERE crmid = ? ";
		$params = array($this->get('id'));
		if($attachmentId) {
			$query .= 'AND jo_attachments.attachmentsid = ?';
			$params[] = $attachmentId;
		}
		$result = $db->pquery($query, $params);
	...
	public function getTitle($fieldInstance) {
		...
		if ($fieldInstance) {
			...
			if (in_array($dataType, array('reference', 'multireference'))) {
				$recordName = Head_Util_Helper::getRecordName($rawValue);
				if ($recordName) {
					return $recordName;
				} else {
					return '';
	...
	function getRollupCommentsForModule($startIndex = 0, $pageLimit = 10) {
		...
		if ($relatedModuleRecordIds) {
			...
			$query .= " AND jo_modcomments.related_to IN (" . generateQuestionMarks($relatedModuleRecordIds)
					. ") AND jo_modcomments.parent_comments=0 ORDER BY jo_crmentity.createdtime DESC LIMIT "
	...
	function getCommentEnabledRelatedEntityIds($modulename, $recordId) {
		...
		foreach ($referenceFieldsModels as $referenceFieldsModel) {
			...
			if (!empty($relModuleFieldValue) && !in_array($relmoduleFieldname, $restrictedFieldnames) && isRecordExists($relModuleFieldValue)) {
				$relModuleRecordModel = Head_Record_Model::getInstanceById($relModuleFieldValue);
				$relmodule = $relModuleRecordModel->getModuleName();

				$relatedmoduleModel = Head_Module_Model::getInstance($relmodule);
		...
		$relatedModuleModels = Head_Relation_Model::getAllRelations($moduleModel, false);
		...
		$indirectrelatedModuleRecordIds = $moduleModel->getRelatedModuleRecordIds(new Head_Request($commentEnabledModules), array($recordId), true);
