...
class Head_MiniList_Model extends Head_Widget_Model {
	...
	public function getTargetModuleModel() {
		if (!$this->targetModuleModel) {
			$this->targetModuleModel = Head_Module_Model::getInstance($this->getTargetModule());
		}
	...
	public function getTitle($prefix='') {
		...
		$suffix = '';
		$customviewrs = $db->pquery('SELECT viewname FROM jo_customview WHERE cvid=?', array($this->widgetModel->get('filterid')));
	...
	public function getHeaders() {
		...
		if (!$this->listviewHeaders) {
			...
			foreach ($this->listviewController->getListViewHeaderFields() as $fieldName => $webserviceField) {
				$fieldObj = Head_Field::getInstance($webserviceField->getFieldId());
				$headerFieldModels[$fieldName] = Head_Field_Model::getInstanceFromFieldObject($fieldObj);
			}
	...
	public function getRecords() {
		...
		if (!$this->listviewRecords) {
			...
			$query = str_replace(" FROM ", ",jo_crmentity.crmid as id FROM ", $query);
            if($this->getTargetModule() == 'Calendar') {
                $query = str_replace(" WHERE ", " WHERE jo_crmentity.setype = 'Calendar' AND ", $query);
            }
