 ...
 */
class Head_Field_Model extends Head_Field {
	...
	public function getModule() {
		if(!$this->module) {
			...
			$this->module = Head_Module_Model::getInstanceFromModuleObject($moduleObj);
		}
		return $this->module;
	}
	...
	public function isNameField() {
		$nameFieldObject = Head_Cache::get('EntityField',$this->getModuleName());
		if(!$nameFieldObject){
	 ...
	 * @return Head_Base_UIType or UI Type specific model instance
	 */
	public function getUITypeModel() {
		return Head_Base_UIType::getInstanceFromField($this);
	}
	...
	public function getPicklistValues() {
		...
		if($fieldDataType == 'picklist' || $fieldDataType == 'multipicklist') {
			...
			if($this->isRoleBased()) {
				...
				$picklistValues = Head_Util_Helper::getRoleBasedPicklistValues($fieldName, $userModel->get('roleid'));
			}else{
				$picklistValues = Head_Util_Helper::getPickListValues($fieldName);
			}
	...
	public static function getInstanceFromFieldObject(Head_Field $fieldObj) {
		...
		$className = Head_Loader::getComponentClassName('Model', 'Field', $fieldObj->getModuleName());
	...
	public function getReportFilterColumnName() {
		...
		if($tableName == 'jo_crmentity' && $columnName !='smownerid'){
			$tableName = 'jo_crmentity'.$moduleName;
	 ...
	 * @param <Head_Module_Model> $blockModel - block instance
	 * @return <array> List of field model
	 */
	public static function getAllForModule($moduleModel){
		$fieldModelList = Head_Cache::get('ModuleFields',$moduleModel->id);
		if(!$fieldModelList){
			...
			foreach($fieldObjects as $fieldObject){
				...
				Head_Cache::set('field-'.$moduleModel->getId(),$fieldModelObject->getId(),$fieldModelObject);
				Head_Cache::set('field-'.$moduleModel->getId(),$fieldModelObject->getName(),$fieldModelObject);
			}

			Head_Cache::set('ModuleFields',$moduleModel->id,$fieldModelList);
		}
	...
	public static function  getInstance($value, $module = false) {
		$fieldObject = null;
		if($module){
			$fieldObject = Head_Cache::get('field-'.$module->getId(), $value);
		}
		if(!$fieldObject){
			$fieldObject = parent::getInstance($value, $module);
			if($module){
				Head_Cache::set('field-'.$module->getId(),$value,$fieldObject);
			}
		}
	...
	function getDocumentFolders() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM jo_attachmentsfolder', array());
	...
	*/
	...
	public function getCurrencyList() {
		...
		$result = $db->pquery('SELECT id, currency_name FROM jo_currency_info WHERE currency_status = ? AND deleted=0', array('Active'));
	...
	public function getPermissions($accessmode = 'readonly') {
		...
		if ($privileges->hasGlobalReadPermission()) {
			...
			$modulePermission = Head_Cache::get('modulePermission-'.$accessmode, $this->getModuleId());
			if (!$modulePermission) {
		...
		$profilelist = $privileges->get('profiles');
				...
				if (count($profilelist) > 0) {
					if ($accessmode == 'readonly') {
						$query = "SELECT jo_profile2field.visible,jo_field.fieldid FROM jo_field INNER JOIN jo_profile2field ON jo_profile2field.fieldid=jo_field.fieldid INNER JOIN jo_def_org_field ON jo_def_org_field.fieldid=jo_field.fieldid WHERE jo_field.tabid=? AND jo_profile2field.visible=0 AND jo_def_org_field.visible=0  AND jo_profile2field.profileid in (" . generateQuestionMarks($profilelist) . ") AND jo_field.presence in (0,2) GROUP BY jo_field.fieldid";
					} else {
				...
				Head_Cache::set('modulePermission-'.$accessmode,$tabid,$modulePermission);
	...
	public function __update() {
		$db = PearDatabase::getInstance();
		$query = 'UPDATE jo_field SET typeofdata=?,presence=?,quickcreate=?,masseditable=?,defaultvalue=?,summaryfield=?,headerfield=?';
	...
	public function getOneToOneRelationField(){
		...
		return Head_Field_Model::getInstance($this->get('related_field_id'));
	}
	 ...
	 * @return Head_Relation_Model / false;
	 */
	public function getRelationShipForThisField() {
		return Head_Relation_Model::getInstanceFromRelationFied($this->getId());
	}
