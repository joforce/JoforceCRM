...
class Head_DashBoard_Model extends Head_Base_Model {
	 ...
	 * @return <Array of Head_Widget_Model>
	 */
	public function getSelectableDashboard() {
		...
		$sql = 'SELECT * FROM jo_links WHERE linktype = ?
					AND tabid IN ('. generateQuestionMarks($moduleTabIdList) .') AND linkid NOT IN (SELECT linkid FROM jo_module_dashboard_widgets
		...
		$sql .= ' UNION SELECT * FROM jo_links WHERE linklabel in (?,?)';
		$params[] = 'Mini List';
		$params[] = 'Notebook';
		...
		for($i=0; $i<$db->num_rows($result); $i++) {
			...
			if($this->checkModulePermission($row)) {
				$widgets[] = Head_Widget_Model::getInstanceFromValues($row);
			}
		}
	...
	public function getDashboards($moduleDashboard) {
		...
		$sql = " SELECT jo_links.*, jo_module_dashboard_widgets.userid, jo_module_dashboard_widgets.filterid, jo_module_dashboard_widgets.data, jo_module_dashboard_widgets.id as widgetid, jo_module_dashboard_widgets.position as position, jo_links.linkid as id FROM jo_links ".
				" INNER JOIN jo_module_dashboard_widgets ON jo_links.linkid=jo_module_dashboard_widgets.linkid".
				" WHERE jo_module_dashboard_widgets.userid = ? AND linktype = ? AND tabid = ?";
		...
		$sql = "SELECT reportid FROM jo_module_dashboard_widgets WHERE userid = ? AND linkid= ? AND reportid IS NOT NULL";
		...
		for($i=0, $len=$db->num_rows($result); $i<$len; $i++) {
			...
			if($moduleDashboard == 'Home' || $moduleDashboard == $chartReportModel->getPrimaryModule()){
				...
				if($tabId && $currentUserPrivilegeModel->hasModulePermission($tabId)) {
					$widgets[] = Head_Widget_Model::getInstanceWithReportId($row['reportid'], $currentUser->getId());
				}
			}
		}
		return $widgets;
	...
	public function getActiveTabs() {
		...
		$db = PearDatabase::getInstance();
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$query = "SELECT id,tabname,sequence,isdefault,appname,modulename FROM jo_dashboard_tabs WHERE userid=? ORDER BY sequence ASC ";
	 ...
	 * Purpose : If user added a widget in Head6 then we need add that widget for first tab
	 * @param type $userId
	 * @return type
	 */
	public function getUserDefaultTab($userId){
		$db = PearDatabase::getInstance();
		$query = "SELECT id,tabname,sequence,isdefault FROM jo_dashboard_tabs WHERE userid=? AND isdefault =?";
	...
	public function addTab($tabName){
		...
		$result = $db->pquery("SELECT MAX(sequence)+1 AS sequence FROM jo_dashboard_tabs",array());
		$sequence = $db->query_result($result, 0,'sequence');

		$query = "INSERT INTO jo_dashboard_tabs(tabname, userid,sequence) VALUES(?,?,?)";
	...
	public function deleteTab($tabId) {
		$db = PearDatabase::getInstance();
		$query = "DELETE FROM jo_dashboard_tabs WHERE id=?";
	...
	public function renameTab($tabId, $tabName) {
		 $db = PearDatabase::getInstance();
		$query = "UPDATE jo_dashboard_tabs SET tabname=? WHERE id=?";
	...
	public function checkTabExist($tabName) {
		$db = PearDatabase::getInstance();
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$query = "SELECT * FROM jo_dashboard_tabs WHERE tabname=? and userid=?";
	...
	public function checkTabsLimitExceeded() {
		$db = PearDatabase::getInstance();
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$query = "SELECT count(*) AS count FROM jo_dashboard_tabs WHERE userid=?";
		$result = $db->pquery($query, array($currentUser->getId()));
	...
	public function updateTabSequence($sequence){
		...
		$query = "UPDATE jo_dashboard_tabs SET sequence = ? WHERE id=?";
	...
	public function getTabInfo($tabId) {
		...
		$query = "SELECT * FROM jo_dashboard_tabs WHERE id=? ";
		$params = array($tabId);
	 ...
	 * @return <Head_DashBoard_Model>
	 */
	public static function getInstance($moduleName) {
		$modelClassName = Head_Loader::getComponentClassName('Model', 'DashBoard', $moduleName);
	...
	public function checkModulePermission($resultData) {
		...
		return ($currentUserPrivilegeModel->hasModulePermission(getTabid($module)) && !Head_Runtime::isRestricted('modules', $module));
	}
