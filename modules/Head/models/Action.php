...
/**
 * Head Action Model Class
 */
class Head_Action_Model extends Head_Base_Model {
	...
	public function isModuleEnabled($module) {
		...
		$tabId = $module->getId();
		$sql = 'SELECT 1 FROM jo_profile2standardpermissions WHERE tabid = ? AND operation = ? LIMIT 1';
	...
	public static function getInstanceFromQResult($result, $rowNo=0) {
		...
		$className = 'Head_Action_Model';
		$actionName = $row['actionname'];
		if(!in_array($actionName, self::$standardActions)) {
			$className = 'Head_Utility_Model';
		}
	...
	public static function getInstance($value, $force=false) {
		...
		if (self::$cachedInstances) {
			$actionid = Head_Utils::isNumber($value) ? $value : false;
	...
	public static function getInstanceWithIdOrName($value) {
		...
		if(Head_Utils::isNumber($value)) {
			$sql = 'SELECT * FROM jo_actionmapping WHERE actionid=? LIMIT 1';
		} else {
			$sql = 'SELECT * FROM jo_actionmapping WHERE actionname=?';
		}
		$params = array($value);
		$result = $db->pquery($sql, $params);
	...
	public static function getAll($configurable=false) {
		$actionModels = Head_Cache::get('vtiger', 'actions');
        if(!$actionModels){
            $db = PearDatabase::getInstance();

            $sql = 'SELECT * FROM jo_actionmapping';
            $params = array();
            if($configurable) {
            ...
            Head_Cache::set('vtiger','actions', $actionModels);
        }
		return $actionModels;
	}
	...
	public static function getAllBasic($configurable=false) {
		...
		$sql = 'SELECT * FROM jo_actionmapping WHERE actionid IN ('. generateQuestionMarks($basicActionIds) .')';
	...
	public static function getAllUtility($configurable=false) {
		...
		$basicActionIds = array_keys(self::$standardActions);
		$sql = 'SELECT * FROM jo_actionmapping WHERE actionid NOT IN ('. generateQuestionMarks($basicActionIds) .')';
		$params = $basicActionIds;
		if($configurable) {
			$sql .= ' AND actionname NOT IN ('. generateQuestionMarks(self::$nonConfigurableActions) .')';
			$params = array_merge($params, self::$nonConfigurableActions);
