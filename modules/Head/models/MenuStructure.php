...
/**
 * Head MenuStructure Model
 */
class Head_MenuStructure_Model extends Head_Base_Model {
	 ...
	 * @return <array> - Associate array of Parent name mapped to Head_Menu_Model instances
	 */
	...
	/**
	 * Function to get an instance of the Head MenuStructure Model from list of menu models
	 * @param <array> $menuModelList - array of Head_Menu_Model instances
	 * @return Head_MenuStructure_Model instance
	 */
	public static function getInstanceFromMenuList($menuModelList, $selectedMenu='') {
		...
		foreach ($menuListArray[self::MORE_MENU_INDEX] as $parent => &$values) {
			uksort($values, array('Head_MenuStructure_Model', 'sortMenuItemsByProcess'));
		}
		//uksort($menuListArray[self::TOP_MENU_INDEX], array('Head_MenuStructure_Model', 'sortMenuItemsByProcess'));
	...
	function regroupMenuByParent($menuGroupedByParent) {
		...
		$oldToNewAppMap = Head_MenuStructure_Model::getOldToNewAppMapping();
