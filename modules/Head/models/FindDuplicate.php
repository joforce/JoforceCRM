...
class Head_FindDuplicate_Model extends Head_Base_Model {
	...
	function getListViewHeaders() {
		...
		$listViewHeaders[] = new Head_Base_Model(array('name' => 'recordid', 'label' => 'Record Id'));
	...
	function getListViewEntries(Head_Paging_Model $paging) {
        $db = PearDatabase::getInstance();
        $moduleModel = $this->getModule();
    ...
    public function getMassDeleteRecords(Head_Request $request) {
        $db = PearDatabase::getInstance();
