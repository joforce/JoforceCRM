...
class Head_ExportData_Action extends Head_Mass_Action {
	...
	function getExportQuery(Head_Request $request) {
		...
		if ($mode !== 'ExportAllData') {
			...
			$searchParams = Head_Util_Helper::transferListSearchParamsToFilterCondition($searchParams, $this->moduleInstance);
			...
			if ($orderBy && $orderByFieldModel) {
				if ($orderByFieldModel->getFieldDataType() == Head_Field_Model::REFERENCE_TYPE || $orderByFieldModel->getFieldDataType() == Head_Field_Model::OWNER_TYPE) {
		...
		switch($mode) {
			...
			case 'ExportCurrentPage' :	$pagingModel = new Head_Paging_Model();
	 ...
	 * @return <String>
	 */
	function getExportContentType(Head_Request $request) {
	...
	function sanitizeValues($arr){
		...
		if(empty ($this->fieldArray)){
			...
			foreach($this->fieldArray as $fieldName => $fieldObj){
				...
				if($fieldObj->get('table') == 'jo_inventoryproductrel' && ($fieldName == 'discount_amount' || $fieldName == 'discount_percent')){
		...
		foreach($arr as $fieldName=>&$value){
			...
			if($fieldname != 'hdnTaxType' && ($uitype == 15 || $uitype == 16 || $uitype == 33)){
				...
				$value = Head_Util_Helper::getOwnerName($value);
				...
				if (trim($value) && $value != '0000-00-00 00:00:00') {
					$value = Head_Datetime_UIType::getDisplayDateTimeValue($value);
				}
			}
