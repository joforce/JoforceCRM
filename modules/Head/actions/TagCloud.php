...
class Head_TagCloud_Action extends Head_Mass_Action {
	...
	public function save(Head_Request $request) {
		...
		$taggedInfo = Head_Tag_Model::getAll($currentUser->id, $request->getModule(), $request->get('record'));
	 ...
	 * @param Head_Request $request
	 */
	public function getTags(Head_Request $request) {
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$record = $request->get('record');
		$module = $request->getModule();
		$tags = Head_Tag_Model::getAll($currentUser->id, $module, $record);

		$response = new Head_Response();
		$response->emit($tags);
	}

	public function saveTags(Head_Request $request) {
		...
		foreach($recordIds as $recordId) {
			if(!empty($recordId)){
				Head_Tag_Model::saveForRecord($recordId, $existingTags, $userId, $module);
				Head_Tag_Model::deleteForRecord($recordId, $deletedTags, $userId, $module);
			}
		}


		$allAccessibleTags =  Head_Tag_Model::getAllAccessible($userId, $module, $recordId);
		...
		$result['moreTagCount'] = $allAccessibleTagCount - Head_Tag_Model::NUM_OF_TAGS_DETAIL;
	...
	public function update(Head_Request $request) {
		$module = $request->get('module');
		...
		$response = new Head_Response();
		try{
			$tagModel = Head_Tag_Model::getInstanceById($tagId);
			$otherTagModelWithSameName = Head_Tag_Model::getInstanceByName($tagName, $currentUser->getId(), $tagId);
			if($otherTagModelWithSameName !== false) {
				throw new Exception(vtranslate('LBL_SAME_TAG_EXISTS', $module, $tagName));
			}
			if($tagModel->getType() == Head_Tag_Model::PUBLIC_TYPE && $visibility == Head_Tag_Model::PRIVATE_TYPE) {
				//TODO : check if there are no other records tagged by other users 
			   if(Head_Tag_Model::checkIfOtherUsersUsedTag($tagId, $currentUser->getId())) {
	...
	public function remove(Head_Request $request) {
		$currentUser = Users_Record_Model::getCurrentUserModel();
