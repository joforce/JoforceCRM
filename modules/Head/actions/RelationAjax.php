...
class Head_RelationAjax_Action extends Head_Action_Controller {
	...
	function deleteRelation($request) {
		...
		if($relatedModule == 'Calendar' && !empty($recurringEditMode) && $recurringEditMode != 'current') {
			foreach($relatedRecordIdList as $relatedRecordId) {
				$recordModel = Head_Record_Model::getCleanInstance($relatedModule);
	 ...
	 */
	function getRelatedListPageCount(Head_Request $request){
