	...
	public function process(Head_Request $request) {
		...
		foreach ($fieldModelList as $fieldName => $fieldModel) {
			...
			if ($fieldModel->getFieldDataType() == 'currency') {
				$displayValue = Head_Currency_UIType::transformDisplayValue($fieldValue);
			}
	...
	public function getRecordModelFromRequest(Head_Request $request) {
		...
		if(!empty($recordId)) {
			...
			foreach ($fieldModelList as $fieldName => $fieldModel) {
				...
				// To support Inline Edit in Head7
				...
				if($fieldName === 'contact_id' && isRecordExists($fieldValue)) {
					$contactRecord = Head_Record_Model::getInstanceById($fieldValue, 'Contacts');
