...
class Head_BasicAjax_View extends Head_Basic_View {
	 ...
	 */
	function showAdvancedSearch(Head_Request $request) {
		//Modules for which search is excluded
		$excludedModuleForSearch = array('Head', 'Reports');
	 ...
	 * @param Head_Request $request
	 */
	function showSearchResults(Head_Request $request) {
		...
		if(is_array($advFilterList) && count($advFilterList) > 0) {
			...
			for($i=0; $i<$rows; ++$i) {
				...
				$recordInstance = Head_Record_Model::getInstanceById($row[0]);
			...
			$matchingRecords =  Head_Record_Model::getSearchResult($searchKey, $searchModule);
		}
