...
class Head_ExtensionViews_View extends Head_Index_View {
	...
	function convertLogDetailsToUserFormat($logDetails, $moduleName) {
		...
		foreach ($logDetails as $logId) {
			if(!is_numeric($logId)) {
				...
				if($logId && $moduleId) {
					$wsObject = HeadWebserviceObject::fromId($db, $moduleId);
	...
	function showLogs(Head_Request $request) {
	...
	function showLogDetail(Head_Request $request) {
