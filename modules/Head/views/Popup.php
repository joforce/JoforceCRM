...
class Head_Popup_View extends Head_Footer_View {
	 ...
	 * @return <String>
	 */
	function getModule(Head_request $request) {
		$moduleName = $request->getModule();
		return $moduleName;
	}
	...
	public function initializeListViewContents(Head_Request $request, Head_Viewer $viewer) {
		...
		$isRecordExists = Head_Util_Helper::checkRecordExistance($relatedParentId);
		...
		if(!empty($relatedParentModule) && !empty($relatedParentId)) {
			$parentRecordModel = Head_Record_Model::getInstanceById($relatedParentId, $relatedParentModule);
			$listViewModel = Head_RelationListView_Model::getInstance($parentRecordModel, $moduleName, $label,$relationId);
	...
	function getListViewCount(Head_Request $request){
		...
		if(!empty($relatedParentModule) && !empty($relatedParentId)) {
			$parentRecordModel = Head_Record_Model::getInstanceById($relatedParentId, $relatedParentModule);
			$listViewModel = Head_RelationListView_Model::getInstance($parentRecordModel, $moduleName, $label);
		}else{
			$listViewModel = Head_ListView_Model::getInstanceForPopup($moduleName);
		}
