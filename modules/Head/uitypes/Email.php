...
class Head_Email_UIType extends Head_Base_UIType {
		...
		if($value){
			...
			if ($internalMailer == 1) {
                ...
                if($moduleName == "Users" && $fieldName == "user_name"){
                    $value = "<a class='cursorPointer' onclick=\"Head_Helper_Js.getInternalMailer($recordId," .
                    		"'$fieldName','$moduleName');\">" . textlength_check($value) . "</a>";
                }else{
                	$value = "<a class='emailField cursorPointer' onclick=\"Head_Helper_Js.getInternalMailer($recordId," .
                    		"'$fieldName','$moduleName');\">" . textlength_check($value) . "</a>";
                }
			} else {
                if($moduleName == "Users" && $fieldName == "user_name"){
