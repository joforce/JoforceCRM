...
class Head_Util_Helper {
	...
	public static function formatDateDiffInStrings($dateTime, $isUserFormat = FALSE) {
		try{
			...
			if($isUserFormat) {
				$dateTime = Head_Datetime_UIType::getDBDateTimeValue($dateTime);
			}
	 ...
	 */
	public static function toHead6SafeHTML($input) {
		$allowableTags = '<a><br>';
	...
	public static function checkRecordExistance($recordId){
		global $adb;
		$query = 'Select deleted from jo_crmentity where crmid=?';
	...
	public static function formatDateIntoStrings($date, $time = false) {
		...
		$dateTimeInUserFormat = Head_Datetime_UIType::getDisplayDateTimeValue($date . ' ' . $time);
		...
		if ($currentUser->get('hour_format') === '12') {
			$displayTime = Head_Time_UIType::getTimeValueInAMorPM($displayTime);
		}

		$today = Head_Date_UIType::getDisplayDateValue(date('Y-m-d H:i:s'));
		$tomorrow = Head_Date_UIType::getDisplayDateValue(date('Y-m-d H:i:s', strtotime('tomorrow')));
	...
	public static function getRecordName ($recordId, $checkDelete=false) {
		...
		$query = 'SELECT label from jo_crmentity where crmid=?';
		if($checkDelete) {
			$query.= ' AND deleted=0';
		}
		$result = $adb->pquery($query,array($recordId));
	...
	public static function getRecordId($recordName, $module = array(), $checkDelete = false) {
		...
		$query = 'SELECT crmid from jo_crmentity where label=?';
	...
	public static function formatDateTimeIntoDayString($dateTime, $skipConversion = FALSE) {
		...
		if (!$skipConversion) {
			$dateTimeInUserFormat = Head_Datetime_UIType::getDisplayDateTimeValue($dateTime);
		}
	...
	public static function getPickListValues($fieldName) {
		if($fieldName !== "default_landing_page")
		{
			...
			$query = 'SELECT '.$primaryKey.', '.$fieldName.' FROM jo_'.$fieldName.' order by sortorderid';
			$values = array();
		...
		{
			$values = getPermittedEntityModuleNames();
			return $values;
		}
	}
	...
	public static function getBaseCurrency() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM jo_currency_info WHERE defaultid < 0', array());
	...
	public static function getRoleBasedPicklistValues($fieldName, $roleId) {
		if(Head_Cache::get('PicklistRoleBasedValues',$fieldName.$roleId)){
			return Head_Cache::get('PicklistRoleBasedValues',$fieldName.$roleId);
		}
		$db = PearDatabase::getInstance();

		$query = "SELECT $fieldName
				  FROM jo_$fieldName
					  INNER JOIN jo_role2picklist on jo_role2picklist.picklistvalueid = jo_$fieldName.picklist_valueid
				  WHERE roleid=? and picklistid in (select picklistid from jo_picklist) order by sortorderid";
		...
		Head_Cache::set('PicklistRoleBasedValues',$fieldName.$roleId,$picklistValues);
...
/**
	...
	public static function getCreator($recordId) {
		...
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT smcreatorid FROM jo_crmentity WHERE crmid = ?', array($recordId));
	 ...
	 * @param <DateTime> $dateTime
	 * @param <Head_Users_Model> $userObject
	...
	public static function convertDateTimeIntoUsersDisplayFormat($dateTime, $userObject = null) {
		...
		if ($userModel->get('hour_format') == '12') {
			$time = Head_Time_UIType::getTimeValueInAMorPM($time);
		}
	...
	public static function getUserCurrencyInfo() {
		$db = PearDatabase::getInstance();
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$result = $db->pquery('SELECT * FROM jo_currency_info WHERE id = ?', array($currentUser->get('currency_id'))); 
		if($db->num_rows($result)) return $db->query_result_rowdata($result, 0);
	}
			...
			foreach($groupInfo as $fieldSearchInfo){
				   ...
				   preg_match('/(\w+) ; \((\w+)\) (\w+)/', $fieldName, $matches);
					if (count($matches) != 0) {
						...
						$fieldInfo = Head_Field_Model::getInstance($referenceFieldName, $referenceModuleModel);
					...
					if(empty($fieldInfo) && $referenceModule == 'Calendar') {
						...
						$fieldInfo = Head_Field_Model::getInstance($referenceFieldName, $eventsModuleModel);
					}
	...
	*/
	public static function setCalendarDefaultActivityTypesForUser($userId) {
		$db = PearDatabase::getInstance();
		$userEntries = $db->pquery('SELECT 1 FROM jo_calendar_user_activitytypes WHERE userid=?', array($userId));
		if($db->num_rows($userEntries) <= 0) {
			$queryResult = $db->pquery('SELECT id, defaultcolor FROM jo_calendar_default_activitytypes WHERE isdefault = ?', array('1'));
			...
			foreach($activityIds as $activityId=>$color) {
				$db->pquery('INSERT INTO jo_calendar_user_activitytypes (id, defaultid, userid, color) VALUES (?,?,?,?)', array($db->getUniqueID('jo_calendar_user_activitytypes'), $activityId, $userId, $color));
			}
		}
	...
	public static function isUserDeleted($userid) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT deleted FROM jo_users WHERE id = ? AND (status=? OR deleted=?)', array($userid, 'Inactive', 1));
   ...
   public static function checkDbUTF8Support($conn) {
	...
	public static function getCalendarReferenceModulesList() {
		$moduleInstance = Head_Module_Model::getInstance('Calendar');
		$fieldInstance = Head_Field_Model::getInstance('parent_id', $moduleInstance);
	...
	public static function getBrowserInfo() {
		...
		$browserInfoModel = new Head_Base_Model($browserInfo);
	...
	static function fetchRecordLabelsForIds($recordIds) {
		...
		$sqlResult = $adb->pquery("SELECT crmid,label from jo_crmentity WHERE crmid IN (".generateQuestionMarks($crmIds).") ;", $crmIds);
		...
		switch ($fieldDataType) {
			...
			case 'reference'	:	$referenceFieldModule = $fieldModel->getReferenceList(true);
									if (count($referenceFieldModule) > 0) {
										...
										$referenceFieldModuleModel = Head_Module_Model::getInstance($referenceModule);
										...
										foreach ($mandatoryFieldModels as $mandatoryFieldModel) {
											...
											$fieldValue = Head_Util_Helper::fillMandatoryFields($fieldName, $referenceModule);
										...
										$query = "SELECT crmid FROM jo_crmentity WHERE label = ? AND deleted = ? AND setype = ?";
