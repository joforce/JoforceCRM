...
class Google_Setting_View extends Head_PopupAjax_View {
	...
	public function emitCalendarSyncSettingUI(Head_Request $request) {
	...
	public function emitContactsSyncSettingUI(Head_Request $request) {
	   echo $this->intializeContactsSyncSettingParameters($request);
	}

	public function intializeContactsSyncSettingParameters(Head_Request $request) {
		$user = Users_Record_Model::getCurrentUserModel();
		$connector = new Google_Contacts_Connector(FALSE);
		...
		$contactsModuleModel = Head_Module_Model::getInstance($request->get('sourcemodule'));
		...
		$viewer = $this->getViewer($request);
		if($request->get('onlyGoogleToHead')) {
			$viewer->assign('ONLY_GOOGLE_TO_JOFORCE', true);
		}
		...
		$viewer->assign('JOFORCE_EMAIL_FIELDS',$emailFields);
		$viewer->assign('JOFORCE_PHONE_FIELDS',$phoneFields);
		$viewer->assign('JOFORCE_URL_FIELDS',$urlFields);
		$viewer->assign('JOFORCE_OTHER_FIELDS',$otherFields);
		$viewer->assign('IS_SYNC_READY',$isSyncReady);
