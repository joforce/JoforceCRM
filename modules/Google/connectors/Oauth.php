...
vimport('~~/includes/Zend/Oauth.php');

vimport('~~/includes/Zend/Oauth/Consumer.php');
vimport('~~/includes/Zend/Gdata.php');
vimport('~~/includes/Zend/Crypt/Rsa/Key/Private.php');
vimport('~~/includes/Zend/Gdata/Query.php');
...
//		}
    ...
    function hasStoredToken($service, $accessToken = false, $requestToken = false) {
        ...
        if (!$accessToken && !$requestToken){
            $query = "SELECT  1 FROM jo_google_oauth WHERE  userid=? and service=?";
            $params = array($this->userId, $service);
        }
        else if ($accessToken){
            $query = "SELECT  access_token FROM jo_google_oauth WHERE  userid=? and service=? AND access_token<>? AND access_token IS NOT NULL";
            $params = array($this->userId, $service, '');
        }
        else if ($requestToken){
            $query = "SELECT  request_token FROM jo_google_oauth WHERE  userid=? and service=? AND request_token<>? AND request_token IS NOT NULL";
            $params = array($this->userId, $service, '');
        }
    ...
    protected function storeAccessToken($service, $token) {
        ...
        $query = "INSERT INTO jo_google_oauth(service,access_token,userid) VALUES(?,?,?)";
        $params = array($service, base64_encode(serialize($token)), $user->getid());
        if (self::hasStoredToken($service, false, true)) {
            $query = "UPDATE jo_google_oauth SET access_token=? WHERE userid=? AND  service=?";
    ...
    protected function retreiveAccessToken($service) {
        ...
        $query = "SELECT access_token FROM jo_google_oauth WHERE userid=? AND service =?";
        $params = array($this->userId, $service);
    ...
    protected function storeRequestToken($service, $token) {
        ...
        $query = "INSERT INTO jo_google_oauth(service,request_token,userid) values(?,?,?)";
    ...
    protected function retrieveRequestToken($service) {
        ...
        $query = "SELECT request_token FROM jo_google_oauth WHERE userid=? AND service =?";
    ...
    function getHttpClient($service) {
	 ...
	 public static function initializeSchema(){
		 if(!Head_Utils::CheckTable('jo_google_oauth')) {
                Head_Utils::CreateTable('jo_google_oauth',
