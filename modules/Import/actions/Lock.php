...
class Import_Lock_Action extends Head_Action_Controller {
	...
	public static function lock($importId, $module, $user) {
		...
		if(!Head_Utils::CheckTable('jo_import_locks')) {
		...
		$adb->pquery('INSERT INTO jo_import_locks VALUES(?,?,?,?,?)',
						array($adb->getUniqueID('jo_import_locks'), $user->id, getTabid($module), $importId, date('Y-m-d H:i:s')));
	}

	public static function unLock($user, $module=false) {
		$adb = PearDatabase::getInstance();
		if(Head_Utils::CheckTable('jo_import_locks')) {
			$query = 'DELETE FROM jo_import_locks WHERE userid=?';
	...
	public static function isLockedForModule($module) {
		...
		if(Head_Utils::CheckTable('jo_import_locks')) {
			$lockResult = $adb->pquery('SELECT * FROM jo_import_locks WHERE tabid=?',array(getTabid($module)));
