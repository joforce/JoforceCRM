...
require_once 'includes/Webservices/Revise.php';
...
require_once 'modules/Head/CRMEntity.php';
...
class Import_Data_Action extends Head_Action_Controller
{
	var $id;
	var $user;
	var $module;
	...
	public function importData()
	{
		...
		$moduleModel = Head_Module_Model::getInstance($this->module);
		// pre fetch the fields and premmisions of module
		Head_Field_Model::getAllForModule($moduleModel);
		if ($this->user->is_admin == 'off') {
			Head_Field_Model::preFetchModuleFieldPermission($moduleModel->getId());
		}
	...
	public function createRecords()
	{
		...
		for ($i = 0; $i < $numberOfRecords; ++$i) {
				...
				if (!empty($mergeType) && $mergeType != Import_Utils_Helper::$AUTO_MERGE_NONE) {
					...
					if ($noOfDuplicates > 0) {
						if ($mergeType == Import_Utils_Helper::$AUTO_MERGE_IGNORE) {
							...
							$baseRecordModel = Head_Record_Model::getInstanceById($baseRecordId);
							...
							for ($index = 0; $index < $noOfDuplicates - 1; ++$index) {
								...
								if ($userPriviligesModel->hasModuleActionPermission($tabId, 'Delete')) {
									...
									if ($moduleName == 'Calendar') {
										$recordModel = Head_Record_Model::getInstanceById($duplicateRecordId);
										$recordModel->delete();
									} else {
			...
			if ($createRecord || $mergeType == Import_Utils_Helper::$AUTO_MERGE_MERGEFIELDS || $mergeType == Import_Utils_Helper::$AUTO_MERGE_OVERWRITE) {
				...
				if (!empty($recordId)) {
					...
					$recordModel = Head_Record_Model::getCleanInstance($this->module);
		...
		if (!empty($createdRecords)) {
			$recordModels = Head_Record_Model::getInstancesFromIds($createdRecords, $this->module);
			$entityInfos = array();
			foreach ($recordModels as $recordModel) {
				$focus = $recordModel->getEntity();
				$entityInfos[] = EntityData::fromCRMEntity($focus);
			}
		...
		if ($this->entitydata) {
			...
			$entity->triggerEvent('jo.batchevent.save', $this->entitydata);
		}
	...
	public function transformForImport($fieldData, $moduleMeta, $fillDefault = true, $checkMandatoryFieldValues = true)
	{
		...
		foreach ($fieldData as $fieldName => $fieldValue) {
			...
			if ($fieldDataType == 'owner') {
				...
				if (empty($ownerId) || !Import_Utils_Helper::hasAssignPrivilege($moduleName, $ownerId)) {
					...
					if (count($fieldValueDetails) > 1) {
						...
						foreach ($referencedModules as $referenceModule) {
							...
							if ($referenceModule == 'Users') {
								...
								if (empty($referenceEntityId) || !Import_Utils_Helper::hasAssignPrivilege($moduleName, $referenceEntityId)) {
				...
				if (!in_array($picklistValueInLowerCase, $allPicklistValuesInLowerCase) && !empty($picklistValueInLowerCase)) {
					if ($moduleName != 'Calendar') {
						...
						$moduleObject = Head_Module::getInstance($moduleName);
						$fieldObject = Head_Field::getInstance($fieldName, $moduleObject);
						...
						Head_Cache::getInstance()->setPicklistDetails($moduleObject->getId(), $fieldName, $wsFieldDetails);
	...
	public static function runScheduledImport()
	{
		...
		foreach ($scheduledImports as $scheduledId => $importDataController) {
			...
			$emailData = 'CRM has just completed your import process. <br/><br/>' .
				$importResult . '<br/><br/>' .
		...
		Head_Mailer::dispatchQueue(null);
	}
	...
	{
		$adb = PearDatabase::getInstance();
		$result = $adb->pquery("SELECT count(*) AS count FROM jo_import_queue WHERE status = ?", array(Import_Queue_Action::$IMPORT_STATUS_SCHEDULED));
			...
			if ($recordData['activitytype']) {
				...
				foreach ($recordData as $fieldName => $fieldValue) {
					if ($fieldValue) {
						...
						if (in_array($fieldDataType, array('reference', 'owner'))) {
							...
							$recordData[$fieldName] = Head_Time_UIType::getTimeValueWithSeconds($fieldValue);
						} else if ($fieldDataType == 'boolean') {
				...
				foreach ($recordData as $fieldName => $fieldValue) {
					if (in_array($fieldName, array('date_start', 'due_date'))) {
						...
						$dateTime = Head_Datetime_UIType::getDBDateTimeValue($dateParts[0] . ' ' . $recordData[$timeField]);
				...
				foreach ($recordData as $fieldName => $fieldValue) {
					...
					if ($fieldDataType == 'picklist') {
						...
						if (!in_array($fieldValueInLowerCase, $picklistValuesInLowerCase) && $fieldName !== 'visibility' && !($fieldName == 'activitytype' && $fieldValue == 'Task')) {
				...
				if (!empty($recordData['createdtime'])) {
					$recordData['createdtime'] = Head_Datetime_UIType::getDBDateTimeValue($recordData['createdtime']);
				}
				if ($operation != 'create') {
				...
				try {
					if ($recordData['id']) {
						$recordModel = Head_Record_Model::getInstanceById($recordData['id'], $moduleName);
					} else {
						$recordModel = Head_Record_Model::getCleanInstance($moduleName);
					}
					...
					$webServiceObj = HeadWebserviceObject::fromName($db, $moduleName);
		...
		try {
			if ($recordData) {
				switch ($operation) {
					...
					case 'update':
						$entityInfo = vtws_update($recordData, $user);
						$entityInfo['status'] = self::$IMPORT_RECORD_UPDATED;
						break;

					case 'revise':
						$entityInfo = vtws_revise($recordData, $user);
						$entityInfo['status'] = self::$IMPORT_RECORD_MERGED;
						break;
				}
			}
