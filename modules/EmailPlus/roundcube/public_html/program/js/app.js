...
function rcube_webmail() {
    ...
    switch (this.task) {
      ...
      case 'mail':
        ...
        if (this.gui_objects.messagelist) {
          this.env.widescreen_list_template = [{
            ...
            {
              className: 'subject',
              cells: ['fromto', 'date', 'status', 'subject']
            },
            {
              className: 'flags',
          ...
          this.message_list
            ...
            .addEventListener('listupdate', function (o) {
              ref.triggerEvent('listupdate', o);
            })
            .init();
        ...
        if (this.env.action == 'show' || this.env.action == 'preview') {
          ...
          if (bw.mz && this.env.mimetype == 'application/pdf') {
            ...
            $(this.gui_objects.messagepartframe).on('load', function () {
              if (n++) try {
                if (this.contentWindow.document) ref.enable_command('print', true);
              }
              catch (e) {
                /* ignore */
              }
      ...
      case 'settings':
        ...
        if (this.gui_objects.identitieslist) {
          ...
          this.sections_list = new rcube_list_widget(this.gui_objects.sectionslist, {
            multiselect: false,
            draggable: false,
            keyboard: true
          });
          this.sections_list
            ...
            .addEventListener('keypress', function (o) {
              if (o.key_pressed == o.ENTER_KEY) ref.section_select(o);
            })
            .init()
            .focus();
          ...
          this.responses_list = new rcube_list_widget(this.gui_objects.responseslist, {
            multiselect: false,
            draggable: false,
            keyboard: true
          });
      ...
      case 'login':
        ...
        input_user.keyup(function (e) {
          return ref.login_user_keyup(e);
        });
    ...
    switch (command) {
      ...
      case 'extwin':
        if (this.env.action == 'compose') {
          ...
          if (win) {
            ...
            form.action = this.url('mail/compose', {
      ...
      case 'edit':
        if (this.task == 'addressbook' && (cid = this.get_single_cid()))
          ...
          url = {
            _mbox: this.get_message_mailbox(uid)
          };
      ...
      case 'always-load':
        ...
        case 'download-attachment':
          ...
          if (command != 'download-attachment' && mimetype && this.env.mimetypes && $.inArray(mimetype, this.env.mimetypes) >= 0) {
            if (this.open_window(this.url('get', $.extend({
                _frame: 1
              }, params))))
              break;
          }
        ...
        case 'forward':
          ...
          if (uids.length) {
            url = {
              _forward_uid: this.uids_to_list(uids),
              _mbox: this.env.mailbox,
              _search: this.env.search_request
            };
        ...
        case 'print':
          if (this.task == 'addressbook') {
            ...
            url = this.url('print', this.params_from_uid(uid, {
        ...
        case 'download':
          if (this.env.action == 'get') {
            ...
            this.goto_url('viewsource', this.params_from_uid(uid, {
              _save: 1
            }), false, true);
          }
          break;
  ...
  this.doc_keypress = function (e) {
    ...
    var focus_menu_item = function (dir) {
      var obj, item, mod = dir < 0 ? 'prevAll' : 'nextAll',
  ...
  this.add_message_row = function (uid, cols, flags, attop) {
    ...
    var c, n, col, html, css_class, label, status_class = '',
      status_label = '',
      tree = '',
      expando = '',
    ...
    for (n in this.env.listcols) {
      ...
      col = {
        className: String(c).toLowerCase(),
        events: {}
      };
  ...
  this.set_list_options = function (cols, sort_col, sort_order, threads, layout) {
    ...
    if (cols && cols.length) {
      ...
      var i, idx, name, newcols = [],
        oldcols = this.env.listcols;
  ...
  this.mailvelope_submit_messageform = function (draft, saveonly) {
    ...
    ref.mailvelope_keyring.validKeyForAddress(recipients).then(function (status) {
      ...
      var senders = [],
        selected_sender = ref.env.identities[$("[name='_from'] option:selected").val()];
  ...
  this.mailvelope_display_container = function (selector, data, keyring, msgid) {
    ...
    mailvelope.createDisplayContainer(selector, data, keyring, {
      showExternalContent: this.env.safemode
      ...
      for (var id, n = 0; n < this.contact_list.selection.length; n++) {
        ...
        if (id && this.env.contactdata[id]) {
          ...
          if (id.charAt(0) == 'E' && this.env.contactdata[id].indexOf('@') < 0 && input.length) {
            var gid = id.substr(1);
            this.group2expand[gid] = {
              name: this.env.contactdata[id],
              input: input.get(0)
            };
  ...
  this.save_response = function () {
    ...
    buttons[this.get_label('save')] = function (e) {
      ...
      ref.http_post('settings/responses', {
        _insert: 1,
        _name: name,
        _text: text
      }, lock);
    ...
    this.show_popup_dialog(html, this.get_label('newresponse'), buttons, {
  ...
  this.delete_response = function (key) {
    ...
    if (key && confirm(this.get_label('deleteresponseconfirm'))) {
      this.http_post('settings/delete-response', {
        _key: key
      }, false);
    }
  };
  ...
  this.rename_attachment = function (id) {
    ...
    this.simple_dialog(content, 'attachmentrename', function () {
      ...
      if ((name = input.val()) && name != attachment.name) {
        ref.http_post('rename-attachment', {
  ...
  this.continue_search = function (request_id) {
    ...
    setTimeout(function () {
      ...
      ref.env.qsearch = {
        lock: lock,
        request: ref.http_request('search', url, lock)
      };
    }, 100);
  };
  ...
  this.insert_recipient = function (id) {
    ...
    if (typeof this.env.contacts[id] === 'object' && this.env.contacts[id].type == 'group' && !this.env.contacts[id].email) {
      ...
      this.group2expand[this.env.contacts[id].id] = $.extend({
        input: this.ksearch_input
      }, this.env.contacts[id]);
      this.http_request('mail/group-expand', {
        _source: this.env.contacts[id].source,
        _gid: this.env.contacts[id].id
      }, false);
  ...
  this.replace_group_recipients = function (id, recipients) {
    if (this.group2expand[id]) {
      ...
      this.triggerEvent('autocomplete_insert', {
        field: this.group2expand[id].input,
        insert: recipients,
  ...
  this.ksearch_query_results = function (results, search, reqid) {
    ...
    if (!this.ksearch_pane) {
      ...
      this.ksearch_pane = $('<div>').attr('id', 'rcmKSearchpane').attr('role', 'listbox')
        .css({
          position: 'absolute',
          'z-index': 30000
        }).append(ul).appendTo(document.body);
    ...
    if (reqid && this.ksearch_pane.data('reqid') == reqid) {
      ...
      this.ksearch_pane.css({
        left: pos.left + 'px',
        top: (pos.top + this.ksearch_input.offsetHeight) + 'px',
        display: 'none'
      });
    }
  ...
  this.group_remove_selected = function () {
    this.http_post('group-delmembers', {
      _cid: this.contact_list.selection,
      _source: this.env.source,
      _gid: this.env.group
    });
  };
  ...
  this.insert_contact_group = function (prop) {
    ...
    }, prop.source, 'contactgroup');

    this.triggerEvent('group_insert', {
      id: prop.id,
      source: prop.source,
      name: prop.name,
      li: this.treelist.get_item(key)
    });
  };
    ...
    if (elem.length) {
      ...
      if (appendcontainer.length && appendcontainer.get(0).nodeName == 'FIELDSET') {
        ...
        if (colprop.type == 'text' || colprop.type == 'date') {
          ...
          var i, childcol, cp, first, templ, cols = [],
            suffices = [];
  ...
  this.remove_search_item = function (id) {
    ...
    if (this.savedsearchlist.remove(key)) {
      this.triggerEvent('search_delete', {
        id: id,
        li: li
      });
    }
  ...
  this.qrcode = function () {
    ...
    img.src = this.url('addressbook/qrcode', {
      _source: this.env.source,
      _cid: this.env.cid
    });

    return this.show_popup_dialog(img, title, buttons, {
      width: 310,
      height: 410
    });
  };
  ...
  this.delete_identity = function (id) {
    ...
    if (id && confirm(this.get_label('deleteidentityconfirm')))
      this.http_post('settings/delete-identity', {
        _iid: id
      }, true);
  };
  ...
  this.init_subscription_list = function () {
    ...
    this.subscription_list
      ...
      .addEventListener('search', function (p) {
        if (p.query) ref.subscription_select();
      })
      .draggable({
        cancel: 'li.mailbox.root,input,div.treetoggle'
      })
      .droppable({
  ...
  this.add_folder_row = function (id, name, display_name, is_protected, subscribed, class_name, refrow, subfolders) {
    ...
    row.attr({
      id: 'rcmli' + this.html_identifier_encode(id),
      'class': class_name
    });
    ...
    $('input[name="_subscribed[]"]:first', row).val(id)
      .prop({
        checked: subscribed ? true : false,
        disabled: is_protected ? true : false
      });
  ...
  this.remove_folder_row = function (folder) {
    ...
    var list = [],
      row = this.subscription_list.get_item(folder, true);
  ...
  this.show_menu = function (prop, show, event) {
    ...
    if (!obj.length) {
      obj = this.triggerEvent('menu-get', {
        name: name,
        props: prop,
        originalEvent: event
      });
    ...
    if (!obj || !obj.length) {
      ...
      return this.triggerEvent(show === false ? 'menu-close' : 'menu-open', {
        name: name,
        props: prop,
        originalEvent: event
      });
  ...
  this.hide_menu = function (name, event) {
    ...
    for (var j = this.menu_stack.length - 1; j >= 0; j--) {
      ...
      this.triggerEvent('menu-close', {
        name: this.menu_stack[j],
        obj: obj,
        props: {
          menu: this.menu_stack[j]
        },
        originalEvent: event
      });
  ...
  this.http_response = function (response) {
    ...
    switch (response.action) {
      ...
      case 'delete':
        if (this.task == 'addressbook') {
          var sid, uid = this.contact_list.get_selection(),
            writable = false;
  ...
  */
  ...
  this.async_upload_form_frame = function (name) {
    return $('<iframe>').attr({
        name: name,
        style: 'border: none; width: 0; height: 0; visibility: hidden'
      })
      .appendTo(document.body);
  };
  ...
  this.file_dropped = function (e) {
    ...
    var submit_data = function () {
      ...
      $.ajax({
        ...
        headers: {
          'X-Roundcube-Request': ref.env.request_token
        },
        xhr: function () {
        ...
        error: function (o, status, err) {
          ref.http_error(o, status, err, null, 'attachment');
        }
      });
    };
  ...
  this.refresh = function () {
    ...
    var params = {},
      lock = this.set_busy(true, 'refreshing');
  ...
  this.get_single_uid = function () {
    ...
    var result = ref.triggerEvent('get_single_uid', {
      uid: uid
    });
    return result || uid;
  };
  ...
  this.get_single_cid = function () {
    ...
    var result = ref.triggerEvent('get_single_cid', {
      cid: cid
    });
    return result || cid;
  };
  ...
  this.get_input_selection = function (obj) {
    ...
    return {
      ...
      text: normalizedValue.substr(start, end - start)
    };
  };
    ...
    $.each(events, function (event_name, event_handler) {
      frame.on('load', function (e) {
        try {
          ...
          /* catch possible permission error in IE */
        }
      });
