...
class PDFMaker_ComposeEmail_View extends Head_Footer_View {
	...
	public function composeMailData($request){
		...
		$viewer->assign('MAX_UPLOAD_SIZE', Head_Util_Helper::getMaxUploadSizeInBytes());
		...
		if ($documentIds) {
			...
			foreach ($documentIds as $documentId) {
				$documentRecordModel = Head_Record_Model::getInstanceById($documentId, $sourceModule);
		...
		if ($sourceRecordId) {
			$sourceRecordModel = Head_Record_Model::getInstanceById($sourceRecordId);
		...
		if (!empty($selectedFields)) {
			if($request->get('emailSource') == 'ListView') {
				...
				foreach($selectIds as $recordId) {
					$recordModel = Head_Record_Model::getInstanceById($recordId, $sourceModule);
					if($recordModel) {
						...
						foreach($selectedFields as $selectedFieldJson) {
							...
							if(!empty($selectedFieldInfo['basefield'])) {
								...
								if(!empty($refFieldValue)) {
									try {
										$refRecordModel = Head_Record_Model::getInstanceById($refFieldValue, $refModule);
			...
			foreach($selectIds as $id) { 
				  if ($id) { 
						...
						if (count($parentIdComponents) > 1) { 
								...
								$recordModel = Head_Record_Model::getInstanceById($id, $fieldModule); 
						} else { 
								$recordModel = Head_Record_Model::getInstanceById($id); 
						} 
						if($selectedFields){ 
		...
		if(empty($to)){
			...
			$getContactId = $adb->pquery("select * from {$moduleInfo[$sourceModule]['table']} where {$moduleInfo[$sourceModule]['id']}  = ?", array($recordId));
			$contactId = $adb->query_result($getContactId, 0, 'contactid');
			$getToEmail = $adb->pquery("select email from jo_contactdetails where contactid = ?", array($contactId));
			$to[] = $adb->query_result($getToEmail, 0, 'email');
		}
		$documentsModel = Head_Module_Model::getInstance('Documents');
		$documentsURL = $documentsModel->getInternalDocumentsURL();

		$emailTemplateModuleModel = Head_Module_Model::getInstance('EmailTemplates');
		...
		$viewer->assign('MODULE_IS_ACTIVE', $userPrevilegesModel->hasModulePermission(Head_Module_Model::getInstance('EmailTemplates')->getId()));
		//
    ...
    public function process(Head_Request $request) {
        ...
        $getTemplateDetails = $adb->pquery('select name,pdfmakerid,settings from jo_pdfmaker where module = ?', array($source_module));
        $template = $adb->query_result($getTemplateDetails, 0, 'name');
        $templateId = $adb->query_result($getTemplateDetails, 0, 'pdfmakerid');
		$settings = $adb->query_result($getTemplateDetails, 0, 'settings');
        ...
        $viewer->assign('FILE_NAME', $filename.'.pdf');
        $viewer->assign('source_module', $source_module);
        $viewer->assign('sourceId', $sourceId);
	...
	function getHeaderScripts(Head_Request $request) {
		...
		$jsHeaderScriptNames = array(
			...
			'layouts.v7.modules.Head.resources.BasicSearch'
		);
	...
	function emailEdit($request){
		...
		foreach($parentIds as $index=>$parentFieldId) {
			...
			if ($parentIdComponents[1] === '-1') {
				...
				$parentRecordModel = Head_Record_Model::getInstanceById($parentId);
			}
			...
			foreach ($referenceFields as $fieldName) {
				...
				$refFieldModlule = Head_Functions::getCRMRecordType($refRecordId);
				if(!$refFieldModlule) continue;
				if(!isRecordExists($refRecordId)) {
					continue;
				}
				$refRecordModel = Head_Record_Model::getInstanceById($refRecordId);
