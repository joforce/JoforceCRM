...
class PDFMaker_MassSaveAjax_View extends Head_Footer_View {
...
//			$recordModel->save();
		...
		$templateId = $request->get('templateId');
                $getTemplateDetails = $adb->pquery('select * from jo_pdfmaker where pdfmakerid = ?', array($templateId));
                $templateDetails = $adb->query_result($getTemplateDetails, 0, 'body');
//                $html = getMergedDescription($templateDetails, $sourceId, $sourceModule);
                $tempName = $adb->query_result($getTemplateDetails, 0, 'name');
                ...
                $mpdf = $helperObj->convertFieldAndExportPDF($sourceModule, $sourceId, $templateId, $type = 'detailview_send_mail');
			...
			$success = true;
			if($flag == 'SENT') {
				$status = $recordModel->send($mpdf, $filename, $unserializedValue);
				if ($status === true) {
