...
class PDFMaker_ListView_Model extends Head_ListView_Model {
	...
	public function getListViewHeaders() {
		...
		foreach ($listViewHeaders as $key => $fieldName) {
			$fieldModel = new PDFMaker_Field_Model();
	...
	public function getListViewEntries($pagingModel) {
		...
		for ($i = 0; $i < $num_rows; $i++) {
			...
			$listViewRecordModels[$row['pdfmakerid']] = $recordModel->setData($row);
		}
	...
	function getQuery() {
		$listQuery = 'SELECT pdfmakerid, name, module, status FROM jo_pdfmaker';
		return $listQuery;
	}
