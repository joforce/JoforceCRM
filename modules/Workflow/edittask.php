...
require_once("includes/events/WSEntityType.inc");

require_once("WorkflowApplication.inc");
require_once("TaskManager.inc");
...
function vtTaskEdit($adb, $request, $current_language, $app_strings)
{
	...
	$module = new WorkflowApplication('edittask');
	...
	if (isset($request["task_id"])) {
		...
		$taskClass = modlib_purifyForSql($request["task_type"]);
	...
	$smarty->assign("returnUrl", modlib_purify($request["return_url"]));
	$smarty->assign("task", $task);
	$smarty->assign("taskType", $taskClass);
	$smarty->assign("saveType", modlib_purify($request['save_type']));
	...
	$et = WSEntityType::usingGlobalCurrentUser($workflow->moduleName);
	...
	if ($task->trigger != null) {
		...
		$smarty->assign('trigger', array(
			'days' => $days, 'direction' => $direction,
			'field' => $trigger['field']
		));
	}
