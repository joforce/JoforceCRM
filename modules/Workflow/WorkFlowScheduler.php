...
require_once('modules/Workflow/WorkflowScheduler.inc');
...
class WorkFlowScheduler
{
	...
	public function queueScheduledWorkflowTasks()
	{
		...
		$vtWorflowManager = new WorkflowManager($adb);
		$taskQueue = new TaskQueue($adb);
				...
				do {
					...
					for ($j = 0; $j < $noOfRecords; ++$j) {
						...
						if ($moduleName == 'Events') {
							Head_Functions::setEventsContactIdToRequest($recordId);
						}
						foreach ($tasks as $task) {
							if ($task->active) {
								$delay = 0;
								$taskClassName = get_class($task);
								//Check whether task is EmailTask and then check emailoptout value 
								//if enabled don't queue the email 
								if ($taskClassName == 'EmailTask') {
		...
		if ($noOfConditions > 0) {
			...
			foreach ($conditions as $index => $condition) {
				...
				if ($fieldname == 'assigned_user_id') {
					$userName = Head_Functions::getUserRecordLabel($value);
					if (empty($userName)) {
						$userName = Head_Functions::getGroupRecordLabel($value);
					}
					$value = $userName;
				}
				if ($referenceField) {
	...
	{
		return array(
			'less than days ago', 'more than days ago', 'in less than', 'in more than', 'days ago', 'days later',
