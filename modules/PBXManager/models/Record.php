...
class PBXManager_Record_Model extends Head_Record_Model{
    
    const moduletableName = 'jo_pbxmanager';
    const lookuptableName = 'jo_pbxmanager_phonelookup';
    const entitytableName = 'jo_crmentity';
    ...
    public function saveRecordWithArrray($params){
        $moduleModel = Head_Module_Model::getInstance('PBXManager');
        $recordModel = Head_Record_Model::getCleanInstance('PBXManager');
    ...
    public static function getUserNumbers(){
        $numbers = null;
        $db = PearDatabase::getInstance();
        $query = 'SELECT id, phone_crm_extension FROM jo_users';
        $result = $db->pquery($query, array());
