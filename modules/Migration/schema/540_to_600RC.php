...
//	updateVtlibModule('Assets', 'packages/head/optional/Assets.zip');
	...
	installVtlibModule('Google', 'packages/head/optional/Google.zip');
	installVtlibModule('EmailTemplates', 'packages/head/optional/EmailTemplates.zip');

	// updated language packs.

	updateVtlibModule('PT Brasil', 'packages/head/optional/BrazilianLanguagePack_bz_bz.zip');
	updateVtlibModule('British English', 'packages/head/optional/BritishLanguagePack_br_br.zip');
	...
	updateVtlibModule('Mexican Spanish', 'packages/head/optional/MexicanSpanishLanguagePack_es_mx.zip');
	updateVtlibModule('Spanish', 'packages/head/optional/Spanish.zip');
	installVtlibModule('Italian', 'packages/head/optional/ItalianLanguagePack_it_it.zip');
	installVtlibModule('RomanianLanguagePack_rm_rm', 'packages/head/optional/RomanianLanguagePack_rm_rm.zip');
	installVtlibModule('Turkce', 'packages/head/optional/TurkishLanguagePack_tr_tr.zip');
	installVtlibModule('Russian', 'packages/head/optional/Russian.zip');
	installVtlibModule('Polish', 'packages/head/optional/PolishLanguagePack_pl_pl.zip');
	installVtlibModule('Russian', 'packages/head/optional/Russian.zip');
}

if(!defined('INSTALLATION_MODE')) {
	Migration_Index_View::ExecuteQuery('ALTER TABLE com_jo_workflows ADD COLUMN filtersavedinnew int(1)', array());
}

Migration_Index_View::ExecuteQuery('UPDATE com_jo_workflows SET filtersavedinnew = 5', array());
...
if (!($adb->num_rows($result))) { $adb->pquery("ALTER TABLE com_jo_workflows ADD schtypeid INT(10)", array()); }
$result = $adb->pquery("show columns from com_jo_workflows like ?", array('schtime'));
if (!($adb->num_rows($result))) { $adb->pquery("ALTER TABLE com_jo_workflows ADD schtime TIME", array()); }
$result = $adb->pquery("show columns from com_jo_workflows like ?", array('schdayofmonth'));
if (!($adb->num_rows($result))) {$adb->pquery("ALTER TABLE com_jo_workflows ADD schdayofmonth VARCHAR(100)", array());}
$result = $adb->pquery("show columns from com_jo_workflows like ?", array('schdayofweek'));
if (!($adb->num_rows($result))) {$adb->pquery("ALTER TABLE com_jo_workflows ADD schdayofweek VARCHAR(100)", array());}
$result = $adb->pquery("show columns from com_jo_workflows like ?", array('schannualdates'));
if (!($adb->num_rows($result))) {$adb->pquery("ALTER TABLE com_jo_workflows ADD schannualdates VARCHAR(100)", array());}
$result = $adb->pquery("show columns from com_jo_workflows like ?", array('nexttrigger_time'));
if (!($adb->num_rows($result))) {$adb->pquery("ALTER TABLE com_jo_workflows ADD nexttrigger_time DATETIME", array());}

if(!defined('INSTALLATION_MODE')) {
	Migration_Index_View::ExecuteQuery("CREATE TABLE IF NOT EXISTS com_jo_workflow_tasktypes (
					id int(11) NOT NULL,
...
Migration_Index_View::ExecuteQuery("CREATE TABLE IF NOT EXISTS jo_shorturls (
...
$moduleInstance = Head_Module::getInstance('Potentials');
$block = Head_Block::getInstance('LBL_OPPORTUNITY_INFORMATION', $moduleInstance);

$forecast_field = new Head_Field();
$forecast_field->name = 'forecast_amount';
$forecast_field->label = 'Forecast Amount';
$forecast_field->table ='jo_potential';
...
Migration_Index_View::ExecuteQuery("UPDATE jo_role SET rolename=? WHERE rolename=? and roleid=?", array('Sales Person', 'Sales Man', 'H5'));

if(!defined('INSTALLATION_MODE')) {
	$picklistResult = $adb->pquery('SELECT distinct fieldname FROM jo_field WHERE uitype IN (15,33)', array());
	$numRows = $adb->num_rows($picklistResult);
	for($i=0; $i<$numRows; $i++) {
		$fieldName = $adb->query_result($picklistResult,$i,'fieldname');
		$query = 'ALTER TABLE jo_'.$fieldName.' ADD COLUMN sortorderid INT(1)';
		Migration_Index_View::ExecuteQuery($query, array());
	}
}

$invoiceModuleInstance = Head_Module::getInstance('Invoice');
$fieldInstance = Head_Field::getInstance('invoicestatus', $invoiceModuleInstance);
...
$selectcolumns = Array(
	Array('jo_contactdetails:lastname:Contacts_Last_Name:lastname:V',
		'jo_contactdetails:email:Contacts_Email:email:E',
		'jo_activity:subject:Emails_Subject:subject:V',
		'jo_email_track:access_count:Emails_Access_Count:access_count:V'),
	Array('jo_account:accountname:Accounts_Account_Name:accountname:V',
		'jo_account:phone:Accounts_Phone:phone:V',
		'jo_account:email1:Accounts_Email:email1:E',
		'jo_activity:subject:Emails_Subject:subject:V',
		'jo_email_track:access_count:Emails_Access_Count:access_count:V'),
	Array('jo_leaddetails:lastname:Leads_Last_Name:lastname:V',
		'jo_leaddetails:company:Leads_Company:company:V',
		'jo_leaddetails:email:Leads_Email:email:E',
		'jo_activity:subject:Emails_Subject:subject:V',
		'jo_email_track:access_count:Emails_Access_Count:access_count:V'),
	Array('jo_vendor:vendorname:Vendors_Vendor_Name:vendorname:V',
		'jo_vendor:glacct:Vendors_GL_Account:glacct:V',
		'jo_vendor:email:Vendors_Email:email:E',
		'jo_activity:subject:Emails_Subject:subject:V',
		'jo_email_track:access_count:Emails_Access_Count:access_count:V'),
);
...
$advfilters = Array(
	Array(
		Array(
			'columnname' => 'jo_email_track:access_count:Emails_Access_Count:access_count:V',
			'comparator' => 'n',
			'value' => ''
		)
	)
...
foreach ($reports as $key => $report) {
	...
	$sql = 'SELECT MAX(folderid) AS count FROM jo_reportfolder';
...
Migration_Index_View::ExecuteQuery("UPDATE jo_report SET sharingtype='Public'", array());
//End.

//Currency Decimal places handling
Migration_Index_View::ExecuteQuery("ALTER TABLE jo_account MODIFY COLUMN annualrevenue decimal(25,5)", array());
Migration_Index_View::ExecuteQuery("ALTER TABLE jo_leaddetails MODIFY COLUMN annualrevenue decimal(25,5)", array());
Migration_Index_View::ExecuteQuery("UPDATE jo_field SET typeofdata='N~O' WHERE fieldlabel='Annual Revenue' and typeofdata='I~O'",array());

Migration_Index_View::ExecuteQuery("ALTER TABLE jo_currency_info MODIFY COLUMN conversion_rate decimal(12,5)", array());
Migration_Index_View::ExecuteQuery("ALTER TABLE jo_productcurrencyrel MODIFY COLUMN actual_price decimal(28,5)", array());
Migration_Index_View::ExecuteQuery("ALTER TABLE jo_productcurrencyrel MODIFY COLUMN converted_price decimal(28,5)", array());
Migration_Index_View::ExecuteQuery("ALTER TABLE jo_pricebookproductrel MODIFY COLUMN listprice decimal(27,5)", array());
Migration_Index_View::ExecuteQuery("ALTER TABLE jo_inventoryproductrel MODIFY COLUMN listprice decimal(27,5)", array());
Migration_Index_View::ExecuteQuery("ALTER TABLE jo_inventoryproductrel MODIFY COLUMN discount_amount decimal(27,5)", array());

$currencyField = new CurrencyField($value);
$result = $adb->pquery("SELECT fieldname,tablename,columnname FROM jo_field WHERE uitype IN (?,?)",array('71','72'));
...
$currencyBlock = Head_Block::getInstance('LBL_CURRENCY_CONFIGURATION', $moduleInstance);

$currency_decimals_field = new Head_Field();
$currency_decimals_field->name = 'no_of_currency_decimals';
$currency_decimals_field->label = 'Number Of Currency Decimals';
$currency_decimals_field->table ='jo_users';
...
for($i = 0; $i < count($inventoryModules); $i++) {
	...
	foreach ($actions as $actionName) {
		Head_Access::updateTool($moduleInstance, $actionName, true, '');
	}
}
...
$fieldTypeId = $adb->getUniqueID("jo_ws_entity_fieldtype");
Migration_Index_View::ExecuteQuery("INSERT INTO jo_ws_entity_fieldtype(fieldtypeid,table_name, field_name,fieldtype) VALUES (?,?,?,?);", array($fieldTypeId, 'jo_inventoryproductrel', 'productid',"reference"));
Migration_Index_View::ExecuteQuery("INSERT INTO jo_ws_entity_referencetype(fieldtypeid,type) VALUES (?,?)",array($fieldTypeId,'Products'));

$fieldTypeId = $adb->getUniqueID("jo_ws_entity_fieldtype");
Migration_Index_View::ExecuteQuery("INSERT INTO jo_ws_entity_fieldtype(fieldtypeid,table_name, field_name,fieldtype) VALUES (?,?,?,?);", array($fieldTypeId, 'jo_inventoryproductrel', 'id',"reference"));
Migration_Index_View::ExecuteQuery("INSERT INTO jo_ws_entity_referencetype(fieldtypeid,type) VALUES (?,?)",array($fieldTypeId,'Invoice'));
Migration_Index_View::ExecuteQuery("INSERT INTO jo_ws_entity_referencetype(fieldtypeid,type) VALUES (?,?)",array($fieldTypeId,'SalesOrder'));
Migration_Index_View::ExecuteQuery("INSERT INTO jo_ws_entity_referencetype(fieldtypeid,type) VALUES (?,?)",array($fieldTypeId,'PurchaseOrder'));
Migration_Index_View::ExecuteQuery("INSERT INTO jo_ws_entity_referencetype(fieldtypeid,type) VALUES (?,?)",array($fieldTypeId,'Quotes'));

$fieldTypeId = $adb->getUniqueID("jo_ws_entity_fieldtype");
Migration_Index_View::ExecuteQuery("INSERT INTO jo_ws_entity_fieldtype(fieldtypeid,table_name,field_name,fieldtype) VALUES (?,?,?,?);", array($fieldTypeId,'jo_inventoryproductrel', 'incrementondel',"autogenerated"));
...
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET block=? where tabid=? and block=?;',
...
Migration_Index_View::ExecuteQuery("INSERT INTO jo_ws_entity_tables(webservice_entity_id,table_name) VALUES (?,?)",array($webserviceObject->getEntityId(),'jo_producttaxrel'));

$fieldTypeId = $adb->getUniqueID("jo_ws_entity_fieldtype");

Migration_Index_View::ExecuteQuery("INSERT INTO jo_ws_entity_fieldtype(fieldtypeid,table_name,field_name,fieldtype) VALUES (?,?,?,?);", array($fieldTypeId,'jo_producttaxrel', 'productid',"reference"));
Migration_Index_View::ExecuteQuery("INSERT INTO jo_ws_entity_referencetype(fieldtypeid,type) VALUES (?,?)",array($fieldTypeId,'Products'));

$fieldTypeId = $adb->getUniqueID("jo_ws_entity_fieldtype");
Migration_Index_View::ExecuteQuery("INSERT INTO jo_ws_entity_fieldtype(fieldtypeid,table_name,field_name,fieldtype) VALUES (?,?,?,?);", array($fieldTypeId,'jo_producttaxrel', 'taxid',"reference"));
Migration_Index_View::ExecuteQuery("INSERT INTO jo_ws_entity_referencetype(fieldtypeid,type) VALUES (?,?)",array($fieldTypeId,'Tax'));

//--
//Changed Columns Display in List view of Leads
$leadsFirstName = 'jo_leaddetails:firstname:firstname:Leads_First_Name:V';
$leadsLastName = 'jo_leaddetails:lastname:lastname:Leads_Last_Name:V';
Migration_Index_View::ExecuteQuery("UPDATE jo_cvcolumnlist SET columnname=? WHERE cvid=? AND columnindex=?", array($leadsFirstName, '1', '1'));
Migration_Index_View::ExecuteQuery("UPDATE jo_cvcolumnlist SET columnname=? WHERE cvid=? AND columnindex=?", array($leadsLastName, '1', '2'));

//Changed the Currency Symbol of Moroccan, Dirham to DH
Migration_Index_View::ExecuteQuery("UPDATE jo_currencies SET currency_symbol=? WHERE currency_name=? AND currency_code=?", array('DH', 'Moroccan, Dirham', 'MAD'));

//Changing picklist values for sales stage of opportunities
Migration_Index_View::ExecuteQuery("UPDATE jo_sales_stage SET sales_stage=? WHERE sales_stage=?", array('Proposal or Price Quote', 'Proposal/Price Quote'));
Migration_Index_View::ExecuteQuery("UPDATE jo_sales_stage SET sales_stage=? WHERE sales_stage=?", array('Negotiation or Review', 'Negotiation/Review'));

//Updating the new picklist values of sales stage in opportunities for migration instances
Migration_Index_View::ExecuteQuery("UPDATE jo_potential SET sales_stage=? WHERE sales_stage=?", array('Proposal or Price Quote', 'Proposal/Price Quote'));
Migration_Index_View::ExecuteQuery("UPDATE jo_potential SET sales_stage=? WHERE sales_stage=?", array('Negotiation or Review', 'Negotiation/Review'));

//Updating Sales Stage History in opportunities related list for migration instances
Migration_Index_View::ExecuteQuery("UPDATE jo_potstagehistory SET stage=? WHERE stage=?", array('Proposal or Price Quote', 'Proposal/Price Quote'));
Migration_Index_View::ExecuteQuery("UPDATE jo_potstagehistory SET stage=? WHERE stage=?", array('Negotiation or Review', 'Negotiation/Review'));

//Updating the sales stage picklist values of opportunities in picklist dependency setup for migration instances
Migration_Index_View::ExecuteQuery("UPDATE jo_picklist_dependency SET sourcevalue=? WHERE sourcevalue=?", array('Proposal or Price Quote', 'Proposal/Price Quote'));
Migration_Index_View::ExecuteQuery("UPDATE jo_picklist_dependency SET sourcevalue=? WHERE sourcevalue=?", array('Negotiation or Review', 'Negotiation/Review'));

//Internationalized the description for webforms
Migration_Index_View::ExecuteQuery("UPDATE jo_settings_field SET description=? WHERE description=?", array('LBL_WEBFORMS_DESCRIPTION', 'Allows you to manage Webforms'));

Migration_Index_View::ExecuteQuery('CREATE TABLE IF NOT EXISTS jo_crmsetup(userid INT(11) NOT NULL, setup_status INT(2))', array());
if (!defined('INSTALLATION_MODE')) {
	Migration_Index_View::ExecuteQuery('INSERT INTO jo_crmsetup(userid, setup_status) SELECT id, 1 FROM jo_users', array());
}

$discountResult = Migration_Index_View::ExecuteQuery('SELECT * FROM jo_selectcolumn WHERE columnname LIKE "jo_inventoryproductrel:discount:%" ORDER BY columnindex', array());
...
for ($i=0; $i<$num_rows; $i++) {
    ...
    $updateQuery = 'UPDATE jo_selectcolumn SET columnname = ? WHERE columnindex = ? and queryid = ?';
    $updateParams = array($updatedColumnName, $columnIndex,$queryId);

	Migration_Index_View::ExecuteQuery($updateQuery, $updateParams);
}

Migration_Index_View::ExecuteQuery('INSERT INTO jo_ws_referencetype VALUES (?,?)', array(31,'Campaigns'));

$moduleInstance = Head_Module::getInstance('Users');
$currencyBlock = Head_Block::getInstance('LBL_CURRENCY_CONFIGURATION', $moduleInstance);
$truncateTrailingZeros = new Head_Field();

$truncateTrailingZeros->name = 'truncate_trailing_zeros';
$truncateTrailingZeros->label = 'Truncate Trailing Zeros';
$truncateTrailingZeros->table ='jo_users';
...
Migration_Index_View::ExecuteQuery("ALTER TABLE jo_productcurrencyrel MODIFY COLUMN actual_price decimal(28,8)", array());
Migration_Index_View::ExecuteQuery("ALTER TABLE jo_productcurrencyrel MODIFY COLUMN converted_price decimal(28,8)", array());
Migration_Index_View::ExecuteQuery("ALTER TABLE jo_pricebookproductrel MODIFY COLUMN listprice decimal(27,8)", array());
Migration_Index_View::ExecuteQuery("ALTER TABLE jo_inventoryproductrel MODIFY COLUMN listprice decimal(27,8)", array());
Migration_Index_View::ExecuteQuery("ALTER TABLE jo_inventoryproductrel MODIFY COLUMN discount_amount decimal(27,8)", array());

$currencyField = new CurrencyField($value);
$result = Migration_Index_View::ExecuteQuery("SELECT tablename,columnname FROM jo_field WHERE uitype IN (?,?)",array('71','72'));
$count = $adb->num_rows($result);
for($i=0;$i<$count;$i++) {
	$tableName = $adb->query_result($result,$i,'tablename');
	$columnName = $adb->query_result($result,$i,'columnname');
...
Migration_Index_View::ExecuteQuery('DELETE FROM jo_no_of_currency_decimals WHERE no_of_currency_decimalsid=?', array(1));

Migration_Index_View::ExecuteQuery('UPDATE jo_field SET uitype=?, typeofdata=? WHERE fieldname=?',array(71, 'N~O', 'listprice'));

Migration_Index_View::ExecuteQuery('UPDATE jo_field SET typeofdata=? WHERE fieldname=?',array('N~O', 'quantity'));

//--
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET typeofdata=?, uitype =?, fieldlabel=? WHERE fieldname =? and tablename=?', array('N~O', 71, 'Discount', 'discount_amount', 'jo_inventoryproductrel'));

//deleting default workflows
Migration_Index_View::ExecuteQuery("delete from com_jo_workflowtasks where task_id=?", array(11));
Migration_Index_View::ExecuteQuery("delete from com_jo_workflowtasks where task_id=?", array(12));
...
$allTabIdResult = Migration_Index_View::ExecuteQuery('SELECT tabid, name FROM jo_tab', array());
...
$moduleInstance = Head_Module::getInstance('ProjectTask');
$blockInstance = Head_Block::getInstance('LBL_PROJECT_TASK_INFORMATION', $moduleInstance);
...
Head_Utils::CreateTable('jo_module_dashboard_widgets', '(id INT(19) NOT NULL AUTO_INCREMENT, linkid INT(19), userid INT(19), filterid INT(19),
				title VARCHAR(100), data VARCHAR(500) DEFAULT "[]", PRIMARY KEY(id))');
$potentials = Head_Module::getInstance('Potentials');
...
$accounts = Head_Module::getInstance('Accounts');
...
$contacts = Head_Module::getInstance('Contacts');
...
$leads = Head_Module::getInstance('Leads');
...
$helpDesk = Head_Module::getInstance('HelpDesk');
...
$home = Head_Module::getInstance('Home');
...
Migration_Index_View::ExecuteQuery('UPDATE jo_blocks SET blocklabel ="LBL_DESCRIPTION_INFORMATION" WHERE blockid=20',array());
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET displaytype=1 WHERE fieldname="location" AND tabid = ?', array($calendarTabId));
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET displaytype=1 WHERE fieldname="visibility" AND tabid = ?', array($eventTabId));

$eventBlockId = getBlockId($eventTabId, 'LBL_EVENT_INFORMATION');
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET block = ? WHERE block = 41', array($eventBlockId));
Migration_Index_View::ExecuteQuery('UPDATE jo_blocks SET blocklabel = "LBL_REMINDER_INFORMATION", show_title = 0 WHERE blockid = 40',array());
Migration_Index_View::ExecuteQuery('UPDATE jo_blocks SET blocklabel = "LBL_DESCRIPTION_INFORMATION", show_title = 0 WHERE blockid = 41',array());
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET block = 41 WHERE fieldname = "description" AND tabid = ?',array($eventTabId));
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET block = ? WHERE fieldname = "contact_id" AND tabid = ?', array($eventBlockId, $eventTabId));
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET displaytype = 3 WHERE fieldname = ? AND tabid = ?', array('notime', $eventTabId));
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET displaytype = 3 WHERE fieldname = ? AND tabid = ?', array('duration_hours', $eventTabId));
...
Migration_Index_View::ExecuteQuery('UPDATE jo_relatedlists SET actions=? WHERE tabid in(?,?) and related_tabid in (?,?,?)',
...
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET presence = 1 WHERE tabid = ? AND fieldname = ?', array($helpDeskTabId, 'comments'));
//$faqTabId = getTabid('Faq');
//Migration_Index_View::ExecuteQuery('UPDATE jo_field SET presence = 1 WHERE tabid = ? AND fieldname = ?', array($faqTabId, 'comments'));

Migration_Index_View::ExecuteQuery('UPDATE jo_users SET truncate_trailing_zeros = ?', array(1));

//deleted the id column from the All filter
Migration_Index_View::ExecuteQuery("DELETE FROM jo_cvcolumnlist WHERE cvid IN
			(SELECT cvid FROM jo_customview WHERE viewname='All' AND entitytype NOT IN
...
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_modtracker_basic ADD INDEX crmidx (crmid)', array());
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_modtracker_basic ADD INDEX idx (id)', array());
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_modtracker_detail ADD INDEX idx (id)', array());

// Ends

require_once 'modules/com_jo_workflow/VTEntityMethodManager.inc';
...
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_links MODIFY column linktype VARCHAR(50)', array());
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_links MODIFY column linklabel VARCHAR(50)', array());
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_links MODIFY column handler_class VARCHAR(50)', array());
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_links MODIFY column handler VARCHAR(50)', array());

//--
//Add ModComments to HelpDesk and Faq module

$moduleInstance = Head_Module::getInstance('ModComments');
$customer = Head_Field::getInstance('customer', $moduleInstance);
if (!$customer) {
	$customer = new Head_Field();
	$customer->name = 'customer';
	$customer->label = 'Customer';
	$customer->uitype = '10';
	$customer->displaytype = '3';
	$blockInstance = Head_Block::getInstance('LBL_MODCOMMENTS_INFORMATION', $moduleInstance);
	$blockInstance->addField($customer);
	$customer->setRelatedModules(array('Contacts'));
}

require_once 'modules/ModComments/ModComments.php';
ModComments::addWidgetTo(array("HelpDesk"));
...
$pageCount = 0;
do {
	$ticketComments = Migration_Index_View::ExecuteQuery(sprintf('SELECT * FROM jo_ticketcomments ORDER BY commentid ASC LIMIT %s,%s', $pageCount*1000, 1000),  array());
	...
	for($i=0; $i<$rows; $i++) {
		...
		Migration_Index_View::ExecuteQuery('UPDATE jo_crmentity SET modifiedtime = ?, smcreatorid = ?, modifiedby = ? WHERE crmid = ?',
...
$pageCount = 0;
/*do {
	$faqComments = Migration_Index_View::ExecuteQuery(sprintf('SELECT * FROM jo_faqcomments ORDER BY commentid ASC LIMIT %s, %s', $pageCount*1000, 1000), array());
...
// Added label column in jo_crmentity table for easier lookup - Also added Event handler to update the label on save of a record
Migration_Index_View::ExecuteQuery("ALTER TABLE jo_crmentity ADD COLUMN label varchar(255)", array());

// To avoid infinite-loop if we not able fix label for non-entity/special modules.
$lastMaxCRMId = 0;
do {
	$rs = $adb->pquery("SELECT crmid,setype FROM jo_crmentity INNER JOIN jo_tab ON jo_tab.name=jo_crmentity.setype WHERE label IS NULL AND crmid > ? LIMIT 500", array($lastMaxCRMId));
	if (!$adb->num_rows($rs)) {
		break;
	}
...
Migration_Index_View::ExecuteQuery('CREATE INDEX jo_crmentity_labelidx ON jo_crmentity(label)', array());

$homeModule = Head_Module::getInstance('Home');
Head_Event::register($homeModule, 'vtiger.entity.aftersave', 'Head_RecordLabelUpdater_Handler', 'modules/Head/RecordLabelUpdater.php');



$moduleInstance = Head_Module::getInstance('Potentials');
$filter = Head_Filter::getInstance('All', $moduleInstance);
$fieldInstance = Head_Field::getInstance('amount', $moduleInstance);
...
if(file_exists('modules/ModTracker/ModTrackerUtils.php')) {
	...
	$modules = $adb->pquery('SELECT * FROM jo_tab WHERE isentitytype = 1', array());
...
Migration_Index_View::ExecuteQuery('UPDATE jo_blocks SET sequence = sequence+1 WHERE tabid=? AND sequence >= 3',
											array($tabId));

// Create Recurrence Information block
$recurrenceBlock = new Head_Block();
...
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET block=? WHERE fieldname=? and tabid=?', array($blockId, 'recurringtype', $tabId));

// Update/Increment the sequence for the succeeding blocks of Users module, with starting sequence 2
$moduleInstance = Head_Module::getInstance('Users');
$tabId = getTabid('Users');
Migration_Index_View::ExecuteQuery('UPDATE jo_blocks SET sequence = sequence+1 WHERE tabid=? AND sequence >= 2', array($tabId));

// Create Calendar Settings block
$calendarSettings = new Head_Block();
$calendarSettings->label = 'LBL_CALENDAR_SETTINGS';
$calendarSettings->sequence = 2;
$moduleInstance->addBlock($calendarSettings);

$calendarSettings = Head_Block::getInstance('LBL_CALENDAR_SETTINGS', $moduleInstance);

$dayOfTheWeek = new Head_Field();
$dayOfTheWeek->name = 'dayoftheweek';
$dayOfTheWeek->label = 'Starting Day of the week';
$dayOfTheWeek->table ='jo_users';
...
$sql = 'UPDATE jo_field SET block = ? , displaytype = ? WHERE tabid = ? AND tablename = ? AND columnname in (?,?,?,?,?,?)';
Migration_Index_View::ExecuteQuery($sql, array($blockId, 1, $tabId, 'jo_users', 'time_zone','activity_view','reminder_interval','date_format','start_hour', 'hour_format'));

Migration_Index_View::ExecuteQuery('UPDATE jo_field SET uitype = ? WHERE tabid = ? AND tablename = ? AND columnname in (?,?)',
		array(16, $tabId, 'jo_users', 'hour_format', 'start_hour'));

$fieldid = getFieldid($tabId, 'hour_format');
$hour_format = Head_Field::getInstance($fieldid, $moduleInstance);
$hour_format->setPicklistValues(array(12,24));

$fieldid = getFieldid($tabId, 'start_hour');
$start_hour = Head_Field::getInstance($fieldid, $moduleInstance);
...
Migration_Index_View::ExecuteQuery('UPDATE jo_users SET hour_format = ? WHERE hour_format = ? OR hour_format = ?', array(12, 'am/pm', ''));

//add user default values
Migration_Index_View::ExecuteQuery('UPDATE jo_users SET dayoftheweek = ?, callduration = ?, othereventduration = ?, start_hour = ? ', array('Sunday', 5, 5, '00:00'));

$moduleInstance = Head_Module::getInstance('Events');
$tabId = getTabid('Events');

// Update/Increment the sequence for the succeeding blocks of Events module, with starting sequence 4
Migration_Index_View::ExecuteQuery('UPDATE jo_blocks SET sequence = sequence+1 WHERE tabid=? AND sequence >= 4', array($tabId));
...
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET block=? WHERE fieldname IN (?,?) and tabid=?', array($blockId, 'contact_id','parent_id', $tabId));
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET displaytype=1 WHERE fieldname=? and tabid=?',array('recurringtype',$tabId));

// END 2012.12.02

// //////////////////////////////////////////////
$inventoryModules = array(
    'Invoice' => array('LBL_INVOICE_INFORMATION', 'jo_invoice', 'invoiceid'),
    'SalesOrder' => array('LBL_SO_INFORMATION', 'jo_salesorder', 'salesorderid'),
    'PurchaseOrder' => array('LBL_PO_INFORMATION', 'jo_purchaseorder', 'purchaseorderid'),
    'Quotes' => array('LBL_QUOTE_INFORMATION', 'jo_quotes', 'quoteid')
);

foreach ($inventoryModules as $module => $details) {
    $tableName = $details[1];
    $moduleInstance = Head_Module::getInstance($module);
    $block = Head_Block::getInstance($details[0], $moduleInstance);

    $preTaxTotalField = new Head_Field();
...
foreach ($allUsers as $id => $name) {
    $query = 'select sharedid from jo_sharedcalendar where userid=?';
    $result = $adb->pquery($query, array($id));
	$count = $adb->num_rows($result);
    if($count > 0){
		Migration_Index_View::ExecuteQuery('UPDATE jo_users SET calendarsharedtype = ? WHERE id = ?', array('selectedusers', $id));
    }else{
		Migration_Index_View::ExecuteQuery('UPDATE jo_users SET calendarsharedtype = ? WHERE id = ? ', array('public', $id));
...
$moduleInstance = Head_Module::getInstance('Users');
$moreInfoBlock = Head_Block::getInstance('LBL_MORE_INFORMATION', $moduleInstance);

$viewField = new Head_Field();
$viewField->name = 'default_record_view';
$viewField->label = 'Default Record View';
$viewField->table ='jo_users';
...
Migration_Index_View::ExecuteQuery('UPDATE jo_users SET default_record_view = ?', array('Summary'));

$InvoiceInstance = Head_Module::getInstance('Invoice');
Head_Event::register($InvoiceInstance, 'vtiger.entity.aftersave', 'InvoiceHandler', 'modules/Invoice/InvoiceHandler.php');

$POInstance = Head_Module::getInstance('PurchaseOrder');
Head_Event::register($POInstance, 'vtiger.entity.aftersave', 'PurchaseOrderHandler', 'modules/PurchaseOrder/PurchaseOrderHandler.php');

$InvoiceBlockInstance = Head_Block::getInstance('LBL_INVOICE_INFORMATION', $InvoiceInstance);
$field1 = Head_Field::getInstance('received', $InvoiceInstance);
if (!$field1) {
    $field1 = new Head_Field();
    $field1->name = 'received';
    $field1->label = 'Received';
    $field1->table = 'jo_invoice';
    $field1->uitype = 72;
...
$field2 = Head_Field::getInstance('balance', $InvoiceInstance);
if (!$field2) {
...
$POBlockInstance = Head_Block::getInstance('LBL_PO_INFORMATION', $POInstance);
$field3 = Head_Field::getInstance('paid', $POInstance);
if (!$field3) {
    $field3 = new Head_Field();
    $field3->name = 'paid';
    $field3->label = 'Paid';
    $field3->table = 'jo_purchaseorder';
    $field3->uitype = 72;
...
$field4 = Head_Field::getInstance('balance', $POInstance);
if (!$field4) {
    $field4 = new Head_Field();
    $field4->name = 'balance';
    $field4->label = 'Balance';
    $field4->table = 'jo_purchaseorder';
    $field4->uitype = 72;
...
$sqltimelogTable = "CREATE TABLE jo_sqltimelog ( id integer, type VARCHAR(10),
...
Migration_Index_View::ExecuteQuery("UPDATE jo_relatedlists SET actions = ? WHERE tabid = ? AND related_tabid IN (?, ?)",
	array('ADD', getTabid('Project'), getTabid('ProjectTask'), getTabid('ProjectMilestone')));

Migration_Index_View::ExecuteQuery("UPDATE jo_field SET typeofdata = ? WHERE columnname = ? AND tablename = ?", array("V~O", "company", "jo_leaddetails"));

if(Head_Utils::CheckTable('jo_cron_task')) {
	Migration_Index_View::ExecuteQuery('ALTER TABLE jo_cron_task MODIFY COLUMN laststart INT(11) UNSIGNED',Array());
	Migration_Index_View::ExecuteQuery('ALTER TABLE jo_cron_task MODIFY COLUMN lastend INT(11) UNSIGNED',Array());
}

if(Head_Utils::CheckTable('jo_cron_log')) {
	Migration_Index_View::ExecuteQuery('ALTER TABLE jo_cron_log MODIFY COLUMN start INT(11) UNSIGNED',Array());
   	Migration_Index_View::ExecuteQuery('ALTER TABLE jo_cron_log MODIFY COLUMN end INT(11) UNSIGNED',Array());
}

require_once 'vtlib/Head/Cron.php';
Head_Cron::deregister('ScheduleReports');
// END 2013.02.18

// Start 2013.03.19
// Mail Converter schema changes
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_mailscanner ADD COLUMN timezone VARCHAR(10) default NULL', array());
Migration_Index_View::ExecuteQuery('UPDATE jo_mailscanner SET timezone=? WHERE server LIKE ? AND timezone IS NULL', array('-8:00', '%.gmail.com'));

Migration_Index_View::ExecuteQuery('UPDATE jo_report SET state=?', array('CUSTOM'));

Migration_Index_View::ExecuteQuery("ALTER TABLE jo_relcriteria MODIFY value VARCHAR(512)", array());
Migration_Index_View::ExecuteQuery("ALTER TABLE jo_cvadvfilter MODIFY value VARCHAR(512)", array());
// End 2013.03.19

// Start 2013.04.23
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_sqltimelog MODIFY started DECIMAL(20,6)', array());
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_sqltimelog MODIFY ended DECIMAL(20,6)', array());

//added Assests tab in contact
//$assetsModuleInstance = Head_Module::getInstance('Assets');
//$contactModule = Head_Module::getInstance('Contacts');
//$contactModule->setRelatedList($assetsModuleInstance, '', false, 'get_dependents_list');
// End 2013.04.23

// Start 2013.04.30
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_users MODIFY signature TEXT', array());
//Adding column to store the state of short cut settings fields
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_settings_field ADD COLUMN pinned int(1) DEFAULT 0',array());

$defaultPinnedFields = array('LBL_USERS','LBL_LIST_WORKFLOWS','VTLIB_LBL_MODULE_MANAGER','LBL_PICKLIST_EDITOR');
$defaultPinnedSettingFieldQuery = 'UPDATE jo_settings_field SET pinned=1 WHERE name IN ('.generateQuestionMarks($defaultPinnedFields).')';
Migration_Index_View::ExecuteQuery($defaultPinnedSettingFieldQuery,$defaultPinnedFields);

Migration_Index_View::ExecuteQuery('ALTER TABLE jo_profile ADD COLUMN directly_related_to_role int(1) DEFAULT 0',array());

$blockId = getSettingsBlockId('LBL_STUDIO');
$result = $adb->pquery('SELECT max(sequence) as maxSequence FROM jo_settings_field WHERE blockid=?', array($blockId));
$sequence = 0;
...
$query = "INSERT INTO jo_settings_field (fieldid, blockid, name, iconpath, description, " .
...
Migration_Index_View::ExecuteQuery('UPDATE jo_role SET rolename = ? WHERE rolename = ? AND depth = ?', array('Organization', 'Organisation', 0));


//Create a new table to support custom fields in Documents module
$adb->query("CREATE TABLE IF NOT EXISTS jo_notescf (notesid INT(19), FOREIGN KEY fk_1_jo_notescf(notesid) REFERENCES jo_notes(notesid) ON DELETE CASCADE);");

if(!defined('INSTALLATION_MODE')) {
	Migration_Index_View::ExecuteQuery('ALTER TABLE jo_salutationtype ADD COLUMN sortorderid INT(1)', array());
}
...
foreach ($summaryFields as $moduleName => $fieldsList) {
	$updateQuery = 'UPDATE jo_field SET summaryfield = 1
...
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET defaultvalue=? WHERE tablename=? AND fieldname= ?', array('Active', 'jo_users', 'status'));
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET defaultvalue=? WHERE tablename=? AND fieldname= ?', array('12', 'jo_users', 'hour_format'));
...
$query = 'SELECT profileid FROM jo_profile';
$result = $adb->pquery($query, array());

for($i=0; $i<$adb->num_rows($result); $i++) {
	$profileId = $adb->query_result($result, $i, 'profileid');
	$sql = 'SELECT fieldid FROM jo_profile2field WHERE profileid = ? AND tabid = ?';
	...
	foreach ($userAccessbleFields as $fieldId => $fieldName) {
		if(!in_array($fieldId, $profile2Fields)){
			$insertQuery = 'INSERT INTO jo_profile2field(profileid,tabid,fieldid,visible,readonly) VALUES(?,?,?,?,?)';
			Migration_Index_View::ExecuteQuery($insertQuery, array($profileId,$tabId,$fieldId,0,0));
		}
	}
}

//Added user field in jo_def_org_field table
$sql = 'SELECT fieldid FROM jo_def_org_field WHERE tabid = ?';
...
foreach ($userAccessbleFields as $fieldId => $fieldName) {
	if(!in_array($fieldId, $def_org_fields)){
		$insertQuery = 'INSERT INTO jo_def_org_field(tabid,fieldid,visible,readonly) VALUES(?,?,?,?)';
...
$sql = 'SELECT fieldname FROM jo_field WHERE uitype IN(?,?,?,?)';
$result = $adb->pquery($sql, array(15,16,33,55));
$num_rows = $adb->num_rows($result);
for($i=0; $i<$num_rows; $i++){
	$fieldName = $adb->query_result($result, $i, 'fieldname');
	$checkTable = $adb->pquery('SHOW TABLES LIKE "jo_'.$fieldName.'"', array());
	if($adb->num_rows($checkTable) > 0) {
		$query = "DELETE FROM jo_$fieldName WHERE $fieldName = ? OR $fieldName = ?";
...
/*$vendorInstance = Head_Module::getInstance('Vendors');
$vendorAssignedToField = Head_Field::getInstance('assigned_user_id', $vendorInstance);
if (!$vendorAssignedToField) {
	$vendorBlock = Head_Block::getInstance('LBL_VENDOR_INFORMATION', $vendorInstance);

	$vendorAssignedToField = new Head_Field();
	$vendorAssignedToField->name = 'assigned_user_id';
	$vendorAssignedToField->label = 'Assigned To';
	$vendorAssignedToField->table = 'jo_crmentity';
	...
	$vendorAllFilter = Head_Filter::getInstance('All', $vendorInstance);
	$vendorAllFilter->addField($vendorAssignedToField, 5);
}

// Allow Sharing access and role-based security for Vendors
Head_Access::deleteSharing($vendorInstance);
Head_Access::initSharing($vendorInstance);
Head_Access::allowSharing($vendorInstance);
Head_Access::setDefaultSharing($vendorInstance);
*/
Head_Module::syncfile();

// Add Email Opt-out for Leads
$leadsInstance = Head_Module::getInstance('Leads');
$leadsOptOutField= Head_Field::getInstance('emailoptout', $leadsInstance);

if (!$leadsOptOutField) {
	$leadsOptOutField = new Head_Field();
	$leadsOptOutField->name = 'emailoptout';
	$leadsOptOutField->label = 'Email Opt Out';
	$leadsOptOutField->table = 'jo_leaddetails';
	...
	$leadsInformationBlock = Head_Block::getInstance('LBL_LEAD_INFORMATION', $leadsInstance);
	$leadsInformationBlock->addField($leadsOptOutField);

	Migration_Index_View::ExecuteQuery('UPDATE jo_leaddetails SET emailoptout=0 WHERE emailoptout IS NULL', array());
}

$module = Head_Module::getInstance('Home');
$module->addLink('DASHBOARDWIDGET', 'Notebook', 'index.php?module=Home&view=ShowWidget&name=Notebook');

Migration_Index_View::ExecuteQuery('ALTER TABLE jo_module_dashboard_widgets MODIFY data TEXT',array());

$linkIdResult = $adb->pquery('SELECT linkid FROM jo_links WHERE jo_links.linklabel="Notebook"', array());
$noteBookLinkId = $adb->query_result($linkIdResult, 0, 'linkid');

$result = $adb->pquery('SELECT jo_homestuff.stufftitle, jo_homestuff.userid, jo_notebook_contents.contents FROM
						jo_homestuff INNER JOIN jo_notebook_contents on jo_notebook_contents.notebookid = jo_homestuff.stuffid
						WHERE jo_homestuff.stufftype = ?', array('Notebook'));
...
for($i=0; $i<$adb->num_rows($result); $i++) {
	...
	$query = 'INSERT INTO jo_module_dashboard_widgets(linkid, userid, filterid, title, data) VALUES(?,?,?,?,?)';
	$params= array($noteBookLinkId,$userId,0,$noteBookTitle,$noteBookContent);
	Migration_Index_View::ExecuteQuery($query, $params);
}

$moduleInstance = Head_Module::getInstance('ModComments');
$modCommentsUserId = Head_Field::getInstance("userid", $moduleInstance);
$modCommentsReasonToEdit = Head_Field::getInstance("reasontoedit", $moduleInstance);

if(!$modCommentsUserId){
	$blockInstance = Head_Block::getInstance('LBL_MODCOMMENTS_INFORMATION', $moduleInstance);
	$userId = new Head_Field();
	$userId->name = 'userid';
	$userId->label = 'UserId';
	$userId->uitype = '10';
...
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_invoice MODIFY balance decimal(25,8)',array());
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_invoice MODIFY received decimal(25,8)',array());
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_purchaseorder MODIFY balance decimal(25,8)',array());
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_purchaseorder MODIFY paid decimal(25,8)',array());

$labels = array('LBL_ADD_NOTE', 'Add Note');
$sql = 'UPDATE jo_links SET handler = ?, handler_class = ?, handler_path = ? WHERE linklabel IN (?, ?)';
Migration_Index_View::ExecuteQuery($sql, array('isLinkPermitted', 'Documents', 'modules/Documents/Documents.php', $labels));

$sql = 'UPDATE jo_links SET handler = ?, handler_class = ?, handler_path = ? WHERE linklabel = ?';
...
Migration_Index_View::ExecuteQuery('DELETE FROM jo_settings_field WHERE name=?', array('EMAILTEMPLATES'));

$tabIdList = array();
$tabIdList[] = getTabid('Invoice');
$tabIdList[] = getTabid('PurchaseOrder');

$query = 'SELECT fieldid FROM jo_field WHERE (fieldname=? or fieldname=? or fieldname=? ) AND tabid IN ('.generateQuestionMarks($tabIdList).')';
$result = $adb->pquery($query, array('received', 'paid', 'balance',$tabIdList));
$numrows = $adb->num_rows($result);

for ($i = 0; $i < $numrows; $i++) {
	$fieldid = $adb->query_result($result, $i, 'fieldid');
	$query = 'Update jo_profile2field set readonly = 0 where fieldid=?';
...
Migration_Index_View::ExecuteQuery("UPDATE jo_leaddetails SET salutation='' WHERE salutation = ?", array('--None--'));

//Update contacts salutation value of none to empty value
Migration_Index_View::ExecuteQuery("UPDATE jo_contactdetails SET salutation='' WHERE salutation = ?", array('--None--'));
// END 2013-06-25

// Start 2013-09-24
Migration_Index_View::ExecuteQuery('UPDATE jo_eventhandlers SET handler_path = ? WHERE handler_class = ?',
				array('modules/Head/handlers/RecordLabelUpdater.php', 'Head_RecordLabelUpdater_Handler'));
...
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_invoice_recurring_info ADD PRIMARY KEY (salesorderid)',array());
...
$numOfRows = $adb->num_rows($result);
require_once 'modules/com_jo_workflow/tasks/VTSendNotificationTask.inc';
for($i=0; $i<$numOfRows; $i++) {
	$tm = new VTTaskManager($adb);
	...
	foreach ($properties as $propertyName => $propertyValue) {
		$sendNotificationTask->$propertyName = str_replace('$(general : (__HeadMeta__) dbtimezone)', '$(general : (__HeadMeta__) usertimezone)', $propertyValue);
	}
	$tm->saveTask($sendNotificationTask);
}

Migration_Index_View::ExecuteQuery('UPDATE jo_field SET masseditable = ? where fieldname = ? and tabid = ?',
			array('1', 'accountname', getTabid('Accounts')));

$result = $adb->pquery('SELECT taxname FROM jo_shippingtaxinfo', array());
$numOfRows = $adb->num_rows($result);
$shippingTaxes = array();
$tabIds = array();
for ($i = 0; $i < $numOfRows; $i++) {
...
$tabIdQuery = 'SELECT tabid FROM jo_tab where name IN ('.generateQuestionMarks($modules).')';
...
$query = 'DELETE FROM jo_field WHERE tabid IN (' . generateQuestionMarks($tabIds) . ') AND fieldname IN (' . generateQuestionMarks($shippingTaxes) . ')';
Migration_Index_View::ExecuteQuery($query, array_merge($tabIds, $shippingTaxes));

$entityModules = Head_Module_Model::getEntityModules();
...
foreach($entityModules as $moduleModel) {
	...
	$updateQuery = 'UPDATE jo_field SET summaryfield=1  where tabid=? and fieldname IN ('.generateQuestionMarks($defaultRelatedFields).')';
	Migration_Index_View::ExecuteQuery($updateQuery,  array_merge(array($tabId), array_values($defaultRelatedFields)));
}

Migration_Index_View::ExecuteQuery('UPDATE jo_currencies SET currency_name = ? where currency_name = ? and currency_code = ?',
		array('Hong Kong, Dollars', 'LvHong Kong, Dollars', 'HKD'));
Migration_Index_View::ExecuteQuery('UPDATE jo_currency_info SET currency_name = ? where currency_name = ?',
		array('Hong Kong, Dollars', 'LvHong Kong, Dollars'));
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET defaultvalue=1 WHERE fieldname = ?',array("filestatus"));
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_role ADD allowassignedrecordsto INT(2) NOT NULL DEFAULT 1', array());
//Migration_Index_View::ExecuteQuery('ALTER TABLE jo_assets MODIFY datesold date', array());
//Migration_Index_View::ExecuteQuery('ALTER TABLE jo_assets MODIFY dateinservice date', array());
//Migration_Index_View::ExecuteQuery('ALTER TABLE jo_assets MODIFY serialnumber varchar(200)', array());
//Migration_Index_View::ExecuteQuery('ALTER TABLE jo_assets MODIFY account int(19)', array());
Migration_Index_View::ExecuteQuery('ALTER TABLE com_jo_workflowtask_queue ADD COLUMN task_contents text', array());
Migration_Index_View::ExecuteQuery('ALTER TABLE com_jo_workflowtask_queue DROP INDEX com_jo_workflowtask_queue_idx',array());
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_mailscanner_ids modify column messageid varchar(512)' , array());
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_mailscanner_ids add index scanner_message_ids_idx (scannerid, messageid)', array());
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_mailscanner_folders add index folderid_idx (folderid)', array());
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_leaddetails add index email_idx (email)', array());
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_contactdetails add index email_idx (email)', array());
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_account add index email_idx (email1, email2)', array());

$moduleInstance = Head_Module::getInstance('Users');
$blockInstance = Head_Block::getInstance('LBL_MORE_INFORMATION',$moduleInstance);
...
Migration_Index_View::ExecuteQuery('UPDATE jo_users SET leftpanelhide = ?', array(0));
$potentialModule = Head_Module::getInstance('Potentials');
$block = Head_Block::getInstance('LBL_OPPORTUNITY_INFORMATION', $potentialModule);

$relatedToField = Head_Field::getInstance('related_to', $potentialModule);
$relatedToField->unsetRelatedModules(array('Contacts'));
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET typeofdata = ? WHERE fieldid = ?', array('V~O', $relatedToField->id));

$contactField = Head_Field::getInstance('contact_id', $potentialModule);
if(!$contactField) {
	$contactField = new Head_Field();
	$contactField->name = 'contact_id';
	$contactField->label = 'Contact Name';
	$contactField->uitype = '10';
	$contactField->column = 'contact_id';
	$contactField->table = 'jo_potential';
	$contactField->columntype = 'INT(19)';
	$block->addField($contactField);
	$contactField->setRelatedModules(array('Contacts'));
	Migration_Index_View::ExecuteQuery('UPDATE jo_field SET summaryfield=1 WHERE fieldid = ?', array($contactField->id));
}

$lastPotentialId = 0;
do {
	$result = $adb->pquery("SELECT potentialid ,related_to FROM jo_potential WHERE potentialid > ? LIMIT 500",
	...
	while ($row = $adb->fetch_array($result)) {
		...
		if($relatedToType != 'Accounts') {
			Migration_Index_View::ExecuteQuery('UPDATE jo_potential SET contact_id = ?, related_to = null WHERE potentialid = ?',
...
$filterResult = $adb->pquery('SELECT * FROM jo_cvadvfilter WHERE columnname like ?',
		array('jo_potential:related_to:related_to:Potentials_Related_%'));
...
for($i=0; $i<$rows; $i++) {
	...
	Migration_Index_View::ExecuteQuery('UPDATE jo_cvadvfilter SET groupid = 2, column_condition = ? WHERE cvid = ?', array('or', $cvid));
	Migration_Index_View::ExecuteQuery('UPDATE jo_cvadvfilter_grouping SET groupid = 2 WHERE cvid = ?', array($cvid));

	Migration_Index_View::ExecuteQuery('INSERT INTO jo_cvadvfilter(cvid,columnindex,columnname,comparator,value,groupid,column_condition)
		VALUES(?,?,?,?,?,?,?)', array($cvid, ++$columnIndex,'jo_potential:contact_id:contact_id:Potentials_Contact_Name:V',
			$comparator, $value, 2, ''));
}
unset($filterResult);

$filterColumnList = $adb->pquery('SELECT * FROM jo_cvcolumnlist WHERE columnname like ?',
		array('jo_potential:related_to:related_to:Potentials_Related_%'));
$filterColumnRows = $adb->num_rows($filterColumnList);
for($j=0; $j<$filterColumnRows; $j++) {
	$cvid = $adb->query_result($filterColumnList, $j, 'cvid');
	$filterResult = $adb->pquery('SELECT MAX(columnindex) AS maxcolumn FROM jo_cvcolumnlist WHERE cvid = ?', array($cvid));
	$maxColumnIndex = $adb->query_result($filterResult, 0, 'maxcolumn');
	Migration_Index_View::ExecuteQuery('INSERT INTO jo_cvcolumnlist(cvid,columnindex,columnname) VALUES (?,?,?)', array($cvid, ++$maxColumnIndex,
		'jo_potential:contact_id:contact_id:Potentials_Contact_Name:V'));
	unset($filterResult);
}
unset($filterColumnList);

$reportColumnResult = $adb->pquery('SELECT * FROM jo_selectcolumn WHERE columnname = ?',
		array('jo_potential:related_to:Potentials_Related_To:related_to:V'));
...
for($k=0; $k<$reportColumnRows; $k++) {
	...
	$filterResult = $adb->pquery('SELECT MAX(columnindex) AS maxcolumn FROM jo_selectcolumn WHERE queryid = ?', array($reportId));
	$maxColumnIndex = $adb->query_result($filterResult, 0, 'maxcolumn');
	Migration_Index_View::ExecuteQuery('INSERT INTO jo_selectcolumn(queryid,columnindex,columnname) VALUES (?,?,?)', array($reportId,
		++$maxColumnIndex, 'jo_potential:contact_id:Potentials_Contact_Name:contact_id:V'));
	unset($filterResult);
}
unset($reportColumnResult);

$filterResult = $adb->pquery('SELECT * FROM jo_relcriteria WHERE columnname = ?',
					array('jo_potential:related_to:Potentials_Related_To:related_to:V'));
$rows = $adb->num_rows($filterResult);
for($i=0; $i<$rows; $i++) {
	...
	Migration_Index_View::ExecuteQuery('UPDATE jo_relcriteria SET groupid = 2, column_condition = ? WHERE queryid = ?', array('or', $reportId));
	Migration_Index_View::ExecuteQuery('UPDATE jo_relcriteria_grouping SET groupid = 2 WHERE queryid = ?', array($reportId));

	Migration_Index_View::ExecuteQuery('INSERT INTO jo_relcriteria(queryid,columnindex,columnname,comparator,value,groupid,column_condition)
		VALUES(?,?,?,?,?,?,?)', array($reportId, ++$columnIndex,'jo_potential:contact_id:Potentials_Contact_Name:contact_id:V',
			$comparator, $value, 2, ''));
}
unset($filterResult);

$ticketsModule = Head_Module::getInstance('HelpDesk');
$ticketsBlock = Head_Block::getInstance('LBL_TICKET_INFORMATION', $ticketsModule);

$relatedToField = Head_Field::getInstance('parent_id', $ticketsModule);
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET uitype = 10 WHERE fieldid = ?', array($relatedToField->id));
$relatedToField->setRelatedModules(array('Accounts'));

$contactField = Head_Field::getInstance('contact_id', $ticketsModule);
if(!$contactField) {
	$contactField = new Head_Field();
	$contactField->name = 'contact_id';
	$contactField->label = 'Contact Name';
	$contactField->table = 'jo_troubletickets';
	$contactField->column = 'contact_id';
	$contactField->columntype = 'INT(19)';
	$contactField->uitype = '10';
...
$lastTicketId = 0;
do {
	$ticketsResult = $adb->pquery("SELECT ticketid ,parent_id FROM jo_troubletickets WHERE ticketid > ?
	...
	while ($row = $adb->fetch_array($ticketsResult)) {
		...
		if($parentType != 'Accounts') {
			Migration_Index_View::ExecuteQuery('UPDATE jo_troubletickets SET contact_id = ?, parent_id = null WHERE ticketid = ?',
...
$ticketFilterResult = $adb->pquery('SELECT * FROM jo_cvadvfilter WHERE columnname like ?',
						array('jo_troubletickets:parent_id:parent_id:HelpDesk_Related%'));
...
for($i=0; $i<$rows; $i++) {
	...
	Migration_Index_View::ExecuteQuery('INSERT INTO jo_cvadvfilter(cvid,columnindex,columnname,comparator,value,groupid,column_condition)
		VALUES(?,?,?,?,?,?,?)', array($cvid, ++$columnIndex,'jo_troubletickets:contact_id:contact_id:HelpDesk_Contact_Name:V',
			$comparator, $value, 2, ''));
}
unset($ticketFilterResult);

$filterColumnList = $adb->pquery('SELECT * FROM jo_cvcolumnlist WHERE columnname like ?',
		array('jo_troubletickets:parent_id:parent_id:HelpDesk_Related_%'));
$filterColumnRows = $adb->num_rows($filterColumnList);
for($j=0; $j<$filterColumnRows; $j++) {
	$cvid = $adb->query_result($filterColumnList, $j, 'cvid');
	$filterResult = $adb->pquery('SELECT MAX(columnindex) AS maxcolumn FROM jo_cvcolumnlist WHERE cvid = ?', array($cvid));
	$maxColumnIndex = $adb->query_result($filterResult, 0, 'maxcolumn');
	Migration_Index_View::ExecuteQuery('INSERT INTO jo_cvcolumnlist(cvid,columnindex,columnname) VALUES (?,?,?)', array($cvid, ++$maxColumnIndex,
		'jo_troubletickets:contact_id:contact_id:HelpDesk_Contact_Name:V'));
	unset($filterResult);
}
unset($filterColumnList);

$reportColumnResult = $adb->pquery('SELECT * FROM jo_selectcolumn WHERE columnname like ?',
		array('jo_troubletickets:parent_id:HelpDesk_Related_To:parent_id%'));
$reportColumnRows = $adb->num_rows($reportColumnResult);
for($k=0; $k<$reportColumnRows; $k++) {
	$reportId = $adb->query_result($reportColumnResult, $k, 'queryid');
	$filterResult = $adb->pquery('SELECT MAX(columnindex) AS maxcolumn FROM jo_selectcolumn WHERE queryid = ?', array($reportId));
	$maxColumnIndex = $adb->query_result($filterResult, 0, 'maxcolumn');
	Migration_Index_View::ExecuteQuery('INSERT INTO jo_selectcolumn(queryid,columnindex,columnname) VALUES (?,?,?)', array($reportId,
		++$maxColumnIndex, 'jo_troubletickets:contact_id:HelpDesk_Contact_Name:contact_id:V'));
	unset($filterResult);
}
unset($reportColumnResult);

$filterResult = $adb->pquery('SELECT * FROM jo_relcriteria WHERE columnname like ?',
					array('jo_troubletickets:parent_id:HelpDesk_Related_To:parent_id%'));
$rows = $adb->num_rows($filterResult);
for($i=0; $i<$rows; $i++) {
	$reportId = $adb->query_result($filterResult, $i, 'queryid');
	...
	Migration_Index_View::ExecuteQuery('INSERT INTO jo_relcriteria(queryid,columnindex,columnname,comparator,value,groupid,column_condition)
		VALUES(?,?,?,?,?,?,?)', array($reportId, ++$columnIndex,'jo_troubletickets:contact_id:HelpDesk_Contact_Name:contact_id:V',
			$comparator, $value, 2, ''));
}
unset($filterResult);
...
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET defaultvalue=? WHERE tablename=? AND fieldname= ? ', array('Active', 'jo_users', 'status'));
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET defaultvalue=? WHERE tablename=? AND fieldname= ? ', array('12', 'jo_users', 'hour_format'));
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET defaultvalue=? WHERE tablename=? AND fieldname= ? ', array('softed', 'jo_users', 'theme'));
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET defaultvalue=? WHERE tablename=? AND fieldname= ? ', array('Monday', 'jo_users', 'dayoftheweek'));
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_shorturls ADD COLUMN onetime int(5)', array());

$checkQuery = 'SELECT 1 FROM jo_currencies  WHERE currency_name=?';
$checkResult = $adb->pquery($checkQuery,array('Iraqi Dinar'));
if($adb->num_rows($checkResult) <= 0) {
	Migration_Index_View::ExecuteQuery('INSERT INTO jo_currencies VALUES ('.$adb->getUniqueID("jo_currencies").',"Iraqi Dinar","IQD","ID")',array());
}
...
$result = $adb->pquery('SELECT sequence,block FROM jo_field WHERE fieldid = ? and tabid = ?', array($relatedToField->id, $potentialTabId));
$relatedToFieldSequence = $adb->query_result($result, 0, 'sequence');
$relatedToFieldBlock = $adb->query_result($result, 0, 'block');

Migration_Index_View::ExecuteQuery('UPDATE jo_field SET sequence = sequence+1 WHERE sequence > ? and tabid = ? and block = ?', array($relatedToFieldSequence, $potentialTabId, $relatedToFieldBlock));
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET sequence = ? WHERE fieldid = ?', array($relatedToFieldSequence+1, $contactField->id));
...
$result = $adb->pquery('SELECT sequence,block FROM jo_field WHERE fieldid = ? and tabid = ?', array($relatedToField->id, $ticketsTabId));
$relatedToFieldSequence = $adb->query_result($result, 0, 'sequence');
$relatedToFieldBlock = $adb->query_result($result, 0, 'block');

Migration_Index_View::ExecuteQuery('UPDATE jo_field SET sequence = sequence+1 WHERE sequence > ? and tabid = ? and block = ?', array($relatedToFieldSequence, $ticketsTabId, $relatedToFieldBlock));
Migration_Index_View::ExecuteQuery('UPDATE jo_field SET sequence = ? WHERE fieldid = ?', array($relatedToFieldSequence+1, $contactField->id));

$checkQuery = 'SELECT 1 FROM jo_currencies  WHERE currency_name=?';
$checkResult = $adb->pquery($checkQuery,array('Maldivian Ruffiya'));
if($adb->num_rows($checkResult) <= 0) {
	Migration_Index_View::ExecuteQuery('INSERT INTO jo_currencies VALUES ('.$adb->getUniqueID("jo_currencies").',"Maldivian Ruffiya","MVR","MVR")',array());
}


$result = $adb->pquery('SELECT count(*) AS count FROM jo_emailtemplates', array());
Migration_Index_View::ExecuteQuery('UPDATE jo_emailtemplates_seq SET id = ?', array(1 + ((int)$adb->query_result($result, 0, 'count'))));

$usersInstance = Head_Module::getInstance('Users');
$blockInstance = Head_Block::getInstance('LBL_MORE_INFORMATION', $usersInstance);
$usersRowHeightField = Head_Field::getInstance('rowheight', $usersInstance);
...
$moduleInstance = Head_Module::getInstance('HelpDesk');
$block = Head_Block::getInstance('LBL_TICKET_INFORMATION', $moduleInstance);
$fromPortal = Head_Field_Model::getInstance('from_portal', $moduleInstance);

if(!$fromPortal){
    $field = new Head_Field();
    $field->name = 'from_portal';
    $field->label = 'From Portal';
    $field->table ='jo_ticketcf';
	...
	foreach ($tasks as $task) {
		...
		switch ($properties['methodName']) {
			case 'CustomerCommentFromPortal' :
				...
				$emailTask->subject = 'Respond to Ticket ID## $(general : (__HeadMeta__) recordId) ## in Customer Portal - URGENT';
			...
			case 'TicketOwnerComments' :
				...
				$emailTask->content = 'Ticket No : $ticket_no<br>
										...
										Thanks<br>$(general : (__HeadMeta__) supportName)';
				$tm->saveTask($emailTask);
...
$result = $adb->pquery('SELECT blockid FROM jo_blocks where tabid = ? AND (blocklabel is NULL OR blocklabel = "")', array(getTabid('Emails')));
$numOfRows = $adb->num_rows($result);

$query = 'UPDATE jo_blocks SET blocklabel = CASE blockid ';
for ($i = 0; $i < $numOfRows; $i++) {
...
$result = $adb->pquery('SELECT 1 FROM jo_currencies WHERE currency_name = ?', array('Ugandan Shilling'));
if(!$adb->num_rows($result)) {
	Migration_Index_View::ExecuteQuery('INSERT INTO jo_currencies (currencyid, currency_name, currency_code, currency_symbol) VALUES(?, ?, ?, ?)',
			array($adb->getUniqueID('jo_currencies'), 'Ugandan Shilling', 'UGX', 'Sh'));
}
$em = new VTEventsManager($adb);
...
Migration_Index_View::ExecuteQuery('ALTER TABLE jo_inventoryproductrel MODIFY comment varchar(500)', array());
