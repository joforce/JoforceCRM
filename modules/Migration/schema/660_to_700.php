...
//		$documentsInstance->setRelatedList(Vtiger_Module::getInstance('Faq'), 'Faq', true);
	...
	Vtiger_Link::addLink($reportTabId, 'LISTVIEWBASIC', 'LBL_CHARTS', 'javascript:Reports_List_Js.addReport("Reports/ChartEdit")', '', '0', $reportModelHandler, $parentLinkId);
...
*/
	...
	Vtiger_Link::addLink(getTabid('Contacts'), 'EXTENSIONLINK', 'Google', 'Contacts/Extension/Google/Index');
	Vtiger_Link::addLink(getTabid('Calendar'), 'EXTENSIONLINK', 'Google', 'Calendar/Extension/Google/Index');
...
//									$moduleTabIds['Assets'],
	...
	$moduleManagerFields = array(	'VTLIB_LBL_MODULE_MANAGER'		=> 'ModuleManager/Settings/List',
									...
									'LBL_CUSTOMIZE_MODENT_NUMBER'	=> 'Vtiger/Settings/CustomRecordNumbering');
	...
	$configurationFields = array(	'LBL_COMPANY_DETAILS'		=> 'Settings/Vtiger/CompanyDetails',
									'LBL_CUSTOMER_PORTAL'		=> 'CustomerPortal/Settings/Index',
									'LBL_CURRENCY_SETTINGS'		=> 'Settings/Currency/List',
									'LBL_MAIL_SERVER_SETTINGS'	=> 'Settings/Vtiger/OutgoingServerDetail',
									'Configuration Editor'		=> 'Vtiger/Settings/ConfigEditorDetail',
	...
	$inventoryFields = array(	'LBL_TAX_SETTINGS'				=> 'Vtiger/Settings/TaxIndex',
								'INVENTORYTERMSANDCONDITIONS'	=> 'Settings/Vtiger/TermsAndConditionsEdit');
    ...
    $getSeqId = $db->pquery('select id from vtiger_customview_seq', array());
    $seqId = $db->query_result($getSeqId, 0, 'id');
    $db->pquery('insert into vtiger_customview values(?,?,?,?,?,?,?)', array($seqId+1, 'All', 1, 0, 'VTPDFMaker', 0,1));
    $db->pquery('insert into vtiger_customview values(?,?,?,?,?,?,?)', array($seqId+2, 'All', 1, 0, 'EmailPlus', 0,1));
    $db->pquery('insert into vtiger_customview_seq values(?)', array($seqId+2));
	if (is_dir('modules/Vtiger/resources')) {
		rename('modules/Vtiger/resources', 'modules/Vtiger/resources_650');
	}

    $db->pquery('insert into vtiger_ws_entity (name, handler_path, handler_class, ismodule) values (?, ?, ?, ?)', array('VTPDFMaker', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', 1));

    $db->pquery('insert into vtiger_ws_entity (name, handler_path, handler_class, ismodule) values (?, ?, ?, ?)', array('EmailPlus', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', 1));

   $db->pquery('insert into vtiger_ws_entity (name, handler_path, handler_class, ismodule) values (?, ?, ?, ?)', array('ModComments', 'include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation', 1));

    $db->pquery('update vtiger_settings_field set linkto=? where name=?', array('Settings/MailConverter/List','LBL_MAIL_SCANNER'));
    $db->pquery('update vtiger_settings_field set linkto=? where name=?', array('ModTracker/BasicSettings/Settings/ModTracker','ModTracker'));
    $db->pquery('update vtiger_settings_field set linkto=? where name=?', array('PBXManager/Settings/Index','LBL_PBXMANAGER'));
    $db->pquery('update vtiger_settings_field set linkto=? where name=?', array('Settings/Server/ProxyConfig','LBL_SYSTEM_INFO'));
    $db->pquery('update vtiger_settings_field set linkto=? where name=?', array('Settings/DefModuleView/Settings','LBL_DEFAULT_MODULE_VIEW'));

   $db->pquery('INSERT INTO vtiger_settings_field(fieldid, blockid, name, iconpath, description, linkto, sequence,active, pinned) VALUES (?, ?,?,?,?,?,?,?,?)',array($db->getUniqueID('vtiger_settings_field'), 6,'Module Studio', 'layouts/v7/modules/Settings/ModuleDesigner/assets/images/ModuleDesigner.png', 'LBL_MODULEDESIGNER_DESCRIPTION', 'ModuleDesigner/Index/Settings', 3, 0, 0));

   $db->pquery('insert into vtiger_settings_blocks values (?,?,?)',array($db->getUniqueID('vtiger_settings_blocks'), 'LBL_JOFORCE', '11'));
   $getBlockId = $db->pquery('select blockid from vtiger_settings_blocks where label = ?', array('LBL_JOFORCE'));
   $blockId = $db->query_result($getBlockId, 0, 'blockid');
   $db->pquery('insert into vtiger_settings_field values (?,?,?,?,?,?,?,?,?)',array($db->getUniqueID('vtiger_settings_field'), $blockId, 'Contributors', '', 'Contributors', 'Vtiger/Credits/Settings',1, 0,0));
   $db->pquery('insert into vtiger_settings_field values (?,?,?,?,?,?,?,?,?)',array($db->getUniqueID('vtiger_settings_field'), $blockId, 'License', '', 'License', 'Vtiger/Settings/License',2, 0,0));
