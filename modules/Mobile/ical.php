...
class Mobile_ICAL {
	...
	function generate() {
		...
		$query = "SELECT " . implode(',', $fieldnames) . " FROM jo_activity
			INNER JOIN jo_crmentity ON
			(jo_activity.activityid=jo_crmentity.crmid 	AND jo_crmentity.deleted = 0 AND jo_crmentity.smownerid = ?)
			LEFT JOIN jo_activity_reminder ON jo_activity_reminder.activity_id=jo_activity.activityid
