...
class Mobile_WS_TaxByType extends Mobile_WS_Controller{
    ...
    protected function getCharges($recordId) {
		...
		if ($recordId) {
			$result = $adb->pquery('SELECT * FROM jo_inventorychargesrel WHERE recordid = ?', array($recordId));
    ...
    protected function getTableName($taxType){
        switch($taxType){
            case 'shipping':
                return 'jo_shippingtaxinfo';
                break;
            case 'inventory':
                return 'jo_inventorytaxinfo';
                break;
            case 'charges':
                return 'jo_inventorychargesrel';
                break;
        }
    }
}
