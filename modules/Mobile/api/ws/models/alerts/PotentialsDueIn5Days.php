...
class Mobile_WS_AlertModel_PotentialsDueIn5Days extends Mobile_WS_AlertModel {
	...
	function query() {
		$sql = Mobile_WS_Utils::getModuleListQuery('Potentials', 
					"jo_potential.sales_stage not like 'Closed%' AND 
					DATEDIFF(jo_potential.closingdate, CURDATE()) <= 5"
				);
		return preg_replace("/^SELECT count\(\*\) as count(.*)/i", "SELECT crmid $1", Head_Functions::mkCountQuery($sql));
	}
