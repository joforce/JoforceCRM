...
class WSAPP_Logs {
	const basetable = 'jo_wsapp_logs_basic';
	const detailstable = 'jo_wsapp_logs_details';
	...
	static function add($syncRecords) {
		...
		$id = $adb->getUniqueID('jo_wsapp_logs_basic');
						...
						if ($type == 'source') {
							...
							switch($data->getMode()) {
								...
								case 'skipped'									:	if(empty($recordInfo['messageidentifier'])) {
																						$moduleModel = Head_Module_Model::getInstance($extensionModule);
