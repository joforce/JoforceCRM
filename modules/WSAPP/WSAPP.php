...
class WSAPP
{
	...
	function registerCustomWebservices($operations)
	{
		...
		foreach ($operations as $operation_name => $operation_info) {
			$checkres = $adb->pquery("SELECT operationid FROM jo_ws_operation WHERE name=?", array($operation_name));
			if ($checkres && $adb->num_rows($checkres) < 1) {
				$operation_id = $adb->getUniqueId('jo_ws_operation');

				$operation_res = $adb->pquery(
					"INSERT INTO jo_ws_operation (operationid, name, handler_path, handler_method, type, prelogin) 
					VALUES (?,?,?,?,?,?)",
					array(
				...
				foreach ($operation_parameters as $parameter_name => $parameter_type) {
					$adb->pquery(
						"INSERT INTO jo_ws_operation_parameters (operationid, name, type, sequence) 
						VALUES(?,?,?,?)",
						array($operation_id, $parameter_name, $parameter_type, ($parameter_index + 1))
					);
					++$parameter_index;
				}
				Head_Utils::Log("Opearation $operation_name enabled successfully.");
			} else {
				Head_Utils::Log("Operation $operation_name already exists.");
			}
		}
	}
	...
	function registerHandlers()
	{
		...
		foreach ($handlerDetails as $appHandlerDetails)
			$adb->pquery("INSERT INTO jo_wsapp_handlerdetails VALUES(?,?,?)", array($appHandlerDetails['type'], $appHandlerDetails['handlerclass'], $appHandlerDetails['handlerpath']));
	}

	function registerHeadCRMApp()
	{
		$db = PearDatabase::getInstance();
		$appName = "vtigerCRM";
		$type  = "user";
		$uid = uniqid();
		$db->pquery("INSERT INTO jo_wsapp (name, appkey,type) VALUES(?,?,?)", array($appName, $uid, $type));
	}
	...
	function registerWsappWorkflowhandler()
	{
		...
		$dependentEventHandlers = array('EntityDelta');
		$dependentEventHandlersJson = Zend_Json::encode($dependentEventHandlers);
		$em->registerHandler('jo.entity.aftersave', 'modules/WSAPP/WorkFlowHandlers/WSAPPAssignToTracker.php', 'WSAPPAssignToTracker', '', $dependentEventHandlersJson);
	}

	function registerSynclibEventHandler()
	{
		$className = 'WSAPP_HeadSyncEventHandler';
		$path = 'modules/WSAPP/synclib/handlers/HeadSyncEventHandler.php';
		$type = 'vtigerSyncLib';
