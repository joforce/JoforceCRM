 ...
 *************************************************************************************/
 ...
 Class Project_Record_Model extends Head_Record_Model {
	...
	public function getSummaryInfo() {
		...
		if($userPrivilegesModel->hasModulePermission($projectTaskInstance->getId())) {
			...
			$query ='SELECT smownerid,enddate,projecttaskstatus,projecttaskpriority
					FROM jo_projecttask
							INNER JOIN jo_crmentity ON jo_crmentity.crmid=jo_projecttask.projecttaskid
								AND jo_crmentity.deleted=0
							WHERE jo_projecttask.projectid = ? ';
	...
	public function getProjectTasks() {
		...
		$sql = "SELECT projecttaskid as recordid,projecttaskname as name,startdate,enddate,projecttaskstatus FROM jo_projecttask 
				INNER JOIN jo_crmentity  ON jo_projecttask.projecttaskid = jo_crmentity.crmid
				WHERE projectid=? AND jo_crmentity.deleted=0 AND jo_projecttask.startdate IS NOT NULL AND jo_projecttask.enddate IS NOT NULL";
 ...
 function getStatusColors() {
		...
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT *FROM jo_projecttask_status_color');
