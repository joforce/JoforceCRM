...
class ProjectTask_MassActionAjax_View extends Project_MassActionAjax_View {
	...
	protected function getEmailFieldsInfo(Head_Request $request) {
		...
		$moduleModel = Head_Module_Model::getInstance('Project');
		...
		foreach ($referenceFields as $referenceField) {
			...
			$refModule = Head_Module_Model::getInstance($refModuleName);
			if ($refModule) {
		...
		if (!empty($referenceFieldValues)) {
			foreach ($referenceFieldValues as $recordId => $refRecordIds) {
				foreach ($refRecordIds as $refRecordId) {
					$refModuleName = Head_Functions::getCRMRecordType($refRecordId);
					if (empty($refModuleName) || $refModuleName == 'Users')
						continue;
	...
	protected function getProjectIds($taskIds = array()) {
		...
		if (!empty($taskIds)) {
			$db = PearDatabase::getInstance();
			$sql = 'SELECT projectid FROM jo_projecttask INNER JOIN jo_crmentity  ON jo_projecttask.projecttaskid = jo_crmentity.crmid
						WHERE jo_crmentity.deleted=0 AND jo_crmentity.crmid IN ('.generateQuestionMarks($taskIds).')';
	...
	protected function isPreferencesNeedToBeUpdated(Head_Request $request) {
		...
		if (!$parentStatus) {
			$recipientPrefModel = Head_RecipientPreference_Model::getInstance($request->getModule());
			if (!$recipientPrefModel)
				return $parentStatus;
			...
			foreach ($refFields as $refField) {
				if ($refField && $refField->isViewable()) {
					...
					foreach ($referenceList as $moduleName) {
						if ($moduleName !== 'Users') {
							$refModuleModel = Head_Module_Model::getInstance($moduleName);
							if (!$prefs[$refModuleModel->getId()])
								continue;
							$moduleEmailPrefs = $prefs[$refModuleModel->getId()];
							foreach ($moduleEmailPrefs as $fieldId) {
								$field = Head_Field_Model::getInstance($fieldId, $refModuleModel);
								if ($field) {
