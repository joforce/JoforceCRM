...
class EmailTemplates_List_View extends Head_Index_View {
	...
	function process(Head_Request $request) {
		...
		$orderParams = Head_ListView_Model::getSortParamsSession($moduleName);
		// TODO : need to remove this when vtiger6 is removed
		$defaultLayout = Head_Viewer::getDefaultLayoutName();
		if($orderParams['viewType'] == 'grid' && $defaultLayout == ''){//SMACKJO
	...
	public function initializeListViewContents(Head_Request $request, Head_Viewer $viewer) {
		...
		if ($request->get('mode') == 'removeAlphabetSearch') {
			Head_ListView_Model::deleteParamsSession($moduleName, array('search_key', 'search_value', 'operator'));
			$searchKey = '';
			$searchValue = '';
			$operator = '';
		}
		if ($request->get('mode') == 'removeSorting') {
			Head_ListView_Model::deleteParamsSession($moduleName, array('orderby', 'sortorder'));
			$orderBy = '';
			$sortOrder = '';
		}
		if (empty($orderBy) && empty($searchValue) && empty($pageNumber)) {
			...
			Head_ListView_Model::setSortParamsSession($moduleName, $params);
		}
