		...
		if (!empty($lastSyncTime) && $currentPage === 0) {
			...
			if ($hasAssignedToField) {
				if ($isPrivateMode) {
					...
					$andsmowneridequal = " AND jo_crmentity.smownerid=?";
				} else {
			...
			if ($module == 'Calendar') {
				$queryDeleted = $adb->pquery("SELECT activityid as crmid, activitytype as setype FROM jo_activity
					INNER JOIN jo_crmentity ON jo_activity.activityid=jo_crmentity.crmid
					AND jo_crmentity.deleted=1 AND jo_crmentity.setype=? AND jo_crmentity.modifiedtime > ?
					LEFT JOIN jo_users ON jo_users.id = jo_crmentity.smownerid
					LEFT JOIN jo_groups ON jo_groups.groupid = jo_crmentity.smownerid WHERE 1=1	$andsmowneridequal ",
				$queryDeletedParameters);
			} else if ($module == 'Leads') {
				$queryDeleted = $adb->pquery("SELECT crmid, modifiedtime, setype FROM jo_crmentity
				...
				WHERE (jo_crmentity.deleted=1 OR (jo_crmentity.deleted=0 AND jo_leaddetails.converted=1)) AND jo_crmentity.setype=? AND jo_crmentity.modifiedtime > ? $andsmowneridequal", $queryDeletedParameters);
			} else {
				$queryDeleted = $adb->pquery("SELECT crmid, modifiedtime, setype FROM jo_crmentity
				LEFT JOIN jo_users ON jo_users.id = jo_crmentity.smownerid
				LEFT JOIN jo_groups ON jo_groups.groupid = jo_crmentity.smownerid
				WHERE jo_crmentity.deleted=1 AND jo_crmentity.setype=? AND jo_crmentity.modifiedtime > ? $andsmowneridequal", $queryDeletedParameters);
			}
