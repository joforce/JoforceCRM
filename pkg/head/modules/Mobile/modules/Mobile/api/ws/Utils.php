...
class Mobile_WS_Utils {
	...
	static function getHeadVersion() {
		global $jo_current_version;
		return $jo_current_version;
	}
	
	static function getVersion() {
		global $adb;
		$versionResult = $adb->pquery("SELECT version FROM jo_tab WHERE name='Mobile'", array());
	...
	static function getEntityModuleWSIds($ignoreNonModule = true) {
		...
		$result = false;
		if($ignoreNonModule) {
			$result = $adb->pquery("SELECT id, name FROM jo_ws_entity WHERE ismodule=1", array());
		} else {
			$result = $adb->pquery("SELECT id, name FROM jo_ws_entity", array());
		}
	...
	static function getEntityFieldnames($module) {
		global $adb;
		$result = $adb->pquery("SELECT fieldname FROM jo_entityname WHERE modulename=?", array($module));
		$fieldnames = array();
		if($result && $adb->num_rows($result)) {
	...
	static function getModuleColumnTableByFieldNames($module, $fieldnames) {
		global $adb;
		$result = $adb->pquery("SELECT fieldname,columnname,tablename FROM jo_field WHERE tabid=? AND fieldname IN (".
	...
	static function documentFoldersInfo() {
		global $adb;
		$folders = $adb->pquery("SELECT folderid, foldername FROM jo_attachmentsfolder", array());
		$folderOptions = array();
	...
	static function getRelatedFunctionHandler($sourceModule, $targetModule) {
		global $adb;
		$relationResult = $adb->pquery("SELECT name FROM jo_relatedlists WHERE tabid=? and related_tabid=? and presence=0", array(getTabid($sourceModule), getTabid($targetModule)));
		$functionName = false;
	...
	static function querySecurityFromSuffix($module, $current_user) {
		...
		if($is_admin==false && $profileGlobalPermission[1] == 1 && $profileGlobalPermission[2] == 1 
			&& $defaultOrgSharingPermission[$tabid] == 3) {
				...
				$querySuffix .= " AND (jo_crmentity.smownerid in($current_user->id) OR jo_crmentity.smownerid IN 
					(
					...
					if(sizeof($current_user_groups) > 0) {
						$querySuffix .= " jo_groups.groupid IN (". implode(",", $current_user_groups) .") OR ";
					}
					$querySuffix .= " jo_groups.groupid IN 
						(
