...
class ModComments extends ModCommentsCore {
	...
	static function transferRecords($currentParentId, $targetParentId) {
		global $adb;
		$adb->pquery("UPDATE jo_modcomments SET related_to=? WHERE related_to=?", array($targetParentId, $currentParentId));
	}
	...
	static function addWidgetTo($moduleNames, $widgetType='DETAILVIEWWIDGET', $widgetName='DetailViewBlockCommentWidget') {
		...
		foreach($moduleNames as $moduleName) {
			$module = Head_Module::getInstance($moduleName);
			if($module) {
		...
		if (count($commentWidgetModules) > 0) {
			$modCommentsModule = Head_Module::getInstance('ModComments');
			$modCommentsModule->addLink('HEADERSCRIPT', 'ModCommentsCommonHeaderScript', 'modules/ModComments/ModCommentsCommon.js');
			$modCommentsRelatedToField = Head_Field::getInstance('related_to', $modCommentsModule);
	...
	public static function copyCommentsToRelatedRecord($currentParentId, $targetParentId) {
		...
		$relatedIdMap = array();
		$result = $db->pquery("SELECT *FROM jo_modcomments WHERE related_to=?", array($currentParentId));
		...
		for($i=0;$i<$count;$i++) {
			...
			$crmEntityResult = $db->pquery('SELECT *FROM jo_crmentity where crmid = ?', array($commentId));
			...
			$commentCrmId = $db->getUniqueID('jo_crmentity');
			...
			$db->pquery('INSERT INTO jo_crmentity values('. generateQuestionMarks($crmentityParams) .')', $crmentityParams);
			...
			$db->pquery('INSERT INTO jo_modcomments values('. generateQuestionMarks($modcommentsParams) .')', $modcommentsParams);
