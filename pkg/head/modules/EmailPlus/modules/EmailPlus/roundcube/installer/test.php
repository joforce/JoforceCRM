...
if ($read_config = is_readable(RCUBE_CONFIG_DIR . 'config/config.inc.php')) {
  $config = $RCI->load_config_file(RCUBE_CONFIG_DIR . 'config/config.inc.php');
  if (!empty($config)) {
    $RCI->pass('config/config.inc.php');
  }
  else {
    $RCI->fail('config/config.inc.php', 'Syntax error');
  }
}
else {
  $RCI->fail('config/config.inc.php', 'Unable to read file. Did you create the config file?');
}
echo '<br />';
...
if ($RCI->configured && ($messages = $RCI->check_config())) {
  ...
  echo html::a(array('href' => './?_mergeconfig=1'), 'config/config.inc.php') . ' &nbsp;';
  echo "</p>";
...
<tt>enable_installer</tt> option in <tt>config/config.inc.php</tt> is disabled.<br />
<br />
