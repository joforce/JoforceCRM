...
var Head_PBXManager_Js = {
	/**
	 * Function registers PBX for popups
	 */
	registerPBXCall : function() {
		Head_PBXManager_Js.requestPBXgetCalls();
	},
	...
	registerPBXOutboundCall : function(number,record) {
		Head_PBXManager_Js.makeOutboundCall(number,record);
	},
	/**
			...
			if(data.success && data.result) {
				for(i=0; i< data.result.length; i++) {
					...
					if(jQuery('#pbxcall_'+record.pbxmanagerid+'').size()== 0 )
						Head_PBXManager_Js.showPBXIncomingCallPopup(record);
					else
						Head_PBXManager_Js.updatePBXIncomingCallPopup(record);
					}
				}
		});
		Head_PBXManager_Js.removeCompletedCallPopup();
	},
	...
	showPBXIncomingCallPopup : function(record) {
		...
		if(record.user) {
			if(record.user != record.current_user_id) {
				Head_PBXManager_Js.removeCallPopup(record.pbxmanagerid);
			}
		}

		// To check if it is new or existing contact
		Head_PBXManager_Js.checkIfRelatedModuleRecordExist(record);
		...
		jQuery('#pbxcontactsave_'+record.pbxmanagerid+'').bind('click', function(e) {
			...
			Head_PBXManager_Js.createRecord(e, record);
			...
			app.request.get(url).then(function(data){
				if(data.result){
					if(data.result!='in-progress' && data.result!='ringing'){
						Head_PBXManager_Js.removeCallPopup(callid);
					}
				}
			});
		}
	...
	makeOutboundCall : function(number, record){
	 ...
	 registerEvents : function(){
		...
		app.request.get(url).then(function(data){
			if(data && data.result) {
				Head_PBXManager_Js.registerPBXCall();
				setInterval("Head_PBXManager_Js.registerPBXCall()", 3000);
			}
		});
	}
...
jQuery(document).ready(function() {
	Head_PBXManager_Js.registerEvents();
});
