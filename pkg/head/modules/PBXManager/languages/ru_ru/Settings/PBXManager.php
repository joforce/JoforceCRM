<?php

/* +***********************************************************************************
 * The contents of this file are subject to the vtiger CRM Public License Version 1.0
 * ("License"); You may not use this file except in compliance with the License
 * The Original Code is:  vtiger CRM Open Source
 * The Initial Developer of the Original Code is vtiger.
 * Portions created by vtiger are Copyright (C) vtiger.
 * All Rights Reserved.
 * Contributor(s): JoForce.com
 * *********************************************************************************** */
$languageStrings = array(
    'LBL_SELECT_ONE' => 'Выберите',
    'LBL_PBXManager' =>'PBXManager',
    'LBL_PBXMANAGER_CONFIG' => 'Подробные сведения о сервере Asterisk',
    'LBL_NOTE' => 'Note:',
    'LBL_INFO_WEBAPP_URL' => 'Настройте URL Asterisk App в формате', 
    'LBL_FORMAT_WEBAPP_URL' => '(Протокол) :/ / (asterisk_ip): (порт)',
    'LBL_FORMAT_INFO_WEBAPP_URL' => 'например: http://0.0.0.0:5000',
    'LBL_INFO_CONTEXT' => 'Head конкретном контексте, сконфигурированные в вашей Asterisk сервера (extensions.conf)',
    'LBL_PBXMANAGER_INFO' => 'Настроить Asterisk сервер детали после Установка Head звездочка разъем вашего сервера Asterisk',
    
    'webappurl'=>'Head Askterisk веб URL',
    'vtigersecretkey'=>'Head Секретный ключ',
    'outboundcontext' => 'Исходящий Контекст',
    'outboundtrunk' => 'Исходящий багажника',
    
);

$jsLanguageStrings = array(
    
);
?>  
