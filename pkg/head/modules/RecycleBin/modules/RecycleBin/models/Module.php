...
class RecycleBin_Module_Model extends Head_Module_Model {
	...
	public function emptyRecycleBin(){
		...
		$db->pquery('DELETE jo_modtracker_basic.* FROM jo_modtracker_basic INNER JOIN jo_crmentity on 
					jo_crmentity.crmid = jo_modtracker_basic.crmid AND jo_crmentity.deleted = 1', array());

		$getIdsQuery='SELECT crmid from jo_crmentity WHERE deleted=?';
		...
		$db->query('DELETE FROM jo_crmentity WHERE deleted = 1');
		$db->query('DELETE FROM jo_relatedlists_rb');
	...
	public function deleteRecords($recordIds){
		...
		$query = 'DELETE FROM jo_relatedlists_rb WHERE entityid in('.generateQuestionMarks($recordIds).')';
		$db->pquery($query, array($recordIds));

		// TODO - Remove records from module tables and other related stores.
		$query = 'DELETE FROM jo_modtracker_basic WHERE crmid in(' . generateQuestionMarks($recordIds) . ')';
		$db->pquery($query, array($recordIds));
		// Delete entries of attachments from jo_attachments and jo_seattachmentsrel
		$this->deleteFiles($recordIds);

		//Delete the records in vtiger crmentity and relatedlists.
		$query = 'DELETE FROM jo_crmentity WHERE deleted = ? and crmid in('.generateQuestionMarks($recordIds).')';
	...
	public function deleteFiles($recordIds){
		global $db;
		$db=PearDatabase::getInstance();
		$getAttachmentsIdQuery='SELECT * FROM jo_seattachmentsrel WHERE crmid in('.generateQuestionMarks($recordIds).')';
		...
		if(!empty($attachmentsIds)){
			$deleteRelQuery='DELETE FROM jo_seattachmentsrel WHERE crmid in('.generateQuestionMarks($recordIds).')';
			$db->pquery($deleteRelQuery,array($recordIds));
			$attachmentsLocation=array();
			$getPathQuery='SELECT * FROM jo_attachments WHERE attachmentsid in ('.generateQuestionMarks($attachmentsIds).')';
			...
			$deleteAttachmentQuery='DELETE FROM jo_attachments WHERE attachmentsid in ('.generateQuestionMarks($attachmentsIds).')';
	...
	public function getDeletedRecordsTotalCount() {  
		$db = PearDatabase::getInstance();  
		$totalCount = $db->pquery('select count(*) as count from jo_crmentity where deleted=1',array());  
	...
	public function isRecordsDeleted() {
		$db = PearDatabase::getInstance(); 
		$totalCount = $db->pquery('SELECT 1 FROM jo_crmentity WHERE deleted=1 LIMIT 1',array());
