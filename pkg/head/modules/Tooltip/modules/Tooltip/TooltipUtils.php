...
function getFieldList($module_name, $field_name = "") {
	...
	$query = "select * from jo_field where tabid = ?";
	$params = array($tabid);
	if (!empty ($field_name)) {
		$query .= " and fieldname not like ?";
		$params = array($tabid,$field_name);
	}
	$query.= " and columnname not like 'imagename' and uitype not in (61, 122) and jo_field.presence in (0,2)";
...
function moduleList() {
	global $adb;
	$sql = "select distinct jo_field.tabid,name from jo_field inner join jo_tab on jo_field.tabid=jo_tab.tabid where name not in ('Calendar','Events','Users')";
...
function tooltip_exists($fieldid, $related_fieldid) {
	global $adb;
	$query = "select * from jo_quickview where fieldid=? and related_fieldid=?";
...
function getToolTipText($view,$fieldname,$module,$value){
	...
	$quickview = 'select fieldname,fieldlabel from jo_quickview inner join jo_field on jo_quickview.related_fieldid=jo_field.fieldid where jo_quickview.fieldid = ? and currentview= ? and jo_field.presence in (0,2) order by jo_quickview.sequence';
...
function ToolTipExists($fieldname,$tabid){
	if(empty($fieldname) || empty($tabid)){
		...
		global $adb;
		$sql = "select fieldid from jo_field where tabid = ? and fieldname = ? and jo_field.presence in (0,2)";
		$result = $adb->pquery($sql,array($tabid,$fieldname));
		$count = $adb->num_rows($result);
		if($count > 0){
			$fieldid = $adb->query_result($result,0,'fieldid');
		
			$sql = "select * from jo_quickview where fieldid = ?";
...
function QuickViewFieldList($module){
	...
	$query = "select * from jo_field where tabid = ? and columnname not like 'imagename' and uitype not in (61, 122) and jo_field.presence in (0,2)";
