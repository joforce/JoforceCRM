...
function SaveTooltipInformation($fieldid, $sequence, $view=1){
	...
	if(empty($fieldid)){
		...
		if(!empty($_REQUEST['checkedFields'])){
			...
			//add to jo_quickview table
			foreach($checkedFields as $checkedField){
				$query = "insert into jo_quickview (fieldid,related_fieldid,sequence,currentview) values (?,?,?,?)";
...
function deleteOldInfo($fieldid, $view=1){
	...
	$query = "delete from jo_quickview where fieldid = ? and currentview = ?";
...
function getFieldLabels($checkedFields){
	...
	$sql = "select * from jo_field where fieldid in (".generateQuestionMarks($checkedFields).") and jo_field.presence in (0,2)";
