...
class MailManager_Draft_Model {
		...
		for($i=0; $i<count($draftMails); $i++) {
			foreach($draftMails[$i] as $fieldname=>$fieldvalue) {
				if($fieldname == "saved_toid" || $fieldname == "ccmail" || $fieldname == "bccmail") {
                    ...
                    if(!empty($fieldvalue)) {
						$value = Head_Date_UIType::getDisplayDateValue($fieldvalue);
						$draftMails[$i][$fieldname] = $value;
					}
                } elseif($fieldname == "id") {
	...
	public function getAttachmentDetails($crmid) {
		...
		$documentRes = $db->pquery("SELECT * FROM jo_senotesrel
									...
									WHERE jo_crmentity.deleted = 0", array($crmid));
	...
	public function saveEmailParentRel($emailId, $parentIds) {
		...
		if(!empty($emailId)) {
			$db->pquery("delete from jo_seactivityrel where activityid=?",array($emailId)); //remove all previous relation
		}
		for ($i=0; $i<(count($myids)); $i++) {
			$realid = explode("@",$myids[$i]);
			if(!empty($realid[0]) && !empty($emailId)) {
				// this is needed as we might save the mail in draft mode earlier
				$result = $db->pquery("SELECT * FROM jo_seactivityrel WHERE crmid=? AND activityid=?",array($realid[0], $emailId));
				if(!$db->num_rows($result)) {
					$db->pquery('INSERT INTO jo_seactivityrel(crmid, activityid) VALUES(?,?)',array($realid[0], $emailId));
				}
			}
		}
	}
	...
	public function getFromEmailAddress() {
		...
		$fromEmail = false;
		if (Head_Version::check('5.2.0', '>=')) {
			$smtpFromResult = $db->pquery('SELECT from_email_field FROM jo_systems WHERE server_type=?', array('email'));
	...
	public function getParentFromEmails($to_string) {
		...
		if (!empty($to_string)) {
			...
			foreach($toArray as $to) {
				$relatedtos = MailManager::lookupMailInHead(trim($to), $currentUserModel);
	...
	public function handleUpload() {
		...
		$uploadLimit = MailManager_Config_Model::get('MAXUPLOADLIMIT', Head_Util_Helper::getMaxUploadSizeInBytes());
	...
	public function saveEmailDocumentRel($emailId, $documentId) {
		...
		if(!empty($emailId) && !empty($documentId)) {
			$db->pquery("INSERT INTO jo_senotesrel(crmid, notesid) VALUES(?,?)",
	...
	public function saveAttachmentRel($crmid, $attachId) {
		...
		if(!empty($crmid) && !empty($attachId)) {
			$db->pquery("INSERT INTO jo_seattachmentsrel(crmid, attachmentsid) VALUES(?,?)",
	...
	public function deleteAttachment($request) {
		...
		if(!empty($docid) && !empty($emailid)) {
			$db->pquery("DELETE FROM jo_senotesrel WHERE crmid = ? AND notesid = ?", array($emailid, $docid));
			return true;
		}
		return false;
	}
