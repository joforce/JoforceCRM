...
class MailManager_Relation_View extends MailManager_Abstract_View {
	...
	public function process(Head_Request $request) {
		...
		if ('find' == $this->getOperationArg($request)) {
			...
			if (!class_exists($quickCreateviewClassName)) {
				$quickCreateviewClassName = 'Head_QuickCreateAjax_View';
			}
			...
			$linkedto = MailManager_Relate_Action::getSalesEntityInfo($parent);
			$recordModel = Head_Record_Model::getCleanInstance($linkModule);
			...
			switch ($linkModule) {
				case 'Calendar' :   $activityType = $recordModel->get('activitytype');
									...
									if ($activityType != 'Task') {
										$endTime = Head_Time_UIType::getTimeValueWithSeconds($request->get('time_end'));
	...
	public function setParentForHelpDesk($parent, $from) {
		...
		if(empty($parent)) {
			if(!empty($from)) {
				$parentInfo = MailManager::lookupMailInHead($from[0], $currentUserModel);
	...
	public function lookupRelModuleRecords($wsId) {
		...
		$wsObject = HeadWebserviceObject::fromId($db, $wsId);
		$entityName = $wsObject->getEntityName();

		foreach ($relModules as $relModule) {
			$relation = Head_Relation_Model::getInstanceByModuleName($entityName, $relModule);
			if(!$relation) {
				continue;
			}
