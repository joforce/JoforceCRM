		...
		if ('open' == $this->getOperationArg($request)) {
			...
			if (!empty($to_string)) {
				...
				foreach($toArray as $to) {
					$relatedtos = MailManager::lookupMailInHead($to, $currentUserModel);
					...
					if ($status === true) {
						...
						if ($realid[1] == -1) {
							$db->pquery('DELETE FROM jo_salesmanactivityrel WHERE smid=? AND activityid=?',$params);
							$db->pquery('INSERT INTO jo_salesmanactivityrel VALUES (?,?)', $params);
						} else {
							$db->pquery('DELETE FROM jo_seactivityrel WHERE crmid=? AND activityid=?', $params);
							$db->pquery('INSERT INTO jo_seactivityrel VALUES (?,?)', $params);
						}
					}
				}
			}
