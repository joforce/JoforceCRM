...
class MailManager_Relate_Action extends Head_MailScannerAction {
	...
	public function __CreateNewEmail($mailrecord, $module, $linkfocus) {
		...
		$recordModel = Head_Record_Model::getCleanInstance('Emails');
		...
		if (is_array($inlineAttachments)) {
			foreach ($inlineAttachments as $index => $att) {
				$cid = $att['cid'];
				$attch_name = Head_MailRecord::__mime_decode($att['filename']);
				$id = $mailrecord->muid();
	...
	public function __SaveAttachements($mailrecord, $basemodule, $recordModel) {
		...
		foreach($mailrecord->_attachments as $attachmentData) {
			...
			if($issaved) {
				...
				$attachRes = $db->pquery("SELECT * FROM jo_attachments WHERE attachmentsid = ?", array($attachid));
	...
	public static function getSalesEntityInfo($crmid) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT setype FROM jo_crmentity WHERE crmid=? AND deleted=0", array($crmid));
	...
	public static function ws_modulename($modulewsid) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT name FROM jo_ws_entity WHERE id=?", array($modulewsid));
