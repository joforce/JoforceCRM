...
class MailManager {
	...
	static function updateMailAssociation($mailuid, $emailid, $crmid) {
		global $adb;
		$adb->pquery("INSERT INTO jo_mailmanager_mailrel (mailuid, emailid, crmid) VALUES (?,?,?)", array($mailuid, $emailid, $crmid));
	}

	static function lookupMailInHead($searchTerm, $user) {
		...
		foreach($referenceModules as $referenceModule) {
			...
			if(!empty($searchFieldList) && !empty($referenceModuleEmailFields)) {
				...
				if($referenceModule == 'Users' && !is_admin($user)){
					...
					global $adb;
					$where .= " AND jo_users.status='Active'";
					$query = "select $searchFieldListString,id from jo_users $where";
	...
	static function lookupMailAssociation($mailuid) {
		...
		$result = $adb->pquery(
			"SELECT jo_mailmanager_mailrel.* FROM jo_mailmanager_mailrel INNER JOIN
			jo_crmentity ON jo_crmentity.crmid=jo_mailmanager_mailrel.crmid AND jo_crmentity.deleted=0
			AND jo_mailmanager_mailrel.mailuid=? LIMIT 1", array(decode_html($mailuid)));
