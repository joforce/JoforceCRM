...
Class Google_Calendar_Connector extends WSAPP_TargetConnector {
    ...
    protected $eventCalendarFieldMappingTableName = 'jo_google_event_calendar_mapping';
    protected $calendars;
    ...
    public function emailLookUp($emailIds) {
        ...
        $sql = 'SELECT crmid FROM jo_emailslookup WHERE setype = "Contacts" AND value IN (' .  generateQuestionMarks($emailIds) . ')';
    ...
    public function transformToSourceRecord($targetRecords, $user = false) {
        ...
        foreach ($targetRecords as $googleRecord) {
            if ($googleRecord->getMode() != WSAPP_SyncRecordModel::WSAPP_DELETE_MODE) {
                ...
                $entity = Head_Functions::getMandatoryReferenceFields('Events');
    ...
    protected function putGoogleEventCalendarMap($event_ids, $calendar_id, $user) {
        if(is_array($event_ids) && count($event_ids)) {
            $db = PearDatabase::getInstance();
            $user_id = $user->getId();
            $sql = 'INSERT INTO jo_google_event_calendar_mapping (event_id, calendar_id, user_id) VALUES ';
            $sqlParams = array();
            ...
            $db->pquery('DELETE FROM jo_google_event_calendar_mapping WHERE event_id IN ('.generateQuestionMarks($event_ids).')',$event_ids);
    ...
    protected function getGoogleEventCalendarMap($user) {
        $db = PearDatabase::getInstance();
        $map = array();
        $sql = 'SELECT event_id, calendar_id FROM jo_google_event_calendar_mapping WHERE user_id = ?';
