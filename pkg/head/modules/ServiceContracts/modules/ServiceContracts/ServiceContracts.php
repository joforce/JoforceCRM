...
class ServiceContracts extends CRMEntity {
	...
	var $table_name = 'jo_servicecontracts';
	 ...
	 */
	var $customFieldTable = Array('jo_servicecontractscf', 'servicecontractsid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('jo_crmentity', 'jo_servicecontracts', 'jo_servicecontractscf');
	...
	var $tab_name_index = Array(
		'jo_crmentity' => 'crmid',
		'jo_servicecontracts' => 'servicecontractsid',
		'jo_servicecontractscf'=>'servicecontractsid');
	...
	function generateReportsSecQuery($module,$secmodule,$queryplanner) {
		...
		$matrix->setDependency('jo_crmentityServiceContracts',array('jo_groupsServiceContracts','jo_usersServiceContracts'));
		if (!$queryplanner->requireTable('jo_servicecontracts', $matrix)) {
			return '';
		}
		$matrix->setDependency('jo_servicecontracts',array('jo_servicecontractscf','jo_crmentityServiceContracts'));

		$query = $this->getRelationQuery($module,$secmodule,"jo_servicecontracts","servicecontractsid", $queryplanner);

		if ($queryplanner->requireTable("jo_crmentityServiceContracts",$matrix)){
			$query .= " left join jo_crmentity as jo_crmentityServiceContracts on jo_crmentityServiceContracts.crmid = jo_servicecontracts.servicecontractsid  and jo_crmentityServiceContracts.deleted=0";
		}
		if ($queryplanner->requireTable("jo_servicecontractscf")){
			$query .= " left join jo_servicecontractscf on jo_servicecontractscf.servicecontractsid = jo_servicecontracts.servicecontractsid";
		}
		if ($queryplanner->requireTable("jo_groupsServiceContracts")){
			$query .= " left join jo_groups as jo_groupsServiceContracts on jo_groupsServiceContracts.groupid = jo_crmentityServiceContracts.smownerid";
		}
		if ($queryplanner->requireTable("jo_usersServiceContracts")){
			$query .= " left join jo_users as jo_usersServiceContracts on jo_usersServiceContracts.id = jo_crmentityServiceContracts.smownerid";
		}
		if($queryplanner->requireTable("jo_contactdetailsRelServiceContracts")){
			$query .= " left join jo_contactdetails as jo_contactdetailsRelServiceContracts on jo_contactdetailsRelServiceContracts.contactid = jo_servicecontracts.sc_related_to";
		}
		if($queryplanner->requireTable("jo_accountRelServiceContracts")){
			$query .= " left join jo_account as jo_accountRelServiceContracts on jo_accountRelServiceContracts.accountid = jo_servicecontracts.sc_related_to";
		}
		 if ($queryplanner->requireTable("jo_lastModifiedByServiceContracts")){
			$query .= " left join jo_users as jo_lastModifiedByServiceContracts on jo_lastModifiedByServiceContracts.id = jo_crmentityServiceContracts.modifiedby ";
		}
		if ($queryplanner->requireTable("jo_createdbyServiceContracts")){
			$query .= " left join jo_users as jo_createdbyServiceContracts on jo_createdbyServiceContracts.id = jo_crmentityServiceContracts.smcreatorid ";
		}
		return $query;
	}
	/**
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		...
		if($eventType == 'module.postinstall') {
			...
			$helpDeskInstance = Head_Module::getInstance("HelpDesk");
			$helpDeskInstance->setRelatedList($moduleInstance,"Service Contracts",Array('ADD','SELECT'));

			// Initialize module sequence for the module
			$adb->pquery("INSERT into jo_modentity_num values(?,?,?,?,?,?)",array($adb->getUniqueId("jo_modentity_num"),$moduleName,'SERCON',1,1,1));

			// Make the picklist value 'Complete' for status as non-editable
			$adb->query("UPDATE jo_contract_status SET presence=0 WHERE contract_status='Complete'");
	...
	function save_related_module($module, $crmid, $with_module, $with_crmids, $otherParams = array()) {
	 ...
	 function updateHelpDeskRelatedTo($focusId, $entityIds) {
		...
		$selectTicketsQuery = "SELECT ticketid FROM jo_troubletickets
		...
		for($i=0; $i < $noOfTickets; ++$i) {
			...
			$serviceContractsRelateToTypeResult = $this->db->pquery('SELECT setype FROM jo_crmentity WHERE crmid =
				(SELECT sc_related_to FROM jo_servicecontracts WHERE servicecontractsid = ?)', array($focusId));
			...
			if($serviceContractsRelateToType == 'Accounts') {
				$updateQuery = "UPDATE jo_troubletickets, jo_servicecontracts SET parent_id=jo_servicecontracts.sc_related_to" .
					" WHERE jo_servicecontracts.sc_related_to IS NOT NULL AND jo_servicecontracts.sc_related_to != 0" .
					" AND jo_servicecontracts.servicecontractsid = ? AND jo_troubletickets.ticketid = ?";
				$this->db->pquery($updateQuery, array($focusId, $ticketId));
			} elseif($serviceContractsRelateToType == 'Contacts') {
				$updateQuery = "UPDATE jo_troubletickets, jo_servicecontracts SET contact_id=jo_servicecontracts.sc_related_to" .
					" WHERE jo_servicecontracts.sc_related_to IS NOT NULL AND jo_servicecontracts.sc_related_to != 0" .
					" AND jo_servicecontracts.servicecontractsid = ? AND jo_troubletickets.ticketid = ?";
				$this->db->pquery($updateQuery, array($focusId, $ticketId));
			}
	...
	function updateServiceContractState($focusId) {
		...
		$contractTicketsResult = $this->db->pquery("SELECT relcrmid FROM jo_crmentityrel
														WHERE module = 'ServiceContracts'
														AND relmodule = 'HelpDesk' AND crmid = ?
													UNION
														SELECT crmid FROM jo_crmentityrel
	...
	function updateUsedUnits($usedUnits) {
		...
		$updateQuery = "UPDATE jo_servicecontracts SET used_units = $usedUnits WHERE servicecontractsid = ?";
	...
	function calculateProgress() {
		...
		if($contractStatus == 'Complete' || (!empty($usedUnits) && !empty($totalUnits) && $usedUnits >= $totalUnits)) {
			if(empty($endDate)) {
				$endDate = date('Y-m-d');
				$this->db->pquery('UPDATE jo_servicecontracts SET end_date=? WHERE servicecontractsid = ?', array(date('Y-m-d'), $this->id));
			}
		} else {
			$endDate = null;
			$this->db->pquery('UPDATE jo_servicecontracts SET end_date=? WHERE servicecontractsid = ?', array(null, $this->id));
		}
		...
		if(count($updateCols) > 0) {
			$updateQuery = 'UPDATE jo_servicecontracts SET '. implode(",", $updateCols) .' WHERE servicecontractsid = ?';
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Documents"=>"jo_senotesrel","Attachments"=>"jo_seattachmentsrel");

		$tbl_field_arr = Array("jo_senotesrel"=>"notesid","jo_seattachmentsrel"=>"attachmentsid");

		$entity_tbl_field_arr = Array("jo_senotesrel"=>"crmid","jo_seattachmentsrel"=>"crmid");
