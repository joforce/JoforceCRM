...
class Helper{
	...
	public function convertFieldAndExportPDF($sourceModule, $recordIds, $tempId, $action_type = false, $templatecontent = false){
		...
		$getValue = $adb->pquery('select * from jo_vtpdfmaker where vtpdfmakerid = ?', array($tempIds[0]));
		...
		foreach($recordId_value as $recordId){
			$getValue = $adb->pquery('select * from jo_vtpdfmaker where vtpdfmakerid = ?', array($tempId));
