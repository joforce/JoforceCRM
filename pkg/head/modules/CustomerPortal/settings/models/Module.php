...
class Settings_CustomerPortal_Module_Model extends Settings_Head_Module_Model {
	...
	public function getCurrentPortalUser() {
		...
		$result = $db->pquery("SELECT prefvalue FROM jo_customerportal_prefs WHERE prefkey = 'userid' AND tabid = 0", array());
	...
	public function getCurrentDefaultAssignee() {
		...
		$result = $db->pquery("SELECT default_assignee FROM jo_customerportal_settings", array());
		if ($db->num_rows($result)) {
	 ...
	 * @return <Array> list of portal modules <Head_Module_Model>
	 */
	public function getModulesList() {
		if (!$this->portalModules) {
			$db = PearDatabase::getInstance();

			$query = "SELECT jo_customerportal_tabs.*, jo_tab.name FROM jo_customerportal_tabs
					INNER JOIN jo_tab ON jo_customerportal_tabs.tabid = jo_tab.tabid AND jo_tab.presence = 0 ORDER BY jo_customerportal_tabs.sequence";
	 ...
	 * @return <Array> list of Contact Related Modules <Head_Module_Model>
	 */
	public function getContactRelatedModulesList() {
		$contacModuleModel = Head_Module_Model::getInstance('Contacts');
		$relationModules = Head_Relation_Model::getAllRelations($contacModuleModel);
	...
	public function save() {
		...
		foreach ($enableModules as $moduleId => $visibility) {
			...
			$db->pquery('INSERT INTO jo_customerportal_tabs(tabid,visible) VALUES(?,?) ON DUPLICATE KEY UPDATE visible = ?', array($tabid, $visibility, $visibility));
		}

		$updateSequenceQuery = " UPDATE jo_customerportal_tabs SET sequence = ? WHERE tabid = ?";
		...
		if ($dashboardWidgets || $dashboardCharts) {
			$db->pquery('UPDATE jo_customerportal_settings SET default_assignee = ? ,support_notification = ?
		...
		if (!empty($relatedModuleList)) {
			foreach ($relatedModuleList as $module => $info) {
				$tabid = getTabid($module);
				$db->pquery('INSERT INTO jo_customerportal_relatedmoduleinfo(tabid, relatedmodules) VALUES(?,?) ON DUPLICATE KEY UPDATE relatedmodules = ?', array($tabid, $info, $info));
			}
		}
		...
		if (!empty($recordsVisible)) {
			foreach ($recordsVisible as $module => $info) {
				$tabid = getTabid($module);
				if ($info == 'all') {
					$db->pquery('UPDATE jo_customerportal_fields SET records_visible = ? WHERE tabid = ?', array(1, $tabid));
				} else if ($info == 'onlymine') {
					$db->pquery('UPDATE jo_customerportal_fields SET records_visible = ? WHERE tabid = ?', array(0, $tabid));
				}
			}
		}

		// clearing mem-cache for CustomerPortal
		Head_Cache::delete('CustomerPortal', 'activeModules');
		Head_Cache::delete('CustomerPortal', 'activeFields');
		...
		if (!empty($recordPermissions)) {
			...
			$db->pquery('UPDATE jo_customerportal_tabs SET createrecord=?,editrecord=? WHERE tabid=?', array($updatedPermissions['create'], $updatedPermissions['edit'], getTabid($updatedPermissions['module'])));
		}
	}

	public function getRelatedModules($sourceModule) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT relatedmodules FROM jo_customerportal_relatedmoduleinfo WHERE tabid = ? ', array(getTabid($sourceModule)));
	...
	public function getDashboardInfo() {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT * FROM jo_customerportal_settings', array());
	...
	public function getSelectedFields($tabId) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT fieldinfo FROM jo_customerportal_fields WHERE tabid = ?', array($tabId));
		if ($db->num_rows($result) > 0) {
	...
	public function updateFields($tabId, $fieldJson) {
		$db = PearDatabase::getInstance();
		$db->pquery('INSERT INTO jo_customerportal_fields(tabid, fieldinfo) VALUES(?,?) ON DUPLICATE KEY UPDATE fieldinfo = ?', array($tabId, $fieldJson, $fieldJson));
	}

	public function getRecordVisiblity($tabId) {
		$db = PearDatabase::getInstance();
		$result = $db->pquery('SELECT records_visible FROM jo_customerportal_fields WHERE tabid= ?', array($tabId));
		if ($db->num_rows($result)) {
	...
	public function getRecordPermissions($tabid) {
		$db = PearDatabase::getInstance();
		$permissionResult = $db->pquery('SELECT createrecord,editrecord FROM jo_customerportal_tabs WHERE tabid=?', array($tabid));
