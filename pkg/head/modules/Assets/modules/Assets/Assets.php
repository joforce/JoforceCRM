...
class Assets extends CRMEntity {
	 ...
	 */
	var $customFieldTable = Array('jo_assetscf', 'assetsid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('jo_crmentity','jo_assets','jo_assetscf');
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		...
		global $adb;
 		...
 		if($eventType == 'module.postinstall') {
			...
			$AssetsModule  = Head_Module::getInstance('Assets');
			Head_Access::setDefaultSharing($AssetsModule);

			//Showing Assets module in the related modules in the More Information Tab
			$assetInstance = Head_Module::getInstance('Assets');
			$assetLabel = 'Assets';
			...
			$productInstance = Head_Module::getInstance('Products');
			...
			if (!($adb->num_rows($result))) {
				...
				$adb->pquery("INSERT INTO jo_modentity_num values(?,?,?,?,?,?)", array($adb->getUniqueId("jo_modentity_num"), $moduleName, 'ASSET', 1, 1, 1));
			}
	...
	function addModuleToCustomerPortal() {
		...
		$assetsResult = $adb->pquery('SELECT tabid FROM jo_tab WHERE name=?', array('Assets'));
		$assetsTabId = $adb->query_result($assetsResult, 0, 'tabid');
		if(getTabid('CustomerPortal') && $assetsTabId) {
			$checkAlreadyExists = $adb->pquery('SELECT 1 FROM jo_customerportal_tabs WHERE tabid=?', array($assetsTabId));
			if($checkAlreadyExists && $adb->num_rows($checkAlreadyExists) < 1) {
				...
				$adb->query("INSERT INTO jo_customerportal_tabs(tabid,visible,sequence) VALUES ($assetsTabId,1,$nextSequence)");
			}
			$checkAlreadyExists = $adb->pquery('SELECT 1 FROM jo_customerportal_prefs WHERE tabid=?', array($assetsTabId));
			if($checkAlreadyExists && $adb->num_rows($checkAlreadyExists) < 1) {
				$adb->query("INSERT INTO jo_customerportal_prefs(tabid,prefkey,prefvalue) VALUES ($assetsTabId,'showrelatedinfo',1)");
			}
		}
	}
