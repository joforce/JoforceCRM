...
class Helper{
	...
	public function __construct(){
		include("modules/VTPDFMaker/mpdf/mpdf.php");
	}
	...
	public function convertFieldAndExportPDF($sourceModule, $recordIds, $tempId, $action_type = false, $templatecontent = false){
		...
		$getValue = $adb->pquery('select * from vtiger_vtpdfmaker where vtpdfmakerid = ?', array($tempIds[0]));
		...
		foreach($recordId_value as $recordId){
			$getValue = $adb->pquery('select * from vtiger_vtpdfmaker where vtpdfmakerid = ?', array($tempId));
			...
			if(is_array($fields['listviewblock_start']) && count($fields['listviewblock_start']) > 0 && is_array($fields['listviewblock_end']) && count($fields['listviewblock_end']) > 0){
				...
				$html = '';
				$count = 0;
				$get_product_details = $adb->pquery('select * from vtiger_inventoryproductrel where id = ?', array($recordId));
unset($record);
				...
				foreach($record as $single_record){
					$count = $count + 1;
					$get_product_name = $adb->pquery('select productname from vtiger_products where productid = ?', array($single_record['productid']));
					if($adb->num_rows($get_product_name) == 0){
						$get_product_name = $adb->pquery('select servicename from vtiger_service where serviceid = ?', array($single_record['productid']));
				...
				$moduleInfo = array('PurchaseOrder' => array('id' => 'purchaseorderid', 'table' => 'vtiger_purchaseorder'),
						'Invoice' => array('id' => 'invoiceid', 'table' => 'vtiger_invoice'),
						'Quotes' => array('id' => 'quoteid', 'table' => 'vtiger_quotes'),
						'SalesOrder' => array('id' => 'salesorderid', 'table' => 'vtiger_salesorder')
						);
			...
			if (is_array($fields['company']) && count($fields['company']) > 0) {
				...
				foreach ($fields['company'] as $column) {
					...
					$getValue = $adb->pquery('select * from vtiger_organizationdetails', array());
			...
			if(count($fields['terms']) > 0){
				$getTandC = $adb->pquery('select * from vtiger_inventory_tandc', array());
