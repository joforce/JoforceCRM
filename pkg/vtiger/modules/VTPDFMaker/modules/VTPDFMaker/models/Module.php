...
class VTPDFMaker_Module_Model extends Vtiger_Module_Model {
	...
	public function saveRecord($recordModel) {
		...
		if(empty($templateid)){
			$templateid = $db->getUniqueID('vtiger_vtpdfmaker');
			$sql = "INSERT INTO vtiger_vtpdfmaker(name, module, description, body, header, footer, status, settings, vtpdfmakerid) VALUES (?,?,?,?,?,?,?,?,?)";
		}else{
			$sql = "UPDATE vtiger_vtpdfmaker SET name=?, module=?, description=?, body=?, header=?, footer=?, status=?, settings=? WHERE vtpdfmakerid = ?";
		}
	...
	public function deleteRecord($recordModel) {
		...
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM vtiger_vtpdfmaker WHERE vtpdfmakerid = ? ', array($recordId));
	}
	...
	public function deleteAllRecords() {
		$db = PearDatabase::getInstance();
		$db->pquery('DELETE FROM vtiger_vtpdfmaker', array());
	}
	...
	public function getRecordIds($skipRecords){
		...
		$query = 'SELECT vtpdfmakerid FROM vtiger_vtpdfmaker WHERE vtpdfmakerid NOT IN ('.generateQuestionMarks($skipRecords).')';
		$result = $db->pquery($query, $skipRecords);
		$num_rows = $db->num_rows($result);
		$recordIds = array();
		for($i; $i<$num_rows; $i++){
			$recordIds[] = $db->query_result($result, $i, 'vtpdfmakerid');
		}
		return $recordIds;
	}
