<?php
			...
			if(($adb->num_rows($getUserId)) == 0){
				...
				if ($config['autologinActive'] == 'true') {
					...
					$checkUserId = $adb->pquery('select vtiger_user_id from users where username = ?', array($account['email']));
					$IsUpdated  = $adb->query_result($checkUserId, 0, 'vtiger_user_id');
					if(empty($IsUpdated)){
						$adb->pquery('update users set vtiger_user_id = ? where username = ?', array($user_id, $account['email']));
					}
					if ($account) {
					...
					$moduleInfo = array('Contacts' => array('id' => 'contactid', 'table' => 'vtiger_contactdetails', 'email' => 'email'),
							'Accounts' => array('id' => 'accountid', 'table' => 'vtiger_account', 'email' => 'email1'),
							'Leads' => array('id' => 'leadid', 'table' => 'vtiger_leaddetails', 'email' => 'email'));
