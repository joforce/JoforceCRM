...
class Settings_ModuleDesigner_MakePackage_Action extends Settings_Vtiger_Index_Action
{
	...
	protected function makeModuleDirectory($dirname, $moduleName, $module_dir, $o_module)
	{
		...
		{	
			...
			{
				$this->copyr("layouts/v7/modules/{$o_module->modifiedModule}", 	DIR_TEMP.$dirname.'/templates');
			}	
			//Vtiger 6
			elseif(is_dir("layouts/v7/modules/{$o_module->modifiedModule}"))
			{
				$this->copyr("layouts/v7/modules/{$o_module->modifiedModule}", DIR_TEMP.$dirname."/templates");
			}	
			//Vtiger 5	
			...
			if(empty($o_module->modifiedModulePath) && !is_dir($o_module->modifiedModulePath.'settings/templates') && is_dir("layouts/v7/modules/Settings/{$o_module->modifiedModule}"))
			{
				$this->copyr("layouts/v7/modules/Settings/{$o_module->modifiedModule}", DIR_TEMP.$dirname."/settings/templates");
			}
			...
			elseif(empty($o_module->modifiedModulePath) && file_exists('layouts/v7/skins/images/'.$o_module->modifiedModule.'.png'))
			{
				$this->copyr('layouts/v7/skins/images/'.$o_module->modifiedModule.'.png', 	DIR_TEMP.$dirname."/".$o_module->name.".png");
			}
	...
	protected function createZipPackage($dirname, $o_module)
	{
		...
		{
			$zip->copyDirectoryFromDisk(DIR_TEMP."{$dirname}/templates", "layouts/v7/modules/".$module);
		}
