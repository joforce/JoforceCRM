...
class Head_WebUI extends Head_EntryPoint {
	...
	global $site_URL;
	$query_params =	$this->getRedirectURLParams();
	if (!$this->hasLogin()) {
	    if(!empty($_SERVER['QUERY_STRING']) && !empty($query_params)) {
		$return_params = $query_params.'&'.$_SERVER['QUERY_STRING'];
	    } else {
	        $return_params = empty($_SERVER['QUERY_STRING']) ? $query_params : $_SERVER['QUERY_STRING'];
	    }
    ...
    /**
     *  Function added by aruna to get the queryparams from the redirect url
     **/
    function getRedirectURLParams() {
	global $site_URL;
	...
	if(array_key_exists(2,$routeInfo)) {
	    return http_build_query($routeInfo[2]);
	} else {
	    return false;
	}
    }
