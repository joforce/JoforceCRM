 ...
 */
function modlib_getModuleNameById($tabid) {
	global $adb;
 ...
 */
function modlib_getModuleNameForSharing() {
	global $adb;
 ...
 */
function modlib_prefetchModuleActiveInfo($force = true) {
	global $__cache_module_activeinfo;

	// Look up if cache has information
	$tabrows = CacheUtils::lookupAllTabsInfo();
	...
	if($tabrows === false || $force) {
		...
		if($tabres) {
			...
			CacheUtils::updateAllTabsInfo($tabrows);
		}
	}
 ...
 */
function modlib_isModuleActive($module) {
	global $adb, $__cache_module_activeinfo;

	if(in_array($module, modlib_moduleAlwaysActive())){
		return true;
	}
 ...
 */
function modlib_RecreateUserPrivilegeFiles() {
	global $adb;
 ...
 */
function modlib_moduleAlwaysActive() {
	$modules = Array (
		'Administration', 'CustomView', 'Settings', 'Users', 'Migration',
		'Utilities', 'uploads', 'Import', 'System', 'Workflow', 'PickList'
	);
	return $modules;
}
 ...
 */
function modlib_toggleModuleAccess($modules, $enable_disable) {
	global $adb, $__cache_module_activeinfo;
	...
	if(version_compare($jo_current_version, '5.0.4', '>')) {
		modlib_RecreateUserPrivilegeFiles();
	}
}
 ...
 */
function modlib_getToggleModuleInfo() {
	global $adb;
 ...
 */
function modlib_getToggleLanguageInfo() {
	global $adb;
 ...
 */
function modlib_toggleLanguageAccess($langprefix, $enable_disable) {
	global $adb;
 ...
 */
function modlib_getFieldHelpInfo($module) {
	global $adb;
	$fieldhelpinfo = Array();
 ...
 */
function modlib_setup_modulevars($module, $focus) {
	if($module == 'Events') $module='Calendar';

	$checkfor = Array('table_name', 'table_index', 'related_tables', 'popup_fields', 'IsCustomModule');
	foreach($checkfor as $check) {
		if(!isset($focus->$check)) $focus->$check = __modlib_get_modulevar_value($module, $check);
	}
}
function __modlib_get_modulevar_value($module, $varname) {
	$mod_var_mapping =
		Array(
			'Accounts' =>
			Array(
 ...
 */
function modlib_tosingular($text) {
 ...
 */
function modlib_getPicklistValues_AccessibleToAll($field_columnname) {
	global $adb;
 ...
 */
function modlib_getPicklistValues($field_columnname) {
	global $adb;
 ...
 */
function modlib_isCustomModule($moduleName) {
 ...
 */
function modlib_getModuleTemplate($module, $templateName) {
 ...
 */
function modlib_isWriteable($path) {
	if(is_dir($path)) {
		return modlib_isDirWriteable($path);
	} else {
 ...
 */
function modlib_isDirWriteable($dirpath) {
	if(is_dir($dirpath)) {
		do {
 ...
 * @return String
 */
function modlib_purify($input, $ignore=false) {
	...
	if(!$ignore) {
	    ...
	    include_once 'vendor/autoload.php';
            ...
            $config->set('Cache.SerializerPath', "$use_root_directory/cache/modlib");
		...
		if($__htmlpurifier_instance) {
			...
			if (is_array($input)) {
				...
				foreach ($input as $k => $v) {
					$value[$k] = modlib_purify($v, $ignore);
				}
			} else { // Simple type
 ...
 */
function modlib_purifyForSql($string, $skipEmpty=true) {
	$pattern = "/^[_a-zA-Z0-9.]+$/";
 ...
 * @return
 */
function modlib_process_widget($widgetLinkInfo, $context = false) {
...
function modlib_module_icon($modulename){
...
function modlib_mime_content_type($filename) {
