...
class HeadModuleOperation extends WebserviceEntityOperation {
	protected $tabId;
	protected $isEntity = true;
	protected $partialDescribeFields = null;
	
	public function HeadModuleOperation($webserviceObject,$user,$adb,$log){
	...
	protected function getMetaInstance(){
		if(empty(WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id])){
			WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id]  = new HeadCRMObjectMeta($this->webserviceObject,$this->user);
		}
		return WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id];
	}
	
	public function create($elementType,$element){
		$crmObject = new HeadCRMObject($elementType, false);
	...
	public function retrieve($id){
		...
		$crmObject = new HeadCRMObject($this->tabId, true);
    ...
    public function relatedIds($id, $relatedModule, $relatedLabel, $relatedHandler=null) {
		...
		$sourceRecordModel = Head_Record_Model::getInstanceById($ids[1], $sourceModule);
		$targetModel       = Head_RelationListView_Model::getInstance($sourceRecordModel, $relatedModule, $relatedLabel);
        $sql = $targetModel->getRelationQuery();

        $relatedWebserviceObject = HeadWebserviceObject::fromName($adb,$relatedModule);
        ...
        $sql = sprintf("SELECT DISTINCT concat('%sx',jo_crmentity.crmid) as wsid FROM %s", $relatedModuleWSId, $sqlFromPart);
