...
require_once 'includes/Webservices/HeadCRMActorMeta.php';
class HeadActorOperation extends WebserviceEntityOperation {
	...
	protected function getMetaInstance(){
		if(empty(WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id])){
			WebserviceEntityOperation::$metaCache[$this->webserviceObject->getEntityName()][$this->user->id]  
					= new HeadCRMActorMeta($this->entityTableName,$this->webserviceObject,$this->pearDB,$this->user);
		}
	...
	protected function getActorTables(){
		...
		$sql = 'select table_name, webservice_entity_id from jo_ws_entity_tables';
