...
class HeadCRMActorMeta extends EntityMeta {
	protected $pearDB;
	protected static $fieldTypeMapping = array();
	protected static $referenceTypeMapping = array();
	
	function HeadCRMActorMeta($tableName,$webserviceObject,$adb,$user){
	...
	protected function getReferenceList($dbField, $tableName){
		...
		if(!isset(HeadCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name])){
			$this->getFieldType($dbField, $tableName);
		}
		$fieldTypeData = HeadCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name];
		
		if (empty(HeadCRMActorMeta::$referenceTypeMapping)) {
			$sql = "SELECT * FROM jo_ws_entity_referencetype";
			...
			for ($index =0, $count = $this->pearDB->num_rows($result); $index < $count; ++$index) {
				...
				HeadCRMActorMeta::$referenceTypeMapping[$row['fieldtypeid']][] = $row['type'];
			}
		}
		
		$referenceTypes = array();
		if (isset(HeadCRMActorMeta::$referenceTypeMapping[$fieldTypeData['fieldtypeid']])) {
			$referenceTypes = HeadCRMActorMeta::$referenceTypeMapping[$fieldTypeData['fieldtypeid']];
		}
	...
	protected function getFieldType($dbField,$tableName){
		...
		if(isset(HeadCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name])){
			if(HeadCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name] === 'null'){
				return null;
			}
			$row = HeadCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name];
			return $row['fieldtype'];
		}
		
		if (empty(HeadCRMActorMeta::$fieldTypeMapping)) {
			// Optimization to avoid repeated initialization
			$sql = "select * from jo_ws_entity_fieldtype";
			$result = $this->pearDB->pquery($sql,array());
			$rowCount = $this->pearDB->num_rows($result);
			while ($rowCount) {
				$row = $this->pearDB->query_result_rowdata($result,$rowCount-1);
				HeadCRMActorMeta::$fieldTypeMapping[$row['table_name']][$row['field_name']] = $row;
				--$rowCount;
			}
		}
		
		if (!isset(HeadCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name])) {
			HeadCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name] = 'null';
			return null;
		}
		return HeadCRMActorMeta::$fieldTypeMapping[$tableName][$dbField->name]['fieldtype'];
	}
	...
	public function getNameFields(){
		$query = "select name_fields from jo_ws_entity_name where entity_id = ?";
	...
	public function getEntityDeletedQuery() {
		if($this->getEntityName() == 'Currency'){
			return 'jo_currency_info.deleted=0';
		}
