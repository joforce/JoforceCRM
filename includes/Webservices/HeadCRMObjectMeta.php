...
class HeadCRMObjectMeta extends EntityMeta {
	...
	function HeadCRMObjectMeta($webserviceObject,$user){
		...
		if($instance->tab_name && in_array('jo_crmentity',$instance->tab_name)){
			$this->defaultTableList = array('jo_crmentity');
		}else{
	...
	private function computeAccess(){
		...
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0){
			...
			$sql = "select * from jo_profile2globalpermissions where profileid in (".generateQuestionMarks($profileList).");";
			...
			$sql = 'select * from jo_profile2tab where profileid in ('.generateQuestionMarks($profileList).') and tabid = ? and permissions = ?';
			...
			$sql = "select * from jo_profile2standardpermissions where profileid in (".generateQuestionMarks($profileList).") and tabid=?";
	...
	function hasAssignPrivilege($webserviceId){
		...
		$webserviceObject = HeadWebserviceObject::fromId($adb,$ownerTypeId);
	...
	private function retrieveMetaForBlock($block){
		...
		if($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] ==0){
			...
			if (count($profileList) > 0) {
				$sql = "SELECT jo_field.*, jo_profile2field.readonly
						FROM jo_field
						...
						WHERE jo_field.tabid =? AND jo_profile2field.visible = 0 
						AND jo_profile2field.profileid IN (". generateQuestionMarks($profileList) .")
						AND jo_def_org_field.visible = 0 and jo_field.block in (".generateQuestionMarks($block).") and jo_field.displaytype in (1,2,3,4,5,6) and jo_field.presence in (0,2) group by columnname";
				...
				$sql = "SELECT jo_field.*, jo_profile2field.readonly
						...
						AND jo_profile2field.visible = 0 
						AND jo_def_org_field.visible = 0 and jo_field.block in (".generateQuestionMarks($block).") and jo_field.displaytype in (1,2,3,4,5,6) and jo_field.presence in (0,2) group by columnname";
				$params = array($tabid, $block);
			}
		}
	...
	function getObjectEntityName($webserviceId){
		...
		if($this->objectName == 'Users'){
			$sql = "select user_name from jo_users where id=? and deleted=0";
			...
			$sql = "select setype from jo_crmentity where crmid=? and deleted=0";
			$result = $adb->pquery($sql , array($id));
			if($result != null && isset($result)){
				if($adb->num_rows($result)>0){
	...
	function exists($recordId){
		...
		if($this->objectName == 'Users'){
			if (array_key_exists($recordId, $user_exists_cache)) {
				...
				$sql = "select 1 from jo_users where id=? and deleted=0 and status='Active'";
			}
			
		}else{
			$sql = "select 1 from jo_crmentity where crmid=? and deleted=0 and setype='".
				$this->getTabName()."'";
		}
