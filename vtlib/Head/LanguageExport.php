 ...
 * @package vtlib
 */
class Head_LanguageExport extends Head_Package {

	const TABLENAME = 'jo_language';
	...
	function __initExport($languageCode, $moduleInstance = null) {
		...
		Head_Utils::checkFileAccessForInclusion("languages/$languageCode/Head.php");
	...
	function export_Language($prefix) {
		...
		$sqlresult = $adb->pquery("SELECT * FROM jo_language WHERE prefix = ?", array($prefix));
