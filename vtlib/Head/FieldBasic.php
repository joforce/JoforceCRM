...
class Head_FieldBasic {
	 ...
	 * @param Array 
	 * @param Head_Module Instance of module to which this field belongs
	 * @param Head_Block Instance of block to which this field belongs
	 * @access private
	 */
	function initialize($valuemap, $moduleInstance=false, $blockInstance=false) {
		...
		$this->block= $blockInstance? $blockInstance : Head_Block::getInstance($valuemap['block'], $moduleInstance);
	...
	function __handleHeadCoreSchemaChanges() {
		// Add helpinfo column to the jo_field table
		if (empty(self::$__cacheSchemaChanges['jo_field.helpinfo'])) {
			Head_Utils::AddColumn('jo_field', 'helpinfo', ' TEXT');
			self::$__cacheSchemaChanges['jo_field.helpinfo'] = true;
		}
		if (empty(self::$__cacheSchemaChanges['jo_field.summaryfield'])) {
			Head_Utils::AddColumn('jo_field', 'summaryfield', ' INT(10) NOT NULL DEFAULT 0');
			self::$__cacheSchemaChanges['jo_field.summaryfield'] = 0;
		}
		if (empty(self::$__cacheSchemaChanges['jo_field.headerfield'])) {
			Head_Utils::AddColumn('jo_field', 'headerfield', ' INT(1) DEFAULT 0');
			self::$__cacheSchemaChanges['jo_field.headerfield'] = 0;
		}
	}
	...
	function __getUniqueId() {
		global $adb;
		return $adb->getUniqueID('jo_field');
	}
	...
	function __getNextSequence() {
		global $adb;
		$result = $adb->pquery("SELECT MAX(sequence) AS max_seq FROM jo_field WHERE tabid=? AND block=?", Array($this->getModuleId(), $this->getBlockId()));
		$maxseq = 0;
	...
	function __getNextQuickCreateSequence() {
		global $adb;
		$result = $adb->pquery("SELECT MAX(quickcreatesequence) AS max_quickcreateseq FROM jo_field WHERE tabid=?", Array($this->getModuleId()));
	 ...
	 * @param Head_Block Instance of the block to use
	 * @access private
	 */
	function __create($blockInstance) {
		...
		if (!empty($this->columntype)) {
			Head_Utils::AddColumn($this->table, $this->column, $this->columntype);
		}
		...
		$adb->pquery("INSERT INTO jo_field (tabid, fieldid, columnname, tablename, generatedtype,
		...
		$adb->pquery('UPDATE jo_field SET masseditable=? WHERE fieldid=?', Array($this->masseditable, $this->id));

		Head_Profile::initForField($this);
	 ...
	 * @param Head_Block Instance of block to which this field should be added.
	 */
	function save($blockInstance = false) {
		...
		// Clearing cache
		Head_Cache::flushModuleandBlockFieldsCache($this->getModuleInstance(), $this->getBlockId());
		return $this->id;
	}
	...
	function setHelpInfo($helptext) {
		...
		global $adb;
		$adb->pquery('UPDATE jo_field SET helpinfo=? WHERE fieldid=?', Array($helptext, $this->id));
	...
	function setMassEditable($value) {
		global $adb;
		$adb->pquery('UPDATE jo_field SET masseditable=? WHERE fieldid=?', Array($value, $this->id));
    ...
    function setSummaryField($value) { 
        global $adb; 
        $adb->pquery('UPDATE jo_field SET summaryfield=? WHERE fieldid=?', Array($value, $this->id)); 
