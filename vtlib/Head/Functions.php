...
class Head_Functions {
	...
	static function getTranslatedString($str, $module='', $language='') {
		return Head_Language_Handler::getTranslatedString($str, $module, $language);
	}
	...
	static function userCurrencyId($userid) {
		...
		if (!isset(self::$userIdCurrencyIdCache[$userid])) {
			$result = $adb->pquery('SELECT id,currency_id FROM jo_users', array());
	...
	protected static function getCurrencyInfo($currencyid) {
		...
		if (!isset(self::$currencyInfoCache[$currencyid])) {
			$result = $adb->pquery('SELECT * FROM jo_currency_info', array());
			while ($row = $adb->fetch_array($result)) {
	...
	static function getCurrencyName($currencyid, $show_symbol = true) {
		...
		if ($show_symbol) {
			return sprintf("%s : %s", Head_Deprecated::getTranslatedCurrencyString($currencyInfo['currency_name']), $currencyInfo['currency_symbol']);
		}
	...
	protected static function getBasicModuleInfo($mixed) {
		...
		if ($reload) {
			global $adb;
			$result = $adb->pquery('SELECT tabid, name, ownedby FROM jo_tab', array());
			while ($row = $adb->fetch_array($result)) {
	...
	static function getEntityModuleInfo($mixed) {
		...
		if ($name && !isset(self::$moduleEntityCache[$name])) {
			global $adb;
			$result = $adb->pquery('SELECT fieldname,modulename,tablename,entityidfield,entityidcolumn from jo_entityname', array());
			while ($row = $adb->fetch_array($result)) {
	...
	protected static function getCRMRecordMetadata($mixedid) {
		...
		if ($missing) {
			$sql = sprintf("SELECT crmid, setype, label FROM jo_crmentity WHERE %s", implode(' OR ', array_fill(0, count($missing), 'crmid=?')));
	...
	static function updateCRMRecordLabel($module, $id) {
		...
		if ($labelInfo) {
			...
			$adb->pquery('UPDATE jo_crmentity SET label=? WHERE crmid=?', array($label, $id));
	...
	static function computeCRMRecordLabels($module, $ids) {
		...
		if ($module) {
			...
			if ($ids) {
				...
				if ($module == 'Groups') {
					$metainfo = array('tablename' => 'jo_groups','entityidfield' => 'groupid','fieldname' => 'groupname');
				} else if ($module == 'DocumentFolders') {
					$metainfo = array('tablename' => 'jo_attachmentsfolder','entityidfield' => 'folderid','fieldname' => 'foldername'); 
				} else {
	...
	static function getGroupName($id) {
		...
		if (!self::$groupIdNameCache[$id]) {
			$result = $adb->pquery('SELECT groupid, groupname FROM jo_groups');
			while ($row = $adb->fetch_array($result)) {
	...
	static function getUserName($id) {
		...
		if (!self::$userIdNameCache[$id]) {
			$result = $adb->pquery('SELECT id, user_name FROM jo_users');
			while ($row = $adb->fetch_array($result)) {
	...
	static function getModuleFieldInfos($mixed) {
		...
		if(Head_Cache::get('ModuleFieldInfo',$module)){
			return Head_Cache::get('ModuleFieldInfo',$module);
		}

		if ($module) {
			$result = $adb->pquery('SELECT * FROM jo_field WHERE tabid=?', array(self::getModuleId($module)));

			while ($row = $adb->fetch_array($result)) {
				$moduleFieldInfo[$module][$row['fieldname']] = $row;
			}
			Head_Cache::set('ModuleFieldInfo',$module,$moduleFieldInfo[$module]);
		}
	...
	static function getModuleFieldInfoWithId($fieldid) {
		global $adb;
		$result = $adb->pquery('SELECT * FROM jo_field WHERE fieldid=?', array($fieldid));
	...
	static function getInventoryTermsAndCondition($moduleName) {
		global $adb;
		$sql = 'SELECT tandc FROM jo_inventory_tandc WHERE type = ?';
	...
	/**
	 * Function to get group permissions given to config/config.inc.php file
	 * @return type
	 */
	static function getGroupPermissionsFromConfigFile(){
		$rootDirectory = vglobal('root_directory');
		return exec("ls -l $rootDirectory/config/config.inc.php | awk 'BEGIN {OFS=\":\"}{print $3,$4}'");
	}
	...
	static function validateImage($file_details) {
		...
		$imageContents = file_get_contents($file_details['tmp_name']);
		if (!getimagesize($file_details['tmp_name'])) {
                         $saveimage = 'false';
                }
                if (preg_match('[\x01-\x08\x0c-\x1f]', $imageContents)) {
                         $saveimage = 'false';
                }
		return $saveimage;
	}
	...
	static function getTicketComments($ticketid) {
		...
		$commentlist = '';
		$sql = "SELECT commentcontent FROM jo_modcomments WHERE related_to = ?";
	...
	static function transformFieldTypeOfData($table_name, $column_name, $type_of_data) {
		...
		static $new_field_details = Array(
			...
			"jo_contactdetails:accountid" => "V",
			"jo_contactsubdetails:birthday" => "D",
			"jo_contactdetails:email" => "V",
			"jo_contactdetails:secondaryemail" => "V",
			//Potential Related Fields
			"jo_potential:campaignid" => "V",
			//Account Related Fields
			"jo_account:parentid" => "V",
			"jo_account:email1" => "V",
			"jo_account:email2" => "V",
			//Lead Related Fields
			"jo_leaddetails:email" => "V",
			"jo_leaddetails:secondaryemail" => "V",
			//Documents Related Fields
			"jo_senotesrel:crmid" => "V",
			//Calendar Related Fields
			"jo_seactivityrel:crmid" => "V",
			"jo_seactivityrel:contactid" => "V",
			"jo_recurringevents:recurringtype" => "V",
			//HelpDesk Related Fields
			"jo_troubletickets:parent_id" => "V",
			"jo_troubletickets:product_id" => "V",
			//Product Related Fields
			"jo_products:discontinued" => "C",
			...
			"jo_quotes:inventorymanager" => "V",
			"jo_quotes:accountid" => "V",
			//Purchase Order Related Fields
			"jo_purchaseorder:vendorid" => "V",
			"jo_purchaseorder:contactid" => "V",
			//SalesOrder Related Fields
			"jo_salesorder:potentialid" => "V",
			"jo_salesorder:quoteid" => "V",
			"jo_salesorder:contactid" => "V",
			"jo_salesorder:accountid" => "V",
			//Invoice Related Fields
			"jo_invoice:salesorderid" => "V",
			"jo_invoice:contactid" => "V",
			"jo_invoice:accountid" => "V",
			//Campaign Related Fields
			"jo_campaign:product_id" => "V",
			//Related List Entries(For Report Module)
			"jo_activityproductrel:activityid" => "V",
			"jo_activityproductrel:productid" => "V",
			"jo_campaigncontrel:campaignid" => "V",
			"jo_campaigncontrel:contactid" => "V",
			"jo_campaignleadrel:campaignid" => "V",
			"jo_campaignleadrel:leadid" => "V",
			"jo_cntactivityrel:contactid" => "V",
			"jo_cntactivityrel:activityid" => "V",
			"jo_contpotentialrel:contactid" => "V",
			"jo_contpotentialrel:potentialid" => "V",
			"jo_pricebookproductrel:pricebookid" => "V",
			"jo_pricebookproductrel:productid" => "V",
			"jo_seactivityrel:crmid" => "V",
			"jo_seactivityrel:activityid" => "V",
			"jo_senotesrel:crmid" => "V",
			"jo_senotesrel:notesid" => "V",
			"jo_seproductsrel:crmid" => "V",
			"jo_seproductsrel:productid" => "V",
			"jo_seticketsrel:crmid" => "V",
			"jo_seticketsrel:ticketid" => "V",
			"jo_vendorcontactrel:vendorid" => "V",
			"jo_vendorcontactrel:contactid" => "V",
			"jo_pricebook:currency_id" => "V",
		);
	...
	static function getPickListValuesFromTableForRole($tablename, $roleid) {
		global $adb;
		$query = "select $tablename from jo_$tablename inner join jo_role2picklist on jo_role2picklist.picklistvalueid = jo_$tablename.picklist_valueid where roleid=? and picklistid in (select picklistid from jo_picklist) order by sortid";
	...
	static function getActivityType($id) {
		global $adb;
		$query = "select activitytype from jo_activity where activityid=?";
		$res = $adb->pquery($query, array($id));
	...
	static function getInvoiceStatus($id) {
		global $adb;
		$result = $adb->pquery("SELECT invoicestatus FROM jo_invoice where invoiceid=?", array($id));
	...
	*/
	static function getUnitPrice($productid, $module='Products') {
		...
		if($module == 'Services') {
			$query = "select unit_price from jo_service where serviceid=?";
		} else {
			$query = "select unit_price from jo_products where productid=?";
		}
	...
	*/
	...
	public static function getMandatoryReferenceFields($module, $mode = 'label') {
		...
		foreach ($referenceFields as $field => $fieldModel) {
			...
			if ($fieldModel->isMandatory() && !empty($referenceModule) && !in_array($uiType, array(50, 51, 52))) {
				$recordName = '?????';
				$result = $db->pquery("SELECT crmid, label FROM jo_crmentity WHERE label LIKE ? AND deleted = ? AND setype = ?", array("%$recordName%", 0, $referenceModule));
				if ($db->num_rows($result) < 1) {
					$moduleModel = Head_Module_Model::getInstance($referenceModule);
					$recordModel = Head_Record_Model::getCleanInstance($referenceModule);
					...
					foreach ($fieldInstances as $blockInstance) {
						foreach ($blockInstance as $fieldInstance) {
							...
							$referenceRelationFields = Head_Functions::getMandatoryReferenceFields($referenceModule, 'id');
					...
					$recordModel->save();
					if ($mode == 'label') {
						$recordName = Head_Util_Helper::getRecordName($recordModel->getId());
					} else {
	...
	static function setEventsContactIdToRequest($recordId) {
		$db = PearDatabase::getInstance();
		$contactIds = array();
		$result = $db->pquery("SELECT contactid FROM jo_cntactivityrel WHERE activityid = ?", array($recordId));
	 ...
	 static function getNonQuickCreateSupportedModules() {
		...
		$modules = Head_Module_Model::getAll(array(0, 2));
	...
	public static function isUserExist($userId) {
		$adb = PearDatabase::getInstance();
		$query = "SELECT 1 FROM jo_users WHERE id=? AND deleted=? AND status = ?";
